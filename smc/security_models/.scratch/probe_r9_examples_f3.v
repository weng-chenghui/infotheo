(* Probe R9 — the material of the permanent smc/security_models/examples_f3.v
   that is NOT already compiled elsewhere:
     - mask_chan_unif3 and statdist_mask_chan (needed by the eps verdict);
     - the Module masking_verdicts kernel instantiation and its three
       verdicts (perfect / eps / insecure);
     - the coin_leak headline centropy_view_honest_neq, which puts the two
       halves of the C1 certificate side by side.
   Everything else is copied from the already-compiled probe_examples_f3.v,
   audit_snd_randomized_F.v and audit_it_delivery_law.v Part A.

   The body below is kept identical to the landed examples_f3.v, so this file
   is the compiled record of the port and of the Print Assumptions run at the
   bottom.
   Compile from repo root:
     /Users/cheng-huiweng/Projects/coq/_opam/bin/coqc -R . infotheo \
       smc/security_models/.scratch/probe_r9_examples_f3.v                  *)
...
(* MUTATION CHECKS — both copies live in this directory, both are rejected by
   coqc with exit status 1.
   1. probe_r9_examples_f3_mut1.v : masking_verdicts.eps_privacy_biased
      restated at 12%:R^-1.  At the closing by, the residual goal is
      6%:R^-1 <= 12%:R^-1:
        Error: No applicable tactic.
      The epsilon of the approximate verdict is therefore tight.
   2. probe_r9_examples_f3_mut2.v : coin_leak.centropy_view_honest_neq
      restated as the equality eq:smc:entropy asserts.  At the closing by, the
      residual goal is 0 = 1:
        Error: No applicable tactic.
   3. The third verdict needs the degenerate ancilla: insecurity_no_mask
      restated at uniform_mask/mask_view is refuted in the permanent file
      itself by masking_verdicts.perfect_privacy_uniform, which exhibits the
      simulator whose nonexistence the mutated statement claims.            *)
