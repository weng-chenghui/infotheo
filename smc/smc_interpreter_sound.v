...
(* (if Send/Recv) to find the applicable reduction, returning None for inert  *)
(* indices. This absorbs the partner-searching logic that would otherwise     *)
(* appear inline in step_sound's Send and Recv branches.                      *)
(*                                                                            *)
(* Key property: reduction_spec_at canonicalizes Send/Recv into a single      *)
(* RScomm (sender-first), eliminating the symmetry duplication where a naive  *)
(* proof handles Send-then-find-Recv and Recv-then-find-Send separately.      *)
(******************************************************************************)

(* Canonical packaging of the three reduction kinds (Init, Ret, matched
   Send/Recv), collapsing the 6-constructor proc analysis to 3 cases. *)
Inductive reduction_spec n : Type :=
...
Definition reduction_spec_lens {n} (r : reduction_spec n) :
    lens n (reduction_spec_size r) :=
  match r return lens n (reduction_spec_size r) with
...
Definition reduction_spec_indices {n} (r : reduction_spec n) : seq 'I_n :=
  tval (reduction_spec_lens r).

(* Input processes — what the reduction expects *)
Definition reduction_spec_input {n} (r : reduction_spec n) :
    (reduction_spec_size r).-tuple (proc data) :=
  match r return (reduction_spec_size r).-tuple (proc data) with
...
Definition reduction_spec_output {n} (r : reduction_spec n) :
    (reduction_spec_size r).-tuple (proc data) :=
  match r return (reduction_spec_size r).-tuple (proc data) with
...
Definition reduction_spec_traces {n} (r : reduction_spec n) :
    (reduction_spec_size r).-tuple (seq data) :=
  match r return (reduction_spec_size r).-tuple (seq data) with
...
Definition reduction_applies {n} (ps : n.-tuple (proc data)) (r : reduction_spec n) :
    Prop :=
  extract (reduction_spec_lens r) ps = reduction_spec_input r :> seq _.
...
rewrite /reduction_applies /reduction_spec_lens /reduction_spec_input /extract /=.
by move=> [] ->.
Qed.
...
(******************************************************************************)
(* Sender-only variant: step_result_reduction_sender                          *)
(*                                                                            *)
(* Takes b ∈ pss as a direct hypothesis. Only handles the i=a (sender) case  *)
(* for RScomm, since the list-based approach processes each RScomm once from  *)
(* its sender index.                                                          *)
(******************************************************************************)
...
- (* RScomm - sender side only *)
  ...
  * move: (rcomm a b y pa pb).
    ...
    apply/val_inj.
    rewrite /= /result_procs !(tnth_map, tnth_mktuple, tnth_ord_tuple).
...
(*                                                                            *)
(* Build a deduplicated list of reduction_specs (one per reduction, sender-side     *)
...
- rewrite /is_sender in Hsender.
  ...
  exists j; split.
  + rewrite /active_senders mem_filter /has_reduction /is_sender Hpj /= mem_enum andbT.
    rewrite /reduction_spec_at Hpj /=.
...
- (* Inductive case *)
  ...
  + move=> a b x pa pb Hr_comm.
    ...
    split => //.
    have Hb_idx : b \in reduction_spec_indices (RScomm a b x pa pb)
      by rewrite /= !inE eqxx orbT.
  ...
  + apply: IHs => //.
    * move=> i Hi_in r' Hr' j Hj.
      ...
      -- rewrite !inE Hj_pss andbT.
         by have /= := Hrdisj j s Hj (reduction_spec_at_mem Hr_eq).
      -- rewrite !inE Hj_pss !andbT.
         have Ha_idx : a \in reduction_spec_indices (RScomm a b y pa pb)
           by rewrite /= !inE eqxx.
         have Hb_idx : b \in reduction_spec_indices (RScomm a b y pa pb)
           by rewrite /= !inE eqxx orbT.
    ...
    * move=> i Hi_pss' Hi_notin.
      ...
      -- rewrite big_cons inE negb_or; apply/andP; split; last exact: Hi_notin.
         ...
         ++ rewrite !inE in Hi_pss'.
            ...
            have := reduction_spec_at_mem Hr_eq; rewrite /= !inE =>
              /orP [/eqP Hsa | /eqP Hsb].
...
have Hrstep1 := reduction_spec_at_rstep Hr1.
have Hrstep2 := reduction_spec_at_rstep Hr2.
have indices_is_lens : forall m (s : reduction_spec m),
    reduction_spec_indices s = tval (reduction_spec_lens s) by [].
...
- move=> [/eqP Hlens_eq [_ _]].
  have Hi1_mem := reduction_spec_at_mem Hr1.
  have Hi2_mem := reduction_spec_at_mem Hr2.
  exfalso; move: Hneq; apply/negP/negPn.
  have Hi2_lens : i2 \in tval (reduction_spec_lens r2) by rewrite -indices_is_lens.
  ...
  + rewrite /= !inE in Hi1_mem Hi2_in_r1.
    have Happ := reduction_spec_at_applies Hr1.
    rewrite /reduction_applies /= in Happ; case: Happ => Hpa Hpb.
    have Hi1_eq : i1 = a.
...
(* 1. Bridge functional step to step_result on the full index set               *)
(* 2. Strip inert indices via step_result_strip_inert                           *)
