...
(* Note: we cannot assume WmZ (x1') are uniform distributed,
   Because it is party#1's input (x1) + random vector s1.
   Unless there is a lemma for:

   "for all RVs, adding a uniform distributed RV yields another uniform distributed RV".

   -- But this won't be true because even adding two uniform distributed RVs won't
      yield a uniform distributed RV.
*)
Let pWmZ_unif:
  `p_ WmZ = fdist_uniform card_rVTX.
Proof.
rewrite /WmZ eq_Wm_RV.
Abort.

(* The result of no way to prove pWmZ_unif (x1' is uniform distributed)
   is that cpr_cond_entropy cannot be applied, which means this won't be held:

   `H( x1 | [% [%x2, s2], x1']) = `H( x1 | [%x2, s2])

   Unless there is another version of cpr_cond_entropy doesn't require the pWmZ_unif.
*)
Lemma eqn8_proof:
  `H(x1|[%[%x2, s2], x1']) = `H(x1|[%x2, s2]).
Proof.
...
rewrite -/W1 -/W2 -/WmZ.
Fail have Ha := cpr_cond_entropy pWmZ_unif W2_WmZ_indep _.
Fail apply: Ha => w w2 wm Hneq0.
Fail rewrite pr_eq_pairC in Hneq0.
rewrite -/W1.
Fail by have := (mc_removal_pr f1 Z_O_indep pZ_unif w Hneq0).
Abort.
  
End eqn8_proof.

Section pi2_bob_view_is_leakage_free.
...
Fail  by rewrite eqn8_proof.
Fail by rewrite eqn_8_1.
Abort.

End pi2_bob_view_is_leakage_free.
