...
Definition sum_fuel {dtype : eqType} {data} (ps : seq (aproc dtype data)) :=
...
Definition stype_stuck (ps : seq (stype dtype)) i :=
  let p := nth STEnd ps i in
  match p with
  | STRecv j ti k =>
      match nth STEnd ps j with
      | STSend i' tj _ => (i == i') && (ti != tj)
      | STRecv i' tj _ => (i == i')
      | STEnd => true
      end
  | STSend j ti k =>
      match nth STEnd ps j with
      | STRecv i' tj _ => (i == i') && (ti != tj)
      | STSend i' tj _ => (i == i')
      | STEnd => true
      end
  | STEnd => false
...
Lemma mem_iota0 n i : i \in iota 0 n = (i < n).
Proof. by rewrite mem_iota leq0n. Qed.

Lemma nth_map_iota0 A (a : A) n i f :
  nth a (map f (iota 0 n)) i = if i < n then f i else a.
Proof.
case: ifPn => Hi.
- by rewrite (nth_map n) (nth_iota,size_iota).
- by rewrite nth_default // size_map size_iota leqNgt.
Qed.

Lemma map_iota_tuple A (f : nat -> A) n :
  map f (iota 0 n) = [tuple f i | i < n].
Proof. by rewrite /mktuple /= [RHS]map_comp val_enum_ord. Qed.

Lemma stypes_interp_stuck h ps :
  has (stype_stuck ps) (iota 0 (size ps)) ->
  size (stypes_interp h ps) = size ps ->
  ~~ all (eq_op STEnd) (stypes_interp h ps).
Proof.
elim: h ps => [|h IH] ps /hasP[i]; rewrite mem_iota0 /= => Hi Hs.
  by move=> Hsz; rewrite -Hsz ltn0 in Hi.
case: ifP => Hhas Hsz.
  rewrite IH // !size_map size_iota //.
  apply/hasP; exists i; first by rewrite mem_iota0 Hi.
  move: Hs.
  rewrite /stype_stuck /unzip1 -map_comp nth_map_iota0 Hi /= {1}/stype_step.
  case Hnthi: nth => //= [n dt p | n dt p].
  - case/boolP: (n < size ps) => Hn; last first.
      rewrite nth_default /=; last by rewrite leqNgt.
      by rewrite nth_map_iota0 (negbTE Hn).
    case Hnth : nth => /= [n' dt' p' | n' dt' p' |].
    + move/eqP => in'; subst n'.
      by rewrite nth_map_iota0 Hn /= /stype_step Hnth Hnthi /= eqxx.
    + case/andP => /eqP in' dtdt'; subst n'.
      rewrite eqxx (negbTE dtdt') /= nth_map_iota0 Hn /= /stype_step.
      by rewrite Hnth Hnthi /= eqxx eq_sym (negbTE dtdt') /= eqxx.
    + by rewrite nth_map_iota0 Hn /= /stype_step Hnth.
  - case/boolP: (n < size ps) => Hn; last first.
      rewrite nth_default /=; last by rewrite leqNgt.
      by rewrite nth_map_iota0 (negbTE Hn).
    case Hnth : nth => //= [n' dt' p' | n' dt' p' |].
    ...
    + by rewrite nth_map_iota0 Hn /= /stype_step Hnth.
rewrite -has_predC.
apply/hasP; exists (nth STEnd ps i).
  by rewrite mem_nth.
by move: Hs; rewrite /stype_stuck; case: nth.
Qed.
...
Definition stypes_compat ps :=
  all (eq_op STEnd) (stypes_interp (stypes_interp_fuel ps) ps).

Lemma stypes_interp_fuel_step ps :
  let ps' := [seq stype_step ps i | i <- iota 0 (size ps)] in
  has snd ps' ->
  stypes_interp_fuel (unzip1 ps') <= sumn [seq senv_depth i | i <- ps].
Proof.
move=> /= Hh.
rewrite /stypes_interp_fuel addn1 -ltnS.
move: Hh; rewrite has_map => /hasP[] /= i.
rewrite mem_iota0 => Hi psi.
rewrite -{3}(map_nth_iota_id STEnd ps) !sumnE ltnS.
rewrite !big_map -(subn0 (size ps)) -/(index_iota _ _) !big_mkord.
rewrite (bigD1 (Ordinal Hi)) //= [X in _ < X](bigD1 (Ordinal Hi)) //=.
rewrite -addSn leq_add //.
  move: psi; rewrite /stype_step.
  case: nth => [p d s|p d s|] //=;
  case: nth => [p' d' s'|p' d' s'|] //=;
  by case: ifP.
apply: leq_sum => j _.
rewrite /stype_step.
...
set ps' := map _ _.
case: ifP => // Hh.
have := stypes_interp_fuel_step Hh.
rewrite Heqh' => Hh'.
rewrite IH; by [rewrite [RHS]IH | rewrite ltnS | exact: leq_trans hh'].
Qed.

Lemma stypes_interp_fuel_size ps h :
  h >= stypes_interp_fuel ps ->
  size (stypes_interp h ps) = size ps.
Proof.
rewrite /stypes_interp_fuel addn1.
elim: h ps => //= h IH ps.
rewrite ltnS => Hfuel.
case: ifP => // /stypes_interp_fuel_step Hh.
by rewrite IH ?(size_map,size_iota) // -addn1 (leq_trans Hh).
Qed.

Lemma stypes_compat_not_stuck ps :
  stypes_compat ps ->
  ~~ has (stype_stuck ps) (iota 0 (size ps)).
Proof.
rewrite /stypes_compat => Hcompat.
apply/negP => /(stypes_interp_stuck (h:=stypes_interp_fuel ps)).
by rewrite Hcompat stypes_interp_fuel_size // => /(_ erefl).
Qed.

Variable data : Type.

Definition aprocs_compat (ps : seq (aproc dtype data)) :=
  stypes_compat (map aproc_env ps).

Lemma aprocs_compat_not_stuck ps :
  aprocs_compat ps ->
  ~~ has (stype_stuck (map aproc_env ps)) (iota 0 (size ps)).
Proof. by move/stypes_compat_not_stuck; rewrite size_map. Qed.
...
Lemma fuel_senv_decreases (ps : seq (aproc dtype data)) k tr :
  k < size ps ->
  let res := step (erase_aprocs ps) (nth [::] tr k) k in
  { ap' | erase_aproc ap' = res.1.1 /\
      aproc_fuel ap' + res.2 <= aproc_fuel (nth aproc_default ps k) /\
...
elim: h ps traces => [|h IH] ps traces.
  ...
  by rewrite mem_iota0 /step nth_nseq => /= ->.
move=> Hps /=.
...
rewrite mem_iota0 size_map => /= Hk Hck.
set traces' := unzip2 _.
...
rewrite -{3}(subn0 (size ps)) -/(index_iota _ _) !big_mkord.
rewrite -subn_gt0 -sumnB.
  rewrite lt0n sum_nat_seq_neq0.
  apply/hasP; exists (Ordinal Hk); first by rewrite mem_index_enum.
  rewrite -lt0n subn_gt0 -addn1 (_ : 1 = true) //=.
  have [_] /= := Haps' (Ordinal Hk).
  by rewrite Hck (tnth_nth aproc_default).
move=> /= i _.
have [_] /= := Haps' i.
...
have [aps' Haps'] :=
  dependent_mktuple (fun k : 'I_(size ps) => 
    fuel_senv_decreases traces (ltn_ord k)).
exists aps'.
have Hsz: size aps' = size ps by rewrite size_tuple.
split.
  (* erase_aprocs aps' = ps' *)
...
split.
  ...
  rewrite -ltnS (leq_trans _ Hps) // ?(ltnW Hk) // /sum_fuel sumnE big_map.
  rewrite -{3}(map_nth_iota_id aproc_default ps) big_map.
  rewrite -{3}(subn0 (size ps)) -/(index_iota _ _) -subn_gt0 !big_mkord -sumnB.
    rewrite lt0n sum_nat_seq_neq0.
    apply/hasP; exists (Ordinal Hk); first by rewrite mem_index_enum.
...
rewrite (big_nth aproc_default) /= big_mkord.
apply/bigmax_leqP => i _.
have [_ [_ ->]] := Haps' i.
apply: leq_trans (leq_bigmax i).
by case: ifP => _ //; case: aproc_env.
Qed.
...
Definition tnthE :=
  (tnth_ord_tuple,tnth_mktuple,tnth_map).
(*    (fun n T x t i => esym (@tnth_nth n T x t i))). *)
...
try (exists [::];
     rewrite !inject_same //; [exact: rrefl | by rewrite tnthE]).
- case: (IH (inject [tuple i] ps [tuple q])).
    by rewrite tnthE /= eqxx.
  move=> tr.
  rewrite inject_dup => Hr.
  exists (tr ++ [:: d]).
  apply: (rtrans (rone (l:=[tuple i]) _) Hr).
    rewrite extract1 Hp.
    exact: rinit.
  apply: eq_mktuple => j.
  rewrite !tnthE /=.
...
Proof.
rewrite /aprocs_compat -map_comp /is_true => <-.
congr stypes_compat.
apply: eq_map => p /=.
...
Hypothesis all_aps1 : all (eq_op STEnd) (map aproc_env aps1).

Lemma aprocs_env_final : map_tuple aproc_env aps' = map_tuple aproc_env aps1.
Proof.
apply: eq_from_tnth => i; rewrite !tnthE.
case: fuel_senv_decreases => /= ap [Her] [_] ->.
rewrite -!tnth_nth !tnthE.
case: ifP => // /andP[_].
move/all_tnthP/(_ i)/eqP: all_aps1.
by rewrite tnth_map => <-.
Qed.

Lemma aprocs_skip_final h :
  all (eq_op STEnd) (stypes_interp (h+1) (map aproc_env aps')).
Proof.
rewrite /is_true -[RHS]all_aps1; congr all.
rewrite addn1 /= ifF; first by rewrite -[RHS](f_equal val aprocs_env_final).
rewrite 2!size_map size_enum_ord -val_enum_ord -val_ord_tuple.
apply: negbTE.
rewrite -all_predC.
apply/all_tnthP => i /=.
rewrite !tnthE /stype_step.
rewrite -[map _ (enum _)]/(val aps') -tnth_nth aprocs_env_final.
by move: all_aps1 => /all_tnthP/(_ i) /eqP <-.
Qed.
...
Lemma aprocs_step_fuel :
  stypes_interp_fuel (map aproc_env aps') <=
  stypes_interp_fuel (map aproc_env aps1).
Proof.
rewrite /stypes_interp_fuel leq_add // !sumnE !big_tuple.
by apply: leq_sum => i _; rewrite !tnthE senv_depth_fuel.
Qed.

Hypothesis has_aps1 :
    has snd [tuple stype_step (map aproc_env aps1) i | i < m].
...
Proof.
move=> Hstep.
move/all_tnthP/(_ i): (Hnf).
rewrite !tnthE.
case: fuel_senv_decreases => pa [ha] [sa] /= ->.
case: ifPn; rewrite -!tnth_nth !tnthE.
  case/andP => _.
  by case: (tnth _ _) => pa1 [ha1] [sa1] [].
move: Hstep.
rewrite ha /stype_step.
...
Lemma aprocs_step_fuel_progress :
  stypes_interp_fuel (map aproc_env aps') <
  stypes_interp_fuel (map aproc_env aps1).
Proof.
rewrite /stypes_interp_fuel !sumnE 4!big_map big_enum big_tuple /=.
case/has_tnthP: has_aps1 => /= i; rewrite tnthE => Hstep.
rewrite (bigD1 i) // [in X in _ < X + 1](bigD1 i) //=.
rewrite !addn1 -!addSn leq_add //.
  by rewrite ltnS tnth_map (@aproc_step_progress i).
by apply: leq_sum => j _; rewrite tnth_map senv_depth_fuel.
Qed.

Let pss1 := unzip1 [tuple stype_step (map aproc_env aps1) i| i < m].

Lemma aproc_step_preserve_progress (i : 'I_m) :
  ~~ stype_stuck pss1 i ->
  aproc_env (sval (fuel_senv_decreases [::] (ltn_aps1 i))) =
  (stype_step (map aproc_env aps1) i).1.
Proof.
rewrite /stype_stuck /pss1 /unzip1 -(tnth_nth STEnd) !tnthE.
move/all_tnthP/(_ i): (Hnocomm).
move/all_tnthP/(_ i): (Hnf).
rewrite /aps' !tnthE.
case: fuel_senv_decreases => pa [ha] [sa] /= ->.
rewrite ha /stype_step /step -!tnth_nth !tnthE.
case Hnthi: (tnth _ _) => /= [pa1 [ha1 [sa1 p]]] /=.
case: ha1 sa1 / p Hnthi => //= ha1 sa1 src dt.
- move=> d k Hnthi.
  case/boolP: (src < m) => Hsrc; last first.
    move=> _ _.
    rewrite nth_default /=; last by rewrite size_tuple leqNgt.
    by rewrite nth_default //= size_tuple card_ord leqNgt.
  move/all_tnthP/(_ (Ordinal Hsrc)): Hnf.
  rewrite !tnthE.
  case: fuel_senv_decreases => pb [->] _.
  move/all_tnthP/(_ (Ordinal Hsrc)): Hnocomm.
  rewrite /aproc_env /erase_aproc /step /aproc_proc.
  rewrite -!(tnth_nth _ _ (Ordinal Hsrc)) !tnthE.
  case Hnth: (tnth _ _) => [pa2 [ha2 [sa2 p2]]] /=.
  case: ha2 sa2 / p2 Hnth => //=.
  move=> ha3 sa3 src' dt' k' Hnth.
  case: ifP => //=; last by rewrite eq_sym => ->.
  move/eqP => ?; subst src'; rewrite eqxx /=.
  rewrite -tnth_nth tnth_map Hnthi /= eqxx /=.
  case: ifP => //=.
  rewrite -val_ord_tuple -(tnth_nth _ _ (Ordinal Hsrc)) !tnthE.
  rewrite -tnth_nth tnth_map Hnth /=.
  rewrite -tnth_nth tnth_map Hnthi /=.
...
(* Preservation:
   ...
   * and then apply a parallel reduction step for communication
   * and the resulting processes are not Fail
   then the resulting process still have compatible session types.
   The non-failure condition is needed because we have Fail in the language.
   If we only use it for type mismatch, then we shall be able to prove
   that no process reduces to Fail. *)
Lemma aprocs_step_preserve (aps : seq (aproc dtype data)) :
  aprocs_compat aps ->
  let aps1 := map aproc_skip_nocomm aps in
  let ps' := [tuple step (erase_aprocs aps1) [::] i | i < size aps1] in
...
move=> Hnf.
exists aps'.
have Hps' : erase_aprocs aps' = ps'.
  rewrite /aps' /ps' /= -[LHS]map_comp -map_comp.
  apply: eq_codom => /= i /=.
  case: fuel_senv_decreases => aps2 [] /= -> _.
  by rewrite nth_default.
have Haps' : aps' =
  [tuple sval (fuel_senv_decreases [::] (ltn_aps1_subproof (in_tuple aps1) i))
  | i < size aps1].
  by apply: eq_mktuple => i; congr sval; congr fuel_senv_decreases.
split => //.
...
set h' := stypes_interp_fuel _.
have hh' : (h' <= h)%N.
  by rewrite /h /h' Haps' -(in_tupleE aps1) aprocs_step_fuel.
case Hh: h => [|h1] //=.
  have -> // : h' = 0.
  by apply/eqP; rewrite -leqn0 -Hh.
case: ifPn => [Hhas | _] Hall1; last first.
   by rewrite [map _ (enum _)](f_equal val Haps') aprocs_skip_final.
rewrite stypes_interp_fuel_ok //.
rewrite -(stypes_interp_fuel_ok (h:=h1)); last first.
  rewrite -ltnS -Hh /h.
  rewrite size_map map_iota_tuple in Hhas.
  rewrite [map _ (enum _)](f_equal val Haps').
  by rewrite aprocs_step_fuel_progress // -Haps' Hps'.
...
apply: eq_map => i /=.
rewrite -[X in map _ X](in_tupleE aps1) -aproc_step_preserve_progress => //.
    by congr (aproc_env (sval (fuel_senv_decreases _ _))).
  by rewrite -Haps' Hps'.
...
rewrite -all_predC -map_iota_tuple => /allP/(_ i).
rewrite mem_iota0 /= /pss1 /unzip1 3!size_map size_iota; exact.
Qed.
