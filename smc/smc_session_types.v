...
Proof.
elim: h ps => [|h IH] ps /hasP[i]; rewrite mem_iota leq0n add0n /= => Hi Hs.
  by move=> Hsz; rewrite -Hsz ltn0 in Hi.
case: ifP => Hhas Hsz.
  rewrite IH // !size_map size_iota //.
  apply/hasP; exists i.
    by rewrite mem_iota add0n Hi.
  move: Hs.
  rewrite /stype_stuck /unzip1 -map_comp (nth_map i) ?size_iota //.
  rewrite nth_iota // add0n /= /stype_step.
  ...
  - case/boolP: (n < size ps) => Hn; last first.
    ...
    + move/eqP => in'; subst n'.
      by rewrite (nth_map i) ?size_iota// nth_iota//= add0n Hnth Hnthi /= eqxx.
    + move=> /andP[] /eqP in' dtdt'; subst n'.
      rewrite eqxx (negbTE dtdt') /=.
      rewrite (nth_map i) ?size_iota// nth_iota//= add0n Hnth Hnthi /= eqxx.
      by rewrite eq_sym (negbTE dtdt') /= eqxx.
    + move=> _.
      by rewrite (nth_map i) ?size_iota// nth_iota//= add0n Hnth.
  - case/boolP: (n < size ps) => Hn; last first.
      rewrite nth_default /=; last by rewrite leqNgt.
      by rewrite nth_default // !size_map size_iota leqNgt.
    case Hnth : nth => //= [n' dt' p' | n' dt' p' |].
...
Proof.
elim: h ps => [|h IH] /= ps.
  by rewrite /stypes_interp_fuel addn1.
rewrite /stypes_interp_fuel addn1 ltnS => Hfuel.
case: ifP => // /stypes_interp_fuel_step Hh.
by rewrite IH ?(size_map,size_iota) // (leq_trans Hh).
Qed.
...
(* A sampling node draws from the party's own seed stream, so it is not a
   communication. *)
...
(* The reconstruction of fuel_decreases, now tracking the session environment
   exactly.  A communication step pops one session type and every other step
   leaves it alone. *)
...
Fixpoint skip_nocomm (p : proc data) :=
  ...
  | Send _ _ _ => p
  | Recv _ _   => p
  (* Sample blocks on the party's own seed stream, so unlike Init it has no
     successor here and is left in place, as Send and Recv are *)
  | Sample _   => p
  | Ret _      => Finish
  | Finish     => p
  | Fail       => p
...
Lemma rsteps_skip_nocomm n (sds : n.-tuple (seq data)) (i : 'I_n)
      (ps : n.-tuple (proc data)) :
  exists tr,
    rsteps sds ps (inject [tuple i] ps [tuple skip_nocomm (tnth ps i)])
      (inject [tuple i] [tuple nil | _ < n] [tuple tr]).
Proof.
move Hp: (tnth ps i) => p.
elim: p ps Hp => [d q IH | j d q _ | j q _ | f _ | d ||] ps Hp /=;
try (exists [::];
...
Fixpoint aproc_skip_nocomm_rec party h env (p : sproc data party h env) :=
  ...
  (* SSample, like SSend and SRecv, keeps its node: its successor depends on
     a drawn value that this function does not have *)
  | SSample _ _ _     => mk_aproc p
  end.
...
Lemma aprocs_step_preserve (aps : seq (aproc dtype data)) :
  ...
  all_nonfail (map (fst \o fst \o fst) ps') ->
  exists aps' : seq (aproc dtype data),
    erase_aprocs aps' = map (fst \o fst \o fst) ps' /\ aprocs_compat aps'.
Proof.
move=> Hcompat /=.
set aps1 := map aproc_skip_nocomm _.
set ps' := map _ _.
set aps' := [tuple sval
              (fuel_senv_decreases (ps:=aps1) [::] [::] [::] (ltn_ord i))
            | i < size aps1].
move=> Hnf.
exists aps'.
...
rewrite -(stypes_interp_fuel_ok (h:=h1)); last first.
  ...
  rewrite mem_iota leq0n add0n /= => Hi Hstep.
  rewrite !big_enum.
  ...
  rewrite !addn1 -!addSn leq_add //.
    ...
    rewrite mem_iota leq0n add0n Hi /= => /(_ isT).
    rewrite /aproc_env /erase_aproc /step.
    rewrite (nth_map (aproc_default _ _)) //.
    case: nth => /= pa1 [ha1] [sa1] p /=.
    case: ha1 sa1 / p => //= ha1 sa1 src dt d k.
    - case/boolP: (src \in iota 0 (size aps1));
        rewrite mem_iota leq0n add0n /= => Hsrc; last first.
...
rewrite mem_iota leq0n add0n /= /pss1 /unzip1 2!size_map size_iota.
...
move: (Hnf); rewrite /all_nonfail 2!all_map => /allP/(_ i).
rewrite mem_iota leq0n add0n ltn_ord /= => /(_ isT).
...
- move=> n' env' dst dt d next.
  ...
  + case: (frm == ctx_idx ctx).
    ...
    * exact (mk_aproc (party:=party) (SSend dst dt d next), false).
  + (* Sample: the receiver is drawing a coin, so the send stays blocked *)
    exact (mk_aproc (party:=party) (SSend dst dt d next), false).
  + exact (mk_aproc (party:=party) (SSend dst dt d next), false).
  + exact (mk_aproc (party:=party) (SSend dst dt d next), false).
  + exact (mk_aproc (party:=party) (SSend dst dt d next), false).
- move=> n' env' src dt cont.
  ...
  + case: (dst' == ctx_idx ctx).
    ...
    * exact (mk_aproc (party:=party) (SRecv src dt cont), false).
  + exact (mk_aproc (party:=party) (SRecv src dt cont), false).
  + (* Sample: the sender is drawing a coin, so the receive stays blocked *)
    exact (mk_aproc (party:=party) (SRecv src dt cont), false).
  + exact (mk_aproc (party:=party) (SRecv src dt cont), false).
  + exact (mk_aproc (party:=party) (SRecv src dt cont), false).
  + exact (mk_aproc (party:=party) (SRecv src dt cont), false).
- move=> n' env' cont.
  ...
  + exact (mk_aproc (party:=party) (SSample cont), false).
  + exact (mk_aproc (party:=party) (cont r), true).
