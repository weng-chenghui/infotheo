(** SSProve extension: bounded simulation security.
    ...
    [advantage_sim_le E admissible Real Ideal Sim bound] — every valid
    adversary [A] in the admissible class [admissible] distinguishes [Real]
    from [Sim ∘ Ideal] with advantage at most [bound A].
...
(* advantage_sim_le E admissible Real Ideal Sim bound — bounded simulation
   ...
   valid, admissible adversary A distinguishes the real package from the
   simulator composed with the ideal package with advantage at most
   bound A. *)
...
(* advantage_sim_le_from_endpoint — a real-versus-endpoint advantage bound
   [bound A] and a perfect equivalence between the endpoint and the
   simulated ideal package give bounded simulation security with the same
   bound.
...
(* advantage_sim_le_reduction — bounded simulation security transports across
   ...
   simulated ideal packages is at most [bound (A ∘ T)].  Admissibility must
   be closed under [A ∘ T]. *)
