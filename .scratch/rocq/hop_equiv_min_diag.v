...
Definition gc_min : game_code :=
  GC_enc_hop 1 (HE_const 0) (GC_ret [:: HE_var 0]).

(* sanity facts *)
Lemma gc_min_count_hops : count_hops gc_min = 1.
Proof. by []. Qed.

Lemma gc_min_hop_sites : hop_sites gc_min = [:: 0].
Proof. by []. Qed.

(* ===================================================================== *)
(* CANONICAL-ORDER GATE re-check after the INLINE-sample fix.             *)
(* gc_min now has NO leading GC_sample and GC_enc_hop carries no rnd      *)
(* field; the hop randomness is drawn INLINE at GC_enc_hop on both sides. *)
(* Expected: a single ssprove_sync_eq couples ir_hop with the oracle's    *)
(* sample, the two chcipher/chmsg round-trip cancels collapse, and        *)
(* reflexivity closes id_game_run with NO ssprove_swap.                   *)
(* ===================================================================== *)
Lemma hop_equiv_min :
...
simplify_eq_rel m.
- ssprove_sync_eq=> ir_hop.
  rewrite chcipher_of_cipherK chmsg_of_msgK.
  rewrite /sample_to_renc /indcpa_ror.sample_to_renc.
  apply: rpost_weaken_rule; first by apply: rreflexivity_rule.
  by move=> [? ?] [? ?] [-> ->].
- ssprove_sync_eq=> stored.
  by case: stored => [v|]; apply: r_ret.
Qed.

(* ===== GENERIC HELPER PROTOTYPE ===== *)
...
Lemma denote_run_shim_untargeted_zero (gc : game_code) :
...
- case: p => [|i'] /=.
  + rewrite add0n eqxx [code_link _ _]/=. ssprove_code_simpl. rewrite /bind /=.
    ...
    apply: denote_run_shim_untargeted_zero. by rewrite ltnSn.
...
Lemma hop_equiv_zero_proto (gc : game_code) (i : nat) :
...
Lemma hop_equiv_real_proto (gc : game_code) (i : nat) :
