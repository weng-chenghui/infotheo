...
A threshold secret-sharing protocol lets a dealer split a secret into shares so
that any sufficiently large coalition can reconstruct it and any smaller
coalition learns nothing. We study such protocols in a multiparty computation
setting where the dealer additionally applies a \emph{false shuffle} to the
dealt shares, that is, a permutation drawn from a fixed finite group. The group
that supplies this shuffle is the single tunable parameter of the whole
...
observed shuffle is to the uniform distribution on the deck. The second varying
...
\emph{compute} step the dealer applies the false shuffle: it hands party $j$ the
share at position $g(j)$, where $g$ is the plugged group element. In the
...
scheme's reconstruction. The three roles are realised by the programs
\id{exchange\_dealer}, \id{exchange\_player}, and \id{exchange\_verifier}, and
the dealt payload carries the plugged group's shuffle, so the group enters the
protocol exactly at the compute step.
...
recoverability through a genus-zero covering built from Reed-Solomon codes and
the algebraic rigidity record, and it plugs into the same shared program as the
mixing instances, where it exhibits the permanent floor that the mixing plugs
escape.
