...
The uniform-shuffle and word-shuffle results concern two distinct distributions.
...
generator-word distribution with that uniform group distribution. The formalization transports the resulting
distributional proximity through the protocol interpreter. Approximate
coalition-view and executed-trace privacy hold under the word distribution with
...
Section~\ref{sec:model} fixes the two distributions and the security scope.
...
The primary uniform-shuffle distribution samples a uniform Boolean secret and uses one fixed
representative $D_s$ for each secret $s$. It then samples an independent
element $g\leftarrow U_G$. For the $\PG$ instance, this is the formal distribution
...
\coqin{orbit\_encode}.}
The all-decks distribution instead samples a uniform valid arrangement in the selected
secret class before it samples $g\leftarrow U_G$.
\footnote{The all-decks distribution is \coqin{pgl27P\_alldecks}.}
Both uniform-shuffle execution distributions use a uniform Boolean prior.
...
The word-shuffle model replaces $U_G$ by $\mu^{*L}$. I measure two distributions $P$
...
\coqin{SecurityWitness} gives an endpoint bound for a shuffle distribution.
...
result is the secret-prior product distribution in Equation~\ref{eq:product-transfer}.}

The proved chain reaches group-distribution proximity, every single-card marginal,
...
that the product-distribution transfer does not reach.
...
uniform group distribution, the instance has executed correctness, recovery
...
passive adversaries, and the dealer distributions stated in Section~\ref{sec:model}.
