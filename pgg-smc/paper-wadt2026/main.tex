...
\begin{theorem}[Exact privacy for the fixed dealer]
...
\begin{table}[t]
  ...
  \begin{tabular}{llllll}
    \toprule
    Instance & Recovery & Exact privacy & Trace privacy & Finite-step mixing & Trust base \\
    \midrule
    den Boer & five-card AND & one card & one player & exact after one uniform cut
      & kernel, \coqin{boolp} \\
    Kim & five-card AND & one card & one player & $L=7$, $L_1<2^{-40}$
      & kernel, \coqin{boolp} \\
    $S_5$ & additive 5-of-5 & fewer than five & one player
      & $L=286$ spectral route & imported Rayleigh bound \\
    $S_5\times S_5$ & two additive 5-of-5 shares
      & fewer than five per pile & one player
      & within-pile decay, global floor $1$ & inherited Rayleigh bound \\
    $\PG$ & $(t,r,n)=(3,7,8)$ & coalitions of at most three
      & coalitions of at most three & $L=200$, $L_1\leq2^{-40}$
      & kernel, \coqin{boolp} \\
    \bottomrule
  \end{tabular}}
...
The artifact uses a paper-specific theorem index. The index covers the PGL
files, the four sibling instances in Table~\ref{tab:instances}, and the
framework files used by displayed results. It excludes unrelated instance
demonstrations. Tables~\ref{tab:pgl-source}, \ref{tab:recovery-source}, and
\ref{tab:privacy-source} give the PGL part of that index in the paper. The
artifact also records a claim matrix. Each row links a paper claim to its
formal theorem, dealer law, shuffle law, adversary scope, and assumptions.

Assumptions are reported per credited theorem with \coqin{Print Assumptions}.
The PGL group, orbit, order, correctness, and recovery results are checked
without additional axioms. The current probability, privacy, trace, and
mixing results use propositional extensionality, dependent functional
...
curve-realization assumptions. The artifact reports these dependencies on
the exact theorem rows instead of assigning them to every result in the same
file.
...
reported build. The artifact lists it as excluded material. For the main
finite-step theorem, a
small scratch file imports the PGL mixing module and invokes
\coqin{Print Assumptions pgl27\_word\_mixing}. The scratch file uses the
repository's module flags and does not modify the formalization.

The work used LLM-generated proof scripts and documentation drafts under
human specification, review, and responsibility. The human author selected
the research questions, wrote the mathematical specification, selected the
claims, made acceptance decisions, verified citations, and approved the
final text. The \Rocq{} kernel checks accepted proof terms. A separate audit
checks that formal statements and paper claims remain aligned.
...
The source, theorem index, claim matrix, assumption report, and build
instructions accompany the submission artifact. The artifact fixes the
formalization at the commit reported in Section~\ref{sec:method}.
