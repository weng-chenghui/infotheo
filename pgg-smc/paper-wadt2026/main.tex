...
$\PG$ on eight projective points. Under the uniform shuffle law, the
...
uniform group law. This word-shuffle theorem transfers to single-card
...
under the word law. Four sibling instances
...
The uniform-shuffle and word-shuffle results concern two distinct laws.
Perfect privacy uses a uniform group element with a uniform Boolean secret
and a fixed dealt representative. Word-shuffle mixing compares the
generator-word law with that uniform group law. The formalization transports the resulting
distributional proximity through the protocol interpreter. Approximate
coalition-view and executed-trace privacy hold under the word law with
error at most $2^{-39}$. The protocol is dealer-based and has no
...
construction and its uniform-shuffle results. Section~\ref{sec:mixing} proves the
word-shuffle bound. Section~\ref{sec:instances} compares the sibling instances.
Section~\ref{sec:related} places the results among related work.
Section~\ref{sec:conclusion} states the remaining extensions and other
future directions.
...
The primary uniform-shuffle law samples a uniform Boolean secret and uses one fixed
...
used by the main uniform-shuffle execution. The representative is the formal orbit
encoder.
\footnote{The uniform-shuffle source objects are \coqin{pgl27P} and
\coqin{orbit\_encode}.}
...
Both uniform-shuffle execution laws use a uniform Boolean prior.
...
The word-shuffle model replaces $U_G$ by $\mu^{*L}$. I measure two laws $P$
...
element.\footnote{The dealer program is \coqin{exchange\_dealer} and the
$\PG$ witness is the singleton cut list in \coqin{pgl27\_dealer\_run}.}
The uniform-shuffle model makes the same assumption, since its single
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[box,right=of uniform] (exact) {perfect coalition view\\and trace theorems};
    ...
    \node[box,right=of transfer] (privacy)
      {coalition view and\\trace privacy at $2^{-39}$};
    \draw[arr] (word) -- (lone);
    \draw[arr] (lone) -- (transfer);
    \draw[arr] (transfer) -- (privacy);
  \end{tikzpicture}}
  \caption{The uniform-shuffle and word-shuffle proof paths. The lower path transports
...
The first theorem supplies the perfect-privacy view argument used by the
PGL instance.
...
probability one under the uniform law and under any word law, and only
...
\[
  (t,r,n)=(3,7,8).
\]
Coalitions of at most three positions have perfect privacy. Some coalition of
...
\begin{theorem}[Perfect privacy for the fixed dealer\footnotemark]
\footnotetext{Formalized in \path{pgg-smc/instances/pgl27/pgl27_secrecy.v}
as
...
\begin{theorem}[All-decks perfect privacy\footnotemark]
\footnotetext{Formalized in \path{pgg-smc/instances/pgl27/pgl27_secrecy.v}
as
...
\begin{table}[t]
  ...
  \begin{tabular}{p{.43\linewidth}p{.49\linewidth}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Source index for perfect view and trace privacy.}
...
\begin{theorem}[Word-shuffle mixing bound\footnotemark]\label{thm:pgl-mixing}
...
and the product with any secret prior. Two transport theorems carry the
certificate to the security observables.

\begin{theorem}[Word-shuffle coalition privacy\footnotemark]\label{thm:word-privacy}
\footnotetext{Formalized in
\path{pgg-smc/instances/pgl27/pgl27_word_privacy.v} as
\coqin{pgl27\_word\_view\_indist} and \coqin{pgl27\_word\_trace\_indist}.}
For every coalition $C$ with $\lvert C\rvert\leq3$ and all secrets
$s,s'\in\{0,1\}$, writing $V_C(s,g)$ for the coalition view at dealt secret
$s$ and shuffle $g$,
\begin{equation}
  \left\lVert
...
\end{equation}
and the executed coalition trace satisfies the same bound.
\end{theorem}

The proof is a triangle inequality through the uniform group law. The
coalition-view law under the uniform shuffle does not depend on the secret,
...
companion theorem bounds the joint view-and-secret law under the word
...
The word-law privacy results cover the fixed representative dealer only,
since the all-decks and shuffle-free dealers use different sample spaces
that the product-law transfer does not reach.
...
coverage. Perfect privacy in the table refers to the dealer law defined by each
...
instantiated it with the action of $\PG$ on eight positions. Under the
uniform group law, the instance has executed correctness, recovery
parameters $(3,7,8)$, and perfect view and trace privacy for coalitions of at
...
any Boolean secret prior. Transport theorems turn these distributional
bounds into approximate coalition-view and executed-trace privacy at
$2^{-39}$.

Two extensions of the word-law security results remain open. The entropy
...
dealers need a word-law mixing statement on their own sample spaces. Another
