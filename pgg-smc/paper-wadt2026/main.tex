...
This section supplies the $\PG$ instance in Table~\ref{tab:bridge}. Let
\[
  X=\mathbb{P}^1(\mathbb{F}_7)=\{0,1,\ldots,6,\infty\}.
\]
The group $G=\PG$ acts on the eight points of $X$ by fractional linear
transformations. Its order is\footnote{Formalized in
...
\begin{equation}
  ...
  \label{eq:pgl-order}
\end{equation}
The formal representation writes $\infty$ as position seven. Card positions
belong to $X$, while card values belong to
$C=\{0,1,\ldots,7\}$.
...
An orbit contains the heart-position patterns that are reachable from one
another by allowed shuffles. Every orbit in this subsection is an orbit in
$\Omega_4$. A labelled deck supplies one heart-position representative of
such an orbit.
...
Let $\rho:G\to S_X$ be the permutation action, and define
\[
...
Because $\rho(g)^{-1}=\rho(g^{-1})$, the inverse permutation is also induced
by an element of $G$.
The shuffle therefore changes the representative inside $\Omega_4$, but not
its orbit label.
...
Orbit invariance preserves the secret under every allowed shuffle. It does
not by itself hide the secret. Under an independent uniform shuffle
...
from two inverses. The equality $|G|=336=8\cdot7\cdot6$ also matches the
number of ordered triples of distinct points. Lemma~\ref{thm:three-transitive}
shows that each ordered destination has exactly one group element.
