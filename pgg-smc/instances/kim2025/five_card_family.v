...
(* distinct probability on each cut), while den Boer's FiveCard_M uses the      *)
(* single generator sigma; both generate <[fc_sigma]> = C_5. This file plugs    *)
...
(** fcI_perm_compatible_kim — reconstruction is invariant under the monodromy
    ...
    but they still generate the same cyclic group <[fc_sigma]> = C_5 as den
    Boer's single-generator instance. The ONLY change from fcI_perm_compatible
    is the cyclicity fact Gcyc: the generating set [set sigma^i | i] generates
...
(** FiveCardKim_PI — the concrete five-sheet starting interface for Kim's
    ...
    exactly as for the den Boer FiveCard_PI. Used-by: five_card_profile. *)
