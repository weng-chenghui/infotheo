...
\begin{theorem}[Profile and end-to-end correctness]
  ...
  \rocq{pgg_smc.den_boer_profile.den_boer_profile,
       ...
       pgg_smc.den_boer_profile.den_boer_run_k}
  \rocqok
...
\begin{definition}[The wired $S_5$ representation]
  ...
  \rocqok
  The wired instance places the secret in coordinate $0$ of $\GF(5)^6$ (fixed)
  and the five shares in coordinates $1..5$ (permuted by $S_5$). Recovery would
  need an $S_5$-invariant submodule carrying the secret direction $e_0$ with a
  dimension in the gap window.
\end{definition}
...
\begin{definition}[The gap-dimension window]
  ...
  \rocqok
  \uses{def:covering_scheme}
  Under the AG-Massey relations at length $6$, any strict threshold gap forces
  the required code dimension into the window $\{3,4\}$.
\end{definition}

\begin{theorem}[The kernel fact]
  \label{thm:kernel_fact}
  \rocq{pgg_reconstruct.s5_nogo.perm_module_no_dim23}
  \rocqok
  \uses{def:wired_s5}
  The natural permutation module $\GF(5)^5$ of $S_5$ has no submodule of
  dimension $2$ or $3$; the submodule dimensions are exactly $\{0,1,4,5\}$. A
  non-constant vector and a transposition force a difference vector
  $e_i-e_j$ into the submodule, and $2$-transitivity spreads it to all of them,
  giving rank $\ge 4$; otherwise the module is constant, rank $\le 1$.
\end{theorem}

\begin{theorem}[A characteristic-5 phenomenon]
  \label{thm:maschke_contrast}
  \rocq{pgg_reconstruct.invariant_profiler.maschke_ss}
  \rocqok
  \uses{thm:kernel_fact}
  The kernel fact is modular: $\mathrm{char}\,\GF = 5$ divides $5$, so the
  all-ones vector sits inside the sum-zero subspace and the lattice is uniserial.
  In coprime characteristic Maschke splits the module and dimensions $2,3$
  reappear.
\end{theorem}
...
\begin{theorem}[The $S_5$ no-go]
  ...
  \rocqok
  \uses{thm:kernel_fact,def:gap_window}
  Projecting away the secret coordinate drops the rank by exactly one, so a
  secret submodule of dimension $3$ or $4$ becomes a kernel submodule of
  dimension $2$ or $3$, which is forbidden. Hence no secret-encoding invariant
  AG code exists for any gap instance: $S_5$ is an AG-code no-go. Axiom-free.
\end{theorem}
