...
(*               statdistC == statdist is symmetric                           *)
(*           ler_statdistD == statdist satisfies the triangle inequality      *)
...
(*              class_advC == class_adv is symmetric                          *)
...
(*          class_adv_setT == class_adv at the full tester class is statdist  *)
(* ```                                                                        *)
(*                                                                            *)
(******************************************************************************)
...
Lemma statdistC p q : statdist p q = statdist q p.
...
(* Off S the mass that q carries above p equals, on S, the mass that p
   carries above q. *)
Local Lemma sum_diffC p q (S : {set B}) :
...
Local Lemma acceptB (D : tester) p q :
...
Proof.
rewrite /adv acceptB ler_norml; apply/andP; split; last exact: sum_diff_le.
rewrite lerNl -sumrN; under eq_bigr do rewrite opprB.
by rewrite statdistC; exact: sum_diff_le.
Qed.
...
Lemma class_advC T p q : class_adv T p q = class_adv T q p.
...
(* The class advantage at the full tester class is the statistical
   distance. *)
Lemma class_adv_setT p q : class_adv [set: tester] p q = statdist p q.
Proof.
