...
> **Post-implementation note (2026-07-13):** the four open items of the audit round are closed by the follow-up round (spec `2026-07-12-pgl27-open-questions-design.md`). `pgl27_3transitive` is now PROVEN in-kernel (commit 6fce3f4, BFS word table + vm_compute checker; the pgl27 chain is boolp-only). The "(no sub-8 recovery)" clause above is superseded: seven reveals determine the class and six never do (`pgl27_recovery.v`, commit 2bf1174); reveal-all is a property of the implemented decoder, not an information-theoretic necessity. All-decks dealer privacy landed (commit 3902366): `pgl27_view_indep_alldecks` and the shuffle-free `pgl27_view_indep_deck` remove the fixed-representative scope limit. The secrecy statements are pre-reveal execution statements; headers now say so.
