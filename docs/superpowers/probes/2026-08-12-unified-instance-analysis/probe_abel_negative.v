...
(******************************************************************************)
(* probe_abel_negative: the exact full-L1 distance one at positive word length*)
(*                                                                            *)
(* Phase 0 probe for the unified-instance-analysis request, sections 6.5,     *)
(* 6.7 and 9.4 to 9.7. The actual shuffle distribution is the pushforward of  *)
(* the uniform length-L.+1 generator-word distribution through the abelian    *)
(* word evaluator; the ideal one is uniform on the four-element generated     *)
(* group. Since both generators are involutions and commute, a word evaluates *)
(* to a group element determined by the parity of its count of the first      *)
(* letter, so a positive-length word reaches exactly one parity class of two  *)
(* elements, each with mass one half. The full-L1 distance to the group       *)
(* uniform is therefore exactly 1, and the complete four-endpoint reader is   *)
(* globally injective, so the executed observation has the same distance.     *)
(*                                                                            *)
(* Probe claims:                                                              *)
(*   abel_word_evalE          == word value from the two letter parities      *)
(*   abel_word_eval_odd/even  == the parity class at each length parity       *)
(*   abel_flip_freq           == first-letter flip inverts the class          *)
(*   abel_parity_mass_half    == each class element carries mass one half     *)
...
(*   abel_ideal_adapter       == the ideal SampleAdapter                      *)
(*   abel_actual_adapter      == the finite-word SampleAdapter                *)
(******************************************************************************)
...
(** abel_word_evalE — a word's value is the product of the two generators
    ...
    (if odd (freq_vec w 1) then s2 else 1). *)
...
(** abel_flip_freq — the first-letter flip inverts the parity of the first
    ...
    the fact that makes the two parity classes equinumerous. *)
...
(******************************************************************************)
(*     The two shuffle distributions and their exact distance                 *)
(******************************************************************************)

Section abel_negative.

Variable R : realType.

(** var_dist_fdistmap_inj — an injective reader preserves the full-L1
    distance.
    @main architecture: var_dist (fdistmap f P) (fdistmap f Q) = var_dist P Q
    for injective f. A local copy of the lemma of instances/s5x5/s5x5_mixing.v;
    Phase 3 relocates the production copy to a shared file rather than
    importing the S_5 x S_5 mixing development for it. *)
...
rewrite addr0 [in RHS](bigD1 a) /=; last by apply/eqP.
rewrite big1 ?addr0 // => a' /andP [Ha' Hne].
move/eqP in Ha'; rewrite -Heq in Ha'; move/Hinj in Ha'.
by rewrite Ha' eqxx in Hne.
Qed.

(** abel_group_uniform — the ideal shuffle distribution.
...
(** abel_word_dist — the actual shuffle distribution at positive word length.
    @intent: the pushforward of the uniform distribution on generator words of
    length L.+1 through the abelian word evaluator. *)
Definition abel_word_dist (L : nat) : R.-fdist {perm 'I_4} :=
  @rho_from_words R 2 1 L.+1 abel_sigmas.

(** abel_word_distE — the actual distribution as a fibre sum.
...
(** abel_parity_mass — the mass of one parity class of words.
    @intent: the total uniform-word mass of the words whose first-letter count
    is odd. *)
...
(** abel_parity_mass_half — each parity class carries mass one half.
    @main bound: abel_parity_mass L = 1/2, from the flip bijection and the
    partition of the word space. *)
...
(** abel_word_dist_class — a two-element parity class carries the whole mass,
    ...
    @main bound: for a word evaluator that returns a on the odd class and b on
    the even class with a distinct from b, the actual distribution is 1/2 at a,
    1/2 at b and 0 elsewhere. *)
...
(** abel_group_uniform_in — the ideal distribution is a quarter on the group.
    @composes: abel_var_distE *)
...
(** abel_group_uniform_out — the ideal distribution vanishes off the group.
    @composes: abel_var_distE *)
...
(** abel_var_distE — the full-L1 distance to the ideal distribution is the sum
    of the four group-element gaps, for a distribution supported in the group.
...
(** abel_word_group_dist — the actual and ideal shuffle distributions are at
    full-L1 distance exactly one, at every positive word length.
    @main bound: var_dist (abel_word_dist L) abel_group_uniform = 1, for every
    L : nat, that is at every generator-word length L.+1. *)
...
(** abel_executed_distance — the executed complete four-endpoint observations
    of the two shuffle distributions are at full-L1 distance exactly one.
    @main bound: var_dist (fdistmap abel_reader (abel_word_dist L))
    (fdistmap abel_reader abel_group_uniform) = 1, for every L : nat. *)
...
Proof. by rewrite (var_dist_fdistmap_inj _ _ abel_reader_inj) abel_word_group_dist. Qed.

(******************************************************************************)
(*     The length-zero mutation witness                                       *)
(******************************************************************************)
...
(** abel_word_group_dist0 — at word length zero the distance is 1 + 1/2, not 1.
    ...
    1 + 1/2. The positive-length hypothesis of abel_word_group_dist is therefore
    load-bearing: the length-zero pushforward is a Dirac distribution, not a
    uniform distribution on a two-element parity class. *)
...
Proof.
apply/fdist_ext => g; rewrite fdistmapE (big_pred1 g) //.
Qed.
...
(** abel_actual_cut_dist — the finite-word adapter's cut distribution is the
    actual shuffle distribution.
    @main architecture: sa_cut_dist (abel_actual_adapter L) = abel_word_dist L,
    so the negative result bounds the distribution this adapter samples. *)
Lemma abel_actual_cut_dist (L : nat) :
  sa_cut_dist (abel_actual_adapter L) = abel_word_dist L.
Proof. by []. Qed.

(** abel_sample_reader — the executed complete four-endpoint observation as a
    function of a sample point.
    @intent: abel_reader read at the cut the sample point selects; by
    abel_shuffle_executed_readerE this is the endpoint list the run's verifier
    collects. *)
...
(** abel_sample_reader_dist — the executed observation's distribution is the
    ...
    fdistmap abel_reader (sa_cut_dist sa), the equality that carries the
    negative result from the cut layer to the executed observation layer. *)
...
(** abel_executed_observation_distance — the executed four-endpoint
    ...
    @main bound: var_dist (fdistmap (abel_sample_reader (abel_actual_adapter
    R L)) (word_uniform)) (fdistmap (abel_sample_reader (abel_ideal_adapter R))
    (abel_group_uniform)) = 1, for every L : nat. *)
...
(** abel_adapter_distance — the two adapters' cut distributions are at full-L1
    distance exactly one at every positive word length.
    @main bound: var_dist (sa_cut_dist (abel_actual_adapter L))
    (sa_cut_dist abel_ideal_adapter) = 1, the negative mixing statement read
    off the probabilistic layer rather than off the raw pushforwards. *)
Theorem abel_adapter_distance (L : nat) :
  var_dist (sa_cut_dist (abel_actual_adapter L))
           (sa_cut_dist abel_ideal_adapter) = 1.
Proof. by rewrite abel_actual_cut_dist abel_ideal_cut_dist abel_word_group_dist. Qed.

End abel_negative.

Print Assumptions abel_word_group_dist.
Print Assumptions abel_executed_distance.
Print Assumptions abel_word_group_dist0.
Print Assumptions abel_adapter_distance.
Print Assumptions abel_executed_observation_distance.
Print Assumptions abel_parity_mass_half.
Print Assumptions abel_flip_freq.
Print Assumptions abel_word_evalE.
