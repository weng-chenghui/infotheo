...
`pgg-smc/paper-wadt2026/main.tex` must identify its direct Rocq counterpart.
The theorem title carries a footnote mark. The footnote gives the
repository-relative `.v` path and the exact Rocq declaration names.

## Coverage

The change applies to all nine `theorem` environments in the paper:
...
body. It does not list proof dependencies or internal helper lemmas.
...
1. Count nine theorem environments and nine theorem-title source footnotes.
...
6. Confirm that the diff contains no `.v` file.
