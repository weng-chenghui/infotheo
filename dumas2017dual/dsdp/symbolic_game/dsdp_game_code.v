...
(* are at most one per-site IND-CPA reduction advantage apart.         *)
(* ------------------------------------------------------------------ *)

(* advantage_hop — any adversary's SSProve advantage between hybrid-ladder
   rungs i and i+1 is at most the IND-CPA real-or-zero advantage of the
   reduction [A ∘ denote_game_shim (zero_hop_prefix i gc) i].
   Advantage_triangle_chain inserts the two oracle-routed shim intermediates
   [denote_game_shim (zero_hop_prefix i gc) i ∘ oracle_real] and
   [… ∘ oracle_zero]; hop_equiv_real and hop_equiv_zero zero the two outer
   perfect-equivalence hops; Advantage_link folds the shim into the adversary so
   the cryptographic middle hop is exactly [indcpa_epsilon] at that
   reduction. *)
Lemma advantage_hop
...
(* advantage_sum_ladder_le — the advantage_sum over the [start+1 .. start+n]
   ...
   realigning by addSnnS.  Used by advantage_le.
...
Lemma advantage_sum_ladder_le
  ...
  advantage_sum (denote_game (zero_hop_prefix start gc))
    ...
    <= \sum_(l < n.+1)
         indcpa_epsilon AHE Renc card_renc renc_card rand_of_renc t_msg t_cipher
           msg_of_chmsg chcipher_of_cipher pkey_of_party
           (A ∘ denote_game_shim (zero_hop_prefix (start + l) gc)
                  (start + l)).
Proof.
elim=> [|n IHn] start.
...
- cbn [iota map advantage_sum]; rewrite big_ord_recl addn0.
  apply: lerD; first exact: advantage_hop.
...
(* is at most the sum of the per-site IND-CPA reduction advantages.    *)
(* ------------------------------------------------------------------ *)

(* advantage_le — the SSProve advantage of any adversary distinguishing the
   real game from the all-zero game is bounded by the sum, over the
   [size (hop_sites gc)] GC_enc_hop sites, of the IND-CPA real-or-zero
   advantage of the site reduction [A ∘ denote_game_shim
   (zero_hop_prefix l gc) l].  Advantage_triangle_chain inserts the
   hybrid_ladder rungs, the empty ladder collapses to advantage_self_zero, and
   the non-empty ladder telescopes through advantage_sum_ladder_le into
   [count_hops gc] single-rung bounds.  size (hop_sites gc) = count_hops gc
   since hop_sites enumerates [iota 0 (count_hops gc)]. *)
Lemma advantage_le
    (LA : Locations) (A : raw_package) (gc : game_code)
    (A_valid : ValidPackage LA game_iface A_export A)
    (A_disj_state : fseparate LA protocol_state)
...
(* hop_sites_game_code_dsdp — back-end validation: the fixture has exactly two
   ...
   through [size_iota] / [count_hops]); it is what lets
   [advantage_game_code_dsdp_le] expand its bound into two explicit
   summands. *)
...
(* advantage_game_code_dsdp_le — headline back-end validation: any
   ...
   game (derived from the fixture) from its all-zero endpoint is at most the
   sum of the two per-hop IND-CPA reduction advantages, at sites 0 and 1.
   Specialises the generic [advantage_le] to [game_code_dsdp], rewrites
   [size (hop_sites game_code_dsdp)] to [2] via [hop_sites_game_code_dsdp],
   and expands the two-element ordinal sum; the four premises are exactly
   [advantage_le]'s adversary well-formedness and state-disjointness
   hypotheses.
   Naming: [advantage] names the AdvantageE being bounded, [game_code_dsdp] the
   fixture it is instantiated at, and [_le] the upper bound; the file's SSProve
   advantage-lemma convention, not a MathComp algebraic property. *)
...
have H := advantage_le game_code_dsdp A_valid A_disj_state A_disj_ore
  A_disj_oze.
rewrite hop_sites_game_code_dsdp big_ord_recl big_ord1 in H.
exact: H.
Qed.
