...
(** card_t_msg_gt0 - strict positivity [0 < card_t_msg].  Follows
    from [card_msg_gt0] since [card_t_msg = card_msg] by definition.
    Kind: helper.
    Why: needed for downstream entropy work in U3 ([log_id] and
    [Hunp_ge_bound] require [card_t_msg > 0]); also keeps the
    [1 / card_t_msg] residual non-vacuous.
    Naming: [card_t_msg] is the abstract section parameter from
    ...
    pattern of [card_msg_gt0]/[card_renc_gt0] above.
    Used by: secrecy_random_guess, U3's Hunp_random_guess. *)
...
(** Pr_real_gt0 - strict positivity of [Pr at game_real with
    ...
    the specific witness predictor used by [Hunp_random_guess].
...
(** epsilon_cpa_ge0 - nonnegativity of the IND-CPA error parameter
    ...
    Kind: section hypothesis.
    Why: needed for [Hunp_random_guess] transitively through
    [Hunp_ge_bound] (its [log_id] step requires
    [1 + 2 * m * eps > 0]).
    Used by: Hunp_random_guess. *)
Hypothesis epsilon_cpa_ge0 : (0 <= indcpa_ror.epsilon_cpa)%R.

(** Hunp_random_guess - the closed-form Alice-secrecy bound in
    ...
    Naming: 3-token snake_case [Hunp_random_guess] mirrors
...
Corollary Hunp_random_guess :
  (dsdp_security_indcpa.bound card_t_msg
   <= dsdp_security_indcpa.Hunp (AHE:=AHE) (Renc:=Renc) (card_renc:=card_renc)
        ...
        random_guess_adv)%R.
Proof.
refine (@dsdp_security_indcpa.Hunp_ge_bound
          AHE Renc card_renc renc_card rand_of_renc
          t_msg t_cipher msg_of_chmsg chmsg_of_msg
          chcipher_of_cipher cipher_of_chcipher
          chcipher_of_cipherK chmsg_of_msgK
...
(** msg_witness - plaintext inhabitance witness at [plain ahe = 'F_p].
    Picked as [0 : 'F_p].
    Kind: inhabitance witness.
    Why: needed for [Hunp_random_guess], which transitively
...
(** secrecy_random_guess - the closed-form Alice-secrecy bound at
    ...
    Used by: Idealized.Hunp_random_guess in U3. *)
...
(** Pr_real_gt0 - strict positivity at the idealised carriers for
    ...
    Kind: section hypothesis.
    Why: feeds [Hunp_random_guess]'s new [Pr_real_gt0]
...
(** Hunp_random_guess - the closed-form Alice-secrecy bound in
    ...
    [Hunp_random_guess] at the idealised carriers and
...
(** secrecy_random_guess - the closed-form Alice-secrecy bound at
    ...
    the entropy-form corollaries in U3 that lift the probability
    inequality to a mutual-information statement.
    Used by: Benaloh.Hunp_random_guess in U3. *)
Definition secrecy_random_guess :
  distr.mu
    (pkg_advantage.Pr
       (dsdp_security_indcpa.guess_indicator_pkg
...
(** Hunp_random_guess - the closed-form Alice-secrecy bound in
    ...
    random-guess adversary.  Specialises
    [Hunp_random_guess] at the Benaloh carriers and the
...
(** secrecy_random_guess - the closed-form Alice-secrecy bound at
    ...
    inequality to a mutual-information statement.
    Used by: Paillier.Hunp_random_guess in U3. *)
Definition secrecy_random_guess :
  distr.mu
    (pkg_advantage.Pr
       (dsdp_security_indcpa.guess_indicator_pkg
...
(** Hunp_random_guess - the closed-form Alice-secrecy bound in
    ...
    random-guess adversary.  Specialises
    [Hunp_random_guess] at the Paillier carriers and the
    Section-local hypotheses.
    Kind: main.
