...
Lemma alice_view_to_cond (A : finType) (Xvar : {RV P -> A})
  (* Three independence hypotheses for peeling encryption layers.
     ...
     these are listed in the order they are consumed (outermost peel first):
     1. Hinde_bob:    peels E_bob_v2     (rightmost of AliceView)
     2. Hinde_charlie: peels E_charlie_v3 (next rightmost)
     3. Hinde_alice:  peels E_alice_d3    (innermost of the three encs) *)
  (Hinde_bob :
     P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3, E_alice_d3, E_charlie_v3], Xvar]
            _|_ E_bob_v2)
  (Hinde_charlie :
     P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3, E_alice_d3], Xvar]
            _|_ E_charlie_v3)
  (Hinde_alice :
     P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3], Xvar]
            _|_ E_alice_d3) :
...
have H_assoc: `H(Xvar | [% Dk_a, R2, R3, V1, U1, U2, U3, S] ) =
  ...
  rewrite (reindex (fun '(o, (v1, u1, u2, u3, s)) =>
    ...
    rewrite /jcPr !setX1 !Pr_set1 !dist_of_RVE !pfwd1E.
    congr (_ / _ * log (_ / _));
       by congr Pr; apply/setP => u; rewrite !inE /= !xpair_eqE !andbA.
...
Lemma joint_centropy_reduction
    ...
    (Hinde_bob_v2v3 :
       P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3, E_alice_d3, E_charlie_v3],
                [% V2, V3]] _|_ E_bob_v2)
    (Hinde_charlie_v2v3 :
       P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3, E_alice_d3], [% V2, V3]]
              _|_ E_charlie_v3)
    (Hinde_alice_v2v3 :
       P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3], [% V2, V3]] _|_ E_alice_d3)
    (* Encryption-peeling hypotheses for Xvar = V2 *)
    (Hinde_bob_v2 :
       P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3, E_alice_d3, E_charlie_v3], V2]
              _|_ E_bob_v2)
    (Hinde_charlie_v2 :
       P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3, E_alice_d3], V2]
              _|_ E_charlie_v3)
    (Hinde_alice_v2 :
       P |= [% [% Dk_a, S, V1, U1, U2, U3, R2, R3], V2] _|_ E_alice_d3) :
  `H([% V2, V3] | AliceView ) = `H(V2 | AliceView).
Proof.
rewrite (alice_view_to_cond Hinde_bob_v2v3 Hinde_charlie_v2v3 Hinde_alice_v2v3
           cinde_V2V3).
rewrite (alice_view_to_cond Hinde_bob_v2 Hinde_charlie_v2 Hinde_alice_v2
           cinde_V2).
