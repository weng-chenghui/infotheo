...
\item \textbf{Real-or-zero indistinguishability.} $\epscpa \in [0, 1]$ is the distinguishing-probability gap allowed for the public-key encryption scheme against class $\C$. This is the real-or-zero form of the standard hybrid/game-hopping use of IND-CPA security: one ciphertext is replaced at a time, and each replacement is justified as a transition based on indistinguishability in the sense of [3].
  \begin{enumerate}[label=8.\arabic*.]
  \item For any party $p \in \{a, b, c\}$ and any efficiently samplable joint distribution $(W,M)$ generated without the secret key $\Dk_p$, where $W$ may be correlated with $M$, the adversary cannot effectively distinguish between the following two views:
    \begin{enumerate}[label=8.1.\arabic*.]
    \item \textbf{Real view:} $(W, \Encf(\pk_p, M; r))$. The adversary sees auxiliary input, or side information, $W$ and a fresh encryption of $M$.
