...
seed)) gc_rest) emptym))` still pays off (the slow peel runs once, not twice), proved via
the v2v3_det peel + `Pr_fst_agree_locs` (drop the heap) + `denote_run_caps_fst` (run's
vt.1.1 marginal = the `drun` marginal); then `guess_inner_out` rewrites it on both sides,
`Hout` collapses the kernels, `view_marginal_indep` collapses the `drun` marginals. Use
`set`-abstraction inside the helper proof to dodge the 18s steps.
