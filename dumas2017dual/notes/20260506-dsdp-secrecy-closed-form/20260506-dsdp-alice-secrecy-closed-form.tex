...
\item \textbf{The adversary class $\C$.} For finite input and output spaces $\mathcal{X}$ and $\mathcal{O}$ and a size bound $s$, $\C_{\mathcal{X} \to \mathcal{O}, s}$ denotes the class of non-uniform randomized circuits of size at most $s$ that take inputs in $\mathcal{X}$ and output values in $\mathcal{O}$. We use this typed family both for predictors, whose randomized output is an element of $R$, and for Boolean distinguishers. When the input/output spaces and the size bound are clear from context, we write $\C$.
  ...
  \item A predictor $A \in \C_{\Yspace \to R, s_0}$ observes a view $Y : \Omega \to \Yspace$ and outputs a randomized guess for a target $X \in R$.
  \item A Boolean distinguisher $D \in \C_{\mathcal{Z} \to \{0,1\}, s}$ observes a candidate view or hybrid sample in a finite sample space $\mathcal{Z}$ and outputs a bit in $\{0,1\}$.
  ...
  \item The notation $\C_{\mathcal{X} \to \mathcal{O},s}$ is local to this note. The size parameter matters because the hybrid proof turns a predictor $A$ of size at most $s_0$ into a Boolean distinguisher $D_A$ of size at most $s_0 + t_{\mathrm{hyb}}$, where $t_{\mathrm{hyb}}$ accounts for hardwiring the real-or-zero hybrid context, sampling internal randomness, forming the candidate party view from the received pair $(W,C)$ by inserting $C$ into the missing ciphertext position, and comparing $A$'s output with the target value. Thus the real-or-zero assumption must hold for the corresponding typed distinguisher class at the larger size $s_0 + t_{\mathrm{hyb}}$. This follows the resource-bounded viewpoint used in computational entropy \cite[Sec.~2]{hsiao-lu-reyzin} and in computational indistinguishability arguments \cite[Secs.~1.3.3 and 3.2.4]{goldreich-foc}.
    ...
    \item In other words, $D_A$ is a fixed wrapper around $A$: it contains the circuit for $A$ plus the fixed logic that expands $(W,C)$ into the relevant party view and tests whether $A$ guessed the target. The circuit does not grow during sampling; its larger size comes from this wrapper construction.
    \end{enumerate}
  \end{enumerate}
...
The matching decryption key $\Dk_c$ is not in $W_1$, and $W_1$ contains no information that permits decryption under $\pk_c$, so real-or-zero indistinguishability applies with $W_1$, target key $\pk_c$, and message $V_3$:
\[
(W_1, \Encf(\pk_c, V_3; r_c)) \approx_{\C, \epscpa} (W_1, \Encf(\pk_c, 0_R; r'_c)),
\]
where $r'_c : \Omega \to \Renc$ is the fresh extension variable from Hypothesis~4, hence uniform on $\Renc$ and jointly independent of $(V_1,V_2,V_3,U_1,U_2,U_3,R_2,R_3,r_a,r_b,r_c)$. In particular, $W_1$ and $V_3$ are deterministic functions of these variables, so $r'_c$ is jointly independent of $(W_1, V_3, r_c)$.

For any allowed predictor $A$ of size at most $s_0$, form the Boolean distinguisher $D_A$ that receives a pair $(W_1,C)$, forms the corresponding Alice view by inserting $C$ into the missing $\pk_c$ ciphertext position, samples $A$'s guess on that view, and outputs $1$ iff the guess equals the auxiliary target value $V_2$. By the wrapper-overhead convention in Setup item~6, $D_A$ belongs to the typed distinguisher class covered by the real-or-zero assumption. By construction,
\[
\Pr[D_A(W_1,\Encf(\pk_c,V_3;r_c))=1] = \Pr[A(H_0)=V_2]
\]
and
\[
...
The matching decryption key $\Dk_b$ is not in $W_2$, and $W_2$ contains no information that permits decryption under $\pk_b$, so real-or-zero indistinguishability applies with $W_2$, target key $\pk_b$, and message $V_2$. This is a known-message use of real-or-zero: the auxiliary input $W_2$ explicitly contains $V_2$, which is allowed by Setup item~8 because the matching decryption key is absent.
\[
(W_2, \Encf(\pk_b, V_2; r_b)) \approx_{\C, \epscpa} (W_2, \Encf(\pk_b, 0_R; r'_b)),
\]
where $r'_b : \Omega \to \Renc$ is the fresh extension variable from Hypothesis~4, hence uniform on $\Renc$ and jointly independent of $(V_1,V_2,V_3,U_1,U_2,U_3,R_2,R_3,r_a,r_b,r_c,r'_c)$. In particular, $W_2$ and $V_2$ are deterministic functions of these variables, so $r'_b$ is jointly independent of $(W_2, V_2, r_b)$.

The same predictor-to-distinguisher conversion, again using the wrapper-overhead convention from Setup item~6, gives
\[
\left|\Pr[A(H_1)=V_2] - \Pr[A(H_2)=V_2]\right| \leq \epscpa.
\]

\paragraph{Step 3, own-key decryption and augmentation.} The ciphertext $\Encf(\pk_a, D_3; r_a)$ is encrypted under Alice's own public key, and $\Dk_a$ is in corrupted Alice's view. By correctness, $D_3$ is a deterministic function of $(\Dk_a,\Encf(\pk_a,D_3;r_a))$. Define the augmented view
\begin{multline*}
\AliceViewHyb := \big( \Dk_a, S, V_1, U_1, U_2, U_3, R_2, R_3, D_3, \\
\Encf(\pk_a, D_3; r_a), \Encf(\pk_c, 0_R; r'_c), \Encf(\pk_b, 0_R; r'_b) \big).
\end{multline*}
Any size-$s_0$ predictor $A_2$ on $H_2$ extends to a size-$(s_0 + O(1))$ predictor $A'_2$ on $\AliceViewHyb$ that ignores the explicit $D_3$ component and feeds the remaining components to $A_2$. The two predictors achieve identical success probability:
\[
\Pr[A_2(H_2) = V_2] = \Pr[A'_2(\AliceViewHyb) = V_2].
\]
Hence it suffices to bound $\Pr[A'(\AliceViewHyb) = V_2]$ for every size-$(s_0 + O(1))$ predictor $A'$ on the augmented view. Given $D_3$ and $\Dk_a$, the own-key ciphertext is a deterministic function of the fresh randomness $r_a$, which is independent of $V_2$ and of the other input variables. Hence it carries no additional information about $V_2$ once $D_3$ is conditioned on.
...
\end{align*}
so that $\AliceViewHyb = (Y, Z)$. The hypothesis $W \indep (X, Y)$ holds by Hypothesis~4 and its extension clauses for $r'_c, r'_b$. The function $Z = g(Y, W)$ is given component-wise by reading $D_3$ out of $Y$ and combining it with the public constants $\pk_a, \pk_c, \pk_b, 0_R$ and the components of $W$. The lemma then yields
\[
...
\begin{align*}
\big|\Pr[A(H_0)=V_2] - \Pr[A(H_1)=V_2]\big| &\leq \epscpa, \\
\big|\Pr[A(H_1)=V_2] - \Pr[A(H_2)=V_2]\big| &\leq \epscpa.
\end{align*}
In particular, $\Pr[A(H_0)=V_2] - \Pr[A(H_1)=V_2] \leq \epscpa$ and $\Pr[A(H_1)=V_2] - \Pr[A(H_2)=V_2] \leq \epscpa$. Adding the two inequalities gives
\begin{align*}
...
\begin{thebibliography}{12}\bibitem{hsiao-lu-reyzin}
Hsiao, Lu, Reyzin.
