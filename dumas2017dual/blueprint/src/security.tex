...
\begin{definition}[The Paillier setting]
  ...
  \rocqok
  \uses{def:dsdp_setting,def:dsdp_instance_sequence,def:indcpa_assumption}
  ...
  ring, the three private keys, the two encryption coins, the sequence of
  IND-CPA assumptions, and two negligibility facts, one on the inverse modulus
  and one on the assumed advantage. The counting side is the uniform law on
  five coordinates of $\mathbb{Z}_{p_k q_k}$ at Alice's own weights, so the two
  axes read one query at every $k$.
  \[ \#\lvert\mathrm{plain}(\mathcal{S}_k)\rvert = p_k q_k \]
  ...
  are assumed values, no Paillier key being constructed anywhere, and the
  assumption sequence with its two negligibility facts is assumed, so every
  $\varepsilon$ read off at this setting is the advantage that sequence assumes
  at $k$.
\end{definition}
...
\begin{corollary}[The twenty-six at the Paillier setting]
  ...
  \rocqok
  \uses{def:paillier_setting,thm:dsdp_securityP,cor:alice_guess_paillier,thm:alice_guess_negligible}
  ...
  every $k$ makes the trace guessing probability negligible.
  ...
  $1 < p_k, q_k$ entering. Its advantage half stays assumed. Decisional
  composite residuosity remains the source a proved advantage sequence would
  start from, so this corollary is the socket such a bound plugs into rather
  than a security proof of Paillier.
\end{corollary}
...
\begin{definition}[The Benaloh setting]
  ...
  plaintext space, and not at $n_k$, which sizes the ciphertext space. Keys and
  the assumption sequence are assumed as at Paillier.
\end{definition}
...
\begin{corollary}[The twenty-six at the Benaloh setting]
  ...
  \rocqok
  \uses{def:benaloh_setting,thm:dsdp_securityP,thm:alice_guess_negligible}
  All twenty-six statements hold at the Benaloh setting, and the asymptotic one
  reads along the Benaloh sequence.
  \[ \mathrm{negl}\big(k \mapsto
       \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]\big) \]
  Its unconditional half is discharged at the block size $r_k$. Its advantage
  half stays assumed, and $r$-th residuosity remains the source a proved
  advantage sequence would start from.
\end{corollary}
