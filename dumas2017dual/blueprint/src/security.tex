...
\begin{definition}[A bound read off a chain]
  ...
  \[ a \le \mathrm{ev}(s) \]
  $\mathrm{plus}(m, l, c, H)$ builds one from a chain $m$ by adding the
  external term $c$, labelled $l$, to its loss, for
  $H : \mathrm{obl}_{\mathrm{p}}(l)$, together with the two equations
  $c = \mathrm{cost}(l)$ and $\mathrm{game}(l) = b_m$, so the term written and
  the game the chain stopped at are both what the label claims.
  ...
  a hybrid reaches its published form here, and the terminal label $l$ is
...
\begin{definition}[The Paillier hybrid as a chain]
  ...
  \rocqok
  \uses{def:chain,def:claim,def:loss,def:dcr_assumption,thm:paillier_dcr_epsilon_le,lem:unit_fdist_translateE}
...
\begin{definition}[The Benaloh hybrid as a chain]
  ...
  \rocqok
  \uses{def:chain,def:claim,def:loss,def:benaloh_residuosity_assumption,thm:benaloh_residuosity_epsilon_le,lem:unit_fdist_translateE}
...
\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  ...
  \rocqok
  \uses{def:chain,def:chain_bound,def:claim,def:loss,lem:alice_view_advantage,lem:guess_all_zero,thm:alice_guess_real}
