...
Running the three DSDP party programs at the idealized scheme, where a
ciphertext is its plaintext, and reading the trace the interpreter writes for a
relay makes that tuple the object the executed protocol hands it.

\begin{definition}[The run at the idealized scheme and its three traces]
  \label{def:ideal_run}
  \rocq{infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.dsdp_run_traces_idealE}
  \rocqok
  \uses{def:protocol_flow,def:interpreter_sample}
  The three piSMC programs of Definition~\ref{def:protocol_flow} run at the
  idealized homomorphic scheme, where plaintext, randomness, ciphertext and
  both keys are the plaintext ring, encryption returns its plaintext and
  decryption returns its ciphertext. Every carrier is therefore finite, so a
  trace of the run is a sequence over a finite alphabet and needs no encoding
  into one.
  \[ \mathrm{enc}(pk, x, r) = x, \qquad \mathrm{dec}(dk, c) = \mathrm{Some}\ c \]
  ...
  Bob's six are his second coin $r_{b2}$, the two combines he receives, his
  first coin $r_{b1}$, his input $V_2$ and his key $k_b$. Charlie's five are
  his second coin, the aggregate ciphertext Bob forwards, his first coin, his
  input $V_3$ and his key.
  \[ \mathrm{BobTrace} = \big[\, r_{b2},\ V_3 U_3 + R_3,\
     V_2 U_2 + R_2,\ r_{b1},\ V_2,\ k_b \,\big] \]
  \[ \mathrm{CharlieTrace} = \big[\, r_{c2},\
     V_3 U_3 + R_3 + V_2 U_2 + R_2,\ r_{c1},\ V_3,\ k_c
     \,\big] \]
...
\begin{lemma}[A relay's trace is its view recoded]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.ideal_trace_of_run,
  ...
  \rocqok
  \uses{def:ideal_run,def:party_views}
...
\begin{theorem}[The relay bounds at the trace]
  ...
  These four are at the idealized scheme, where a ciphertext is its plaintext
  and no ciphertext depends on a coin, so the coin slots the trace
