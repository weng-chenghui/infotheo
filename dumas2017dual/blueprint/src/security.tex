...
exactly what the three steps of Definition~\ref{def:hop_script} build, so the
label list of a finished script is the list of assumptions its bound rests on,
carried beside their numeric total. epsHop is the dual of the piSMC language of
...
Every step carries its proof slot, so a finished script holds every obligation
...
\begin{definition}[A script and its three steps]
  ...
  An assumption enters a script at $\mathrm{hop}$ and nowhere else, which is
  why the loss of a finished script is the complete list of what its bound
  rests on. The equation $g' = \mathrm{tgt}(C(l))$ is discharged by
  reduction, so the game a hop reaches is the game its label claims, and the
  game a hop leaves is the source of that claim. $\mathrm{same}$ is the step an
  information-theoretic identity between two games takes, and it leaves the
  loss as it stands. The one triangle inequality of the development is the
  soundness statement above, so a script of any length is read through it
  alone. \texttt{le\_of\_eq} turns an equality of advantages into the
...
\begin{definition}[The Paillier hybrid as a script]
  ...
  The three-step hybrid of Theorem~\ref{thm:paillier_dcr_epsilon_le} as a script
...
\begin{definition}[Alice's trace argument as a script opening at the protocol]
  ...
  That step carries no label and adds nothing to the loss: by
  ...
  is therefore a bound on $\gamma_\tau(D_g)$. \texttt{alice\_claim} fixes what
  ...
  The script spends the two hop labels before the endpoint label, so the bound
...
\begin{definition}[Alice's trace against the simulation at the class epsilon]
  ...
  \[ \mathrm{adv}^\tau(D)
       = \big\lvert \gamma_\tau(D) - \Pr_{\Ideal^\tau}[D] \big\rvert, \qquad
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  Definition~\ref{def:alice_script}, one at each $k$. That script spends the
  same three labels at every $k$, and the loss of each of
