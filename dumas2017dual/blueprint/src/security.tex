...
\begin{itemize}
  ...
  \item \texttt{s ;; bound l c by H} bounds the chain: its last game is at
    most $c$, a term labelled $l$, guaranteed by
    $H : \mathrm{current} \le c$, so the first game is at most the total loss.
\end{itemize}
...
\begin{definition}[A bound read off a chain]
  ...
  \rocq{infotheo.computational_security.epshop.chain_bound,
       infotheo.computational_security.epshop.chain_eval}
  \rocqok
  \uses{def:chain}
  A chain bound is a first endpoint $a$, a loss $s$, and a proof that the total
  of the loss bounds $a$ itself.
  \[ a \le \mathrm{ev}(s) \]
  $\mathrm{eval}(m, l, c, H_c)$ builds one from a chain $m$ by charging its
  current endpoint at $c$ under the label $l$, for $H_c : b_m \le c$.
  \[ \mathrm{eval}(m, l, c, H_c) = \big(a_m,\;
       s_m \mathbin{+\!\!+} [(l, c)]\big) \]
  ...
  a step. It is the \texttt{bound} statement of a program, written after the
