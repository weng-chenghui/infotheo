...
\begin{theorem}[Guessing Bob's input from Alice's real tuple]
  ...
  \rocqok
  \uses{def:dsdp_setting,thm:alice_simulation_secure,lem:simulator_factorization,lem:guess_all_zero,def:distinguisher_of_predictor}
  ...
  advantages of it. The epsHop program \texttt{alice\_chain} returns the same
  bound, its last line adding that fiber term to the two hops.
...
\begin{theorem}[The two bounds at the trace]
  ...
  \rocqok
  \uses{def:dsdp_setting,lem:trace_of_hop_tuple,def:alice_trace_chain,def:alice_trace_sim_chain,thm:alice_guess_real,thm:alice_simulation_secure}
  ...
  Proved in \texttt{dsdp\_alice\_trace\_link.v}. The record
  \texttt{dsdp\_security} carries both bounds at the raw trace,
  Theorems~\ref{thm:raw_trace_guess} and~\ref{thm:raw_trace_simulation}, the
  two bounds read through the decoding of Lemma~\ref{lem:raw_trace_decode}.
\end{theorem}
...
\begin{theorem}[Trace and tuple leave the same uncertainty]
  ...
  \rocqok
  \uses{def:dsdp_setting,lem:trace_of_hop_tuple}
...
\begin{corollary}[The bound at the Paillier setting]
  \label{cor:alice_guess_paillier}
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_main.paillier_trace_guess_V2_admissible_pq_le,
  ...
  At the $k$-th instance of the Paillier setting, with modulus $p_k q_k$, a
  trace predictor whose two reduction adversaries the class induced by
  $\mathcal{B}_k$ admits matches Bob's input with probability at most
  $1/(p_k q_k)$ plus four times the assumed residuosity advantage.
  \[ \Pr[\,g(\mathrm{AliceTrace}_k) = V_2\,]
  ...
  exactly $p_k q_k$ elements, and the coin plumbing the abstract bound
  quantifies over is supplied by the scheme's own randomness. The
  computational side is decisional composite residuosity and nothing further,
  at the cost Definition~\ref{def:paillier_indcpa_assumption} fixes: the
  factor four is two keys, Bob's and Charlie's, times two residuosity calls
  per key, the identity \texttt{paillier\_epsilon\_at\_dcrE}. What is assumed
  here is $\mathcal{B}_k$, and the class premises restrict which predictors
  the bound covers.
\end{corollary}

\begin{corollary}[The bound at the Benaloh setting]
  \label{cor:alice_guess_benaloh}
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_main.benaloh_trace_guess_V2_admissible_pq_le,
  ...
  At the $k$-th instance of the Benaloh setting, whose plaintext space has
  $r_k = p_k q_k$ elements, a trace predictor whose two reduction adversaries
  the class induced by $\mathcal{B}_k$ admits matches Bob's input with
  probability at most $1/(p_k q_k)$ plus four times the assumed residuosity
  advantage.
  \[ \Pr[\,g(\mathrm{AliceTrace}_k) = V_2\,]
       \le \tfrac{1}{p_k q_k} + 4\,\varepsilon_{\mathrm{res},k} \]
  The computational side is $r$-th residuosity at the cost
  Definition~\ref{def:benaloh_indcpa_assumption} fixes, the factor four again
  two keys times two residuosity calls per key, the identity
...
\begin{theorem}[The class cannot admit every adversary]
  ...
  anything.
  Proved in \texttt{dsdp\_alice\_trace\_link.v}, the lower bound is field
  \texttt{decrypt\_bob\_epsilon\_ge} of \texttt{dsdp\_security}, read off at
  each instance of \texttt{dsdp\_main.v}.
\end{theorem}
