...
\begin{definition}[Drawing a coin inside a program]
  ...
  continuation, advances the stream, and appends nothing to the trace.
  \[ \mathrm{step}\big(\mathrm{Sample}\ f,\ \mathit{tr},\ r :: \mathit{sd}\big)
       = \big(f(r),\ \mathit{tr},\ \mathit{sd}\big) \]
  The three DSDP streams are the six coins of one record, two per party in the
  order that party encrypts. A drawn coin reaches the trace only through the
  ciphertext its continuation builds, so Alice's trace is the image of the
  sampled coins through the programs and holds no coin of any party as an entry
  of its own.
\end{definition}
...
\begin{definition}[Alice's executed trace]
  ...
  eighteen entries, each a plaintext, a ciphertext, or a private key. Alice's
...
\begin{theorem}[Trace and tuple leave the same uncertainty]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_tuple,
