...
\begin{definition}[A label and the claim it stands for]
  ...
  sends it to what it asserts. A claim $\mathrm{Claim}(s, t, e)$ names a
  ...
  $\mathrm{tgt}$ and $\mathrm{cost}$. The claim is the whole content of a
...
\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  ...
  of the reduction its label names, at Bob's key and then at Charlie's, and a
  third step hops from the all-zero endpoint to the zero game, so the program
  is a chain bound on $\gamma_0(D_g)$. \texttt{alice\_claim} fixes what
  each of the three labels claims: the games a hop label joins and the
  advantage it costs, and the all-zero game against the zero game.
  ...
  later statement requires it to be small. The term
...
\begin{definition}[Alice's trace argument as a chain opening at the protocol]
  ...
  step to the zero game and the terminal statement are those of
  Definition~\ref{def:alice_chain}, so the loss and the bound are the same
  three terms.
...
\begin{definition}[Alice's trace argument at the assumed advantage]
  ...
  is the shape Definition~\ref{def:paillier_chain} has. The zero-game label costs the same
...
\begin{definition}[Alice's trace against the simulation at the assumed advantage]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_advantage,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_advantageE}
  \rocqok
...
\begin{theorem}[A sequence of programs has a negligible advantage]
  ...
  \rocq{infotheo.computational_security.epshop_family.advantage_negligible}
  \rocqok
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  \rocqok
  \uses{def:negligible_fun,def:dsdp_instance,def:dsdp_instance_sequence,thm:alice_guess_admissible,def:alice_trace_chain_assumed,def:negligible_claims,thm:advantage_negligible}
  ...
  \varepsilon_{\mathrm{cpa},k})$ and the zero-game label costs
...
\begin{theorem}[The trace simulation distance sequence is negligible]
  ...
  \rocqok
  \uses{def:alice_trace_sim_chain_assumed,def:negligible_claims,thm:advantage_negligible}
