...
\begin{theorem}[Residual uncertainty of the relay inputs]
  ...
  \rocqok
  \uses{def:dsdp_security,def:party_views,lem:dsdp_is_correct}
  ...
  The theorem is stated over one setting and one $k$: the two primes, the
  sample law, the eleven inputs, their uniformity and their independence are
  read off the record, and the conditioner's $S$ is the output computed from
  the inputs rather than a free variable. Because the record's
...
\begin{definition}[The two hop reductions]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.hop0_stateT,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.hop0_reduction,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.hop1_reduction}
  \rocqok
...
\begin{theorem}[Guessing Bob's input from Alice's real tuple]
  ...
  \rocqok
  \uses{def:dsdp_security,lem:alice_view_advantage,lem:guess_all_zero,def:distinguisher_of_predictor}
  ...
  The bound is stated over one setting and one $k$, at the scheme that
  setting's sequence supplies at that $k$, in place of an instance supplied on
  its own.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Unpredictability entropy]
  ...
  \rocqok
  \uses{def:dsdp_security,thm:alice_guess_real}
  ...
  The two forms proved in \texttt{dsdp\_main.v} are stated over one setting
  and one $k$, at the scheme that setting's sequence supplies there.
\end{theorem}
...
\begin{corollary}[The same bound at Alice's full view]
  ...
  \rocqok
  \uses{def:dsdp_security,thm:alice_guess_real,def:party_views}
  ...
  The \texttt{dsdp\_main.v} form of the corollary is stated over one setting
  and one $k$.
\end{corollary}
...
\begin{theorem}[Bounded simulation security]
  ...
  \rocqok
  \uses{def:dsdp_security,def:party_views,lem:alice_view_advantage,lem:simulator_factorization}
  ...
  The bound is stated over one setting and one $k$, at the scheme that
  setting's sequence supplies there.
\end{theorem}
\begin{proof}
...
\begin{theorem}[The three bounds at the trace]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_main.alice_trace_guess_V2_le,
       ...
       infotheo.dumas2017dual.dsdp.dsdp_main.alice_trace_unpredictability_ge,
  ...
  \rocqok
  \uses{def:dsdp_security,lem:trace_of_hop_tuple,thm:alice_guess_real,thm:alice_unpredictability,thm:alice_simulation_secure}
  ...
  The three bounds proved in \texttt{dsdp\_main.v} are stated over one setting
  and one $k$, at the scheme that setting's sequence supplies there.
\end{theorem}
...
\begin{theorem}[Trace, view, and tuple leave the same uncertainty]
  ...
  \rocqok
  \uses{def:dsdp_security,lem:trace_of_hop_tuple,cor:alice_guess_view}
  ...
  The two equalities proved in \texttt{dsdp\_main.v} are stated over one
  setting and one $k$.
\end{theorem}
...
\begin{theorem}[The class-conditional trace bound]
  ...
  \rocqok
  \uses{def:dsdp_security,thm:alice_trace_bounds,def:indcpa_assumption}
  ...
  Both forms are stated over one setting and one $k$, and the composite form
  reads its two primes and the plaintext count off the setting's link field
...
\begin{theorem}[The class cannot admit every adversary]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_main.decrypt_bob_epsilon_ge,
       infotheo.dumas2017dual.dsdp.dsdp_main.decrypt_epsilon_sum_ge}
  \rocqok
  \uses{def:dsdp_security,def:trace_decryptor,thm:alice_guess_real,lem:guess_all_zero}
  ...
  anything.
  Both lower bounds are stated over one setting and one $k$.
\end{theorem}
...
\begin{corollary}[A small-epsilon assumption must reject the decryptor]
  ...
  \rocqok
  \uses{def:dsdp_security,thm:decrypt_lower_bound,def:indcpa_assumption}
  ...
  The corollary is stated over one setting and one $k$.
\end{corollary}

\begin{corollary}[The premise-free bound is false]
  \label{cor:admissible_premise_free}
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_main.decrypt_guess_V2_premise_free_lt}
  \rocqok
  \uses{def:dsdp_security,thm:alice_guess_admissible,def:trace_decryptor,def:indcpa_assumption}
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance_sequence.alice_trace_guess_V2_negligible,
       infotheo.dumas2017dual.dsdp.dsdp_main.alice_trace_guess_V2_negligible}
  \rocqok
...
\begin{corollary}[The bound along a Paillier sequence]
  \label{cor:alice_guess_paillier_negligible}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance_sequence.alice_trace_guess_V2_paillier_negligible}
  \rocqok
  \uses{thm:alice_guess_negligible,cor:alice_guess_paillier,def:dsdp_instance_sequence}
  At the sequence built from a sequence of Paillier moduli $p_k q_k$, the
  guessing sequence is negligible.
  \[ \mathrm{negl}\big(k \mapsto \tfrac{1}{p_k q_k}\big) \wedge
     \mathrm{negl}\big(k \mapsto \varepsilon_{\mathcal{A}_k}\big)
     \implies \mathrm{negl}\big(k \mapsto
       \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]\big) \]
  The sequence is built from the modulus sequence $p_k q_k$, the weights, keys
  and coins supplied as sequences, the assumption sequence $\mathcal{A}_k$, and
  the two displayed hypotheses, which are exactly its two negligibility fields.
  The first is discharged at the plaintext cardinality of the Paillier
  packaging, read at $k$ as $p_k q_k$. The packaging, coin type, cardinality
  and coin map are the fixed-instance ones applied at $k$, so no number theory
  beyond $1 < p_k, q_k$ enters. Decisional composite residuosity remains the
  source a proved advantage sequence would start from.
\end{corollary}

\begin{corollary}[The bound along a Benaloh sequence]
  \label{cor:alice_guess_benaloh_negligible}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance_sequence.alice_trace_guess_V2_benaloh_negligible}
  \rocqok
  \uses{thm:alice_guess_negligible,def:negligible_fun,def:dsdp_instance_sequence}
  At the sequence built from a sequence of Benaloh block sizes $r_k$, the
  guessing sequence is negligible.
  \[ \mathrm{negl}\big(k \mapsto \tfrac{1}{r_k}\big) \wedge
     \mathrm{negl}\big(k \mapsto \varepsilon_{\mathcal{A}_k}\big)
     \implies \mathrm{negl}\big(k \mapsto
       \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]\big) \]
  The sequence is built from the block-size sequence $r_k$, the weights, keys
  and coins supplied as sequences, the assumption sequence $\mathcal{A}_k$, and
  the two displayed hypotheses, which are exactly its two negligibility fields.
  ...
  space. $r$-th residuosity remains the source a proved advantage sequence
  would start from.
\end{corollary}
...
\begin{definition}[The idealized setting]
  \label{def:idealized_security}
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_security.idealized_security}
  \rocqok
  \uses{def:dsdp_security,def:cipher_constant_assumption,def:negligible_fun}
...
\begin{theorem}[The trace determines Bob's input]
  ...
  \rocqok
  \uses{def:dsdp_security,def:trace_decryptor,thm:centropy_trace_tuple}
  ...
  well.
  The equality is stated over one setting and one $k$.
\end{theorem}
...
\begin{theorem}[Where the Shannon statement is not degenerate]
  ...
  \rocqok
  \uses{def:dsdp_security,thm:dsdp_centropy_uniform,def:alice_hop_ladder}
  ...
  Both equalities are stated over one setting and one $k$, at the scheme that
  setting's sequence supplies there.
\end{theorem}
...
\begin{theorem}[A degenerate query leaks Bob's input]
  ...
  \rocqok
  \uses{def:dsdp_security,def:party_views,thm:dsdp_centropy_uniform}
  ...
  The theorem is stated over one setting and one $k$, and the query $U_2 = 1$,
  $U_3 = 0$ stays a premise, since it is the corrupted Alice's choice and not a
...
\begin{theorem}[Relay privacy of $V_1$]
  ...
  \rocqok
  \uses{def:dsdp_security,def:party_views}
  ...
  Both statements are made over one setting and one $k$: the uniformity of
  $V_1$ is a field of the record, and the independence of each relay's data from
  $V_1$ is derived from its each-against-the-rest fields.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Bob's view hides $V_3$]
  ...
  The theorem is stated over one setting and one $k$: the uniformity of $V_3$
  and of $R_3$ are fields of the record, and the freshness of $R_3$ against
  Charlie's weighted input is derived from its each-against-the-rest fields.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Charlie's view hides $V_2$]
  ...
  The theorem is stated over one setting and one $k$: the uniformity of $V_2$
  and of $R_2$ are fields of the record, and the freshness of $R_2$ against
  Bob's weighted input is derived from its each-against-the-rest fields.
\end{theorem}
\begin{proof}
  \uses{lem:one_time_pad}
  \rocqok
