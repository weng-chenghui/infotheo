...
\begin{theorem}[The two bounds at the trace]
  ...
  \rocqok
  \uses{lem:trace_of_hop_tuple,def:alice_trace_script,def:alice_trace_sim_script,thm:alice_guess_real,thm:alice_simulation_secure}
  ...
  The simulation bound is read off the script of
  Definition~\ref{def:alice_trace_sim_script}, and the guessing bound is that
  simulation bound at the predictor's distinguisher plus the fiber term of the
  ideal trace, as at the tuple, read off the script of
  Definition~\ref{def:alice_trace_script}.
...
\begin{lemma}[A finite sum of negligible sequences is negligible]
  ...
  script totals one loss term per label of its loss, so a bound read off such
...
\begin{theorem}[The class-conditional trace bound]
  ...
  \rocqok
  \uses{thm:alice_trace_bounds,def:indcpa_assumption,def:alice_script}
  ...
  The bound is the script of Definition~\ref{def:alice_script} read through
  soundness, and that script spends the two premises at its two hops, so the
  one triangle inequality here is the soundness statement of
  Definition~\ref{def:hop_script}.
  Proved in \texttt{dsdp\_alice\_main.v} as
...
\section{epsHop, the scripts a multi-hop bound is written in}
...
section states the form they are written in, epsHop. A hopping argument is a
script, a value of the type $\mathrm{Script}_C(a, b, s)$ indexed by the game
$a$ it opens at, the game $b$ it stops at, and the list $s$ of labels it spends.
The Rocq source displays that type as the inequality its values witness,
\texttt{\textbackslash{}hops[ C ] `| a - b | <= s}. The values of the type are
exactly what the three steps of Definition~\ref{def:hop_script} build. A label
enters a script at hop and nowhere else, so the loss of a finished script lists
every term its bound is charged to, carried beside their numeric total. epsHop is the dual of the piSMC language of
\texttt{smc/pismc.v}: piSMC writes what the parties of a protocol do, epsHop
writes what a security argument about them loses. The scripts of the section
are the Paillier hybrid, the Benaloh hybrid, and Alice's guessing and
simulation arguments at her executed trace, each at the advantage its
reductions show and at the epsilon an adversary class assumes, and the section
closes with the reading of a script along a sequence of security parameters.

A script is written one tactic line per step, on a goal of script type from the
current game $x$ to the final game $z$, and each line leaves the script type of
the rest as the residual goal.
\begin{itemize}
  \item \texttt{hop l to g' by (H)} spends the label $l$ on a goal
    $\mathrm{Script}_C(x, z, l :: s)$ with $x = \mathrm{src}(C(l))$. It checks
    $g'$ against $\mathrm{tgt}(C(l))$ by reduction, takes
    $H : \mathrm{obl}(C(l))$, and leaves $\mathrm{Script}_C(g', z, s)$.
  \item \texttt{same to g' by (H)} rewrites the game to $g'$ at no loss, for
    $H : x = g'$, and leaves $\mathrm{Script}_C(g', z, s)$ over the same list
    $s$.
  \item \texttt{plus l by (H)} is a hop under $l$ to the zero game followed by
    \texttt{stop}. It closes $\mathrm{Script}_C(x, 0, [l])$ for a label whose
    target is the zero game, with $H : \lvert x - 0 \rvert \le
    \mathrm{loss}(C(l))$, and is spelled \texttt{plus} so that a reader sees a
    term added to the bound.
  \item \texttt{stop} closes $\mathrm{Script}_C(g, g, [\,])$, the script that
    spends nothing.
\end{itemize}
The label slot names the assumption invoked, and the proof slot is the
justification that assumption's claim asks for, checked against the claim.
Every hop and same step carries its proof slot, so a finished script holds every obligation
its labels name, and a bound is read off it by the soundness statement of
Definition~\ref{def:hop_script}.
...
\begin{definition}[A script and its three steps]
  \label{def:hop_script}
  \rocq{infotheo.computational_security.epshop.hop_script,
       infotheo.computational_security.epshop.script_stop,
       infotheo.computational_security.epshop.script_hop,
       infotheo.computational_security.epshop.script_same,
       infotheo.computational_security.epshop.hop_script_sound,
       infotheo.computational_security.epshop.hop_script_total,
  ...
  \rocqok
  \uses{def:claim,def:loss}
  A script from $a$ to $b$ over $s$ is a derivation, one step per constructor,
  that the acceptance probabilities $a$ and $b$ lie within the total of $s$.
  Its type $\mathrm{Script}_C(a, b, s)$ is inductive, with three constructors.
  \[ \mathrm{stop}(g) : \mathrm{Script}_C(g,\, g,\, [\,]) \]
  \[ \mathrm{hop}(l, g', H, p) :
       \mathrm{Script}_C\big(\mathrm{src}(C(l)),\, z,\, l :: s\big)
     \quad\text{for } H : \mathrm{obl}(C(l)),\
       g' = \mathrm{tgt}(C(l)),\ p : \mathrm{Script}_C(g', z, s) \]
  \[ \mathrm{same}(g', H, p) : \mathrm{Script}_C(x,\, z,\, s)
     \quad\text{for } H : x = g',\ p : \mathrm{Script}_C(g', z, s) \]
  Every script bounds the distance between its two games by the total of its
  loss, in both readings of the total.
  \[ \mathrm{Script}_C(a, b, s) \implies
       \lvert a - b \rvert \le \mathrm{ev}(s), \qquad
     \mathrm{Script}_C(a, b, s) \implies
       \lvert a - b \rvert \le \mathrm{tot}(s) \]
  A label enters a script at $\mathrm{hop}$ and nowhere else, so the loss of a
  finished script lists every term its bound is charged to. The game a hop
  reaches is the target its label claims; the hop tactic discharges the
  equation by reduction. The game a hop leaves is the source of that claim.
  $\mathrm{same}$ is the step an information-theoretic identity between two
  games takes, and it leaves the loss as it stands. Every script, of any
  length, is read through this one soundness statement. \texttt{le\_of\_eq} turns an equality of advantages into the
  inequality a hop asks for, \texttt{plus\_le} turns $0 \le x \le c$ into the
  form $\lvert x - 0 \rvert \le c$ that a hop to the zero game asks for, and
  \texttt{advantage0} reads $\lvert x - 0 \rvert$ back as $x$, so a script
  stopping at the zero game bounds the game it opens at.
\end{definition}

\begin{lemma}[What a script type guarantees]
  \label{lem:hop_script_guard}
  \rocq{infotheo.computational_security.epshop.loss_obligations,
       infotheo.computational_security.epshop.hop_script_obligations,
       infotheo.computational_security.epshop.hop_script_nil,
       infotheo.computational_security.epshop.hop_script_not_total}
  \rocqok
  \uses{def:hop_script}
  Every label a script spends has its claim proved, a script over the empty
  loss joins equal games, and some script type is empty.
  \[ \mathrm{Script}_C(a, b, s) \implies
       \bigwedge_{l \in s} \mathrm{obl}(C(l)), \qquad
     \mathrm{Script}_C(a, b, [\,]) \implies a = b \]
  \[ \neg\big(\forall C\, a\, b\, s,\ \mathrm{Script}_C(a, b, s)\big) \]
  The last statement is witnessed at the games $0$ and $1$ over the empty
  loss. Together the three make the label list of a script an assumption trail
  that the kernel checks, whatever tactics built the script: the obligations of
  its labels are recoverable from the script itself, a bound spending the empty
  list is an identity of games, and a script is a witness whose label list is
  data a result carries.
\end{lemma}

\begin{definition}[What a script returns]
  \label{def:script_result}
  \rocq{infotheo.computational_security.epshop.script_result,
       infotheo.computational_security.epshop.result_of_script}
  \rocqok
  \uses{def:hop_script,def:loss}
  ...
  A script returns the result on the distance between its two games at the
  total of its loss, the loss being the index $s$ of its type.
  \[ p : \mathrm{Script}_C(a, b, s) \ \longmapsto\
       \big(\lvert a - b \rvert,\; s,\; \mathrm{tot}(s)\big) \]
  A script stopping at the zero game therefore returns a bound on the
  probability of the game it opened at, through \texttt{advantage0}, the game
  being nonnegative, which is the shape a secrecy theorem
  has. The last field keeps the label list the source of a published bound. A
  reading of a bound along a sequence of security parameters is a reading of
  that list, and it is sound because of this field. At one parameter a theorem
  reads its number off a script through soundness and one identity between
  $\mathrm{tot}(s)$ and that number, and since $\mathrm{tot}$ converts to a
  plain sum on a list of literal labels, that identity is ring arithmetic.
\end{definition}

\begin{definition}[The Paillier hybrid as a script]
  \label{def:paillier_script}
  \rocq{infotheo.computational_security.paillier_indcpa_scheme.paillier_label,
       ...
       infotheo.computational_security.paillier_indcpa_scheme.paillier_script,
       infotheo.computational_security.paillier_indcpa_scheme.dcr_totalE}
  \rocqok
  \uses{def:hop_script,def:script_result,def:claim,def:loss,def:dcr_assumption,thm:paillier_dcr_epsilon_le,lem:unit_fdist_translateE}
  The two-call hybrid of Theorem~\ref{thm:paillier_dcr_epsilon_le} as a script
  over the four acceptance probabilities it passes through. It opens at the
  ...
  Lemma~\ref{lem:unit_fdist_translateE}, an identity that adds nothing to the
  loss; hops to the plain reduction under $\mathsf{res}_{pq}$, which is the
  zero arm; and stops. Its loss, the last index of its type, is the two labels
  $\mathsf{dcr\_g}$ and $\mathsf{dcr\_0}$, named after the reduction each hop
  invokes the assumption through, and \texttt{paillier\_claim} fixes for each
  of them the two experiments its call moves between and the epsilon it
  assumes.
  \[ \mathrm{tot}[\mathsf{dcr\_g}, \mathsf{dcr\_0}]
       = \varepsilon_{\mathrm{dcr}} + \varepsilon_{\mathrm{dcr}}
       = 2\,\varepsilon_{\mathrm{dcr}} \]
  The bound $2\,\varepsilon_{\mathrm{dcr}}$ is read off the script through
  \texttt{hop\_script\_total} and the identity \texttt{dcr\_totalE}. The two
  class memberships $\mathrm{adm}(\mathcal{B}_{\mathrm{mul}})$ and
  $\mathrm{adm}(\mathcal{B}_{\mathrm{pl}})$ are hypotheses of the script, spent
  in the justification of the hop each one licenses. Both terms are
  conditional on $\mathcal{B}$, so this loss has one kind of term. The number
  in Theorem~\ref{thm:paillier_dcr_epsilon_le} is the bound this script
  carries and its two hypotheses are those two memberships, so that theorem is
  this script read through soundness.
\end{definition}

\begin{definition}[The Benaloh hybrid as a script]
  \label{def:benaloh_script}
  \rocq{infotheo.computational_security.benaloh_indcpa_scheme.benaloh_label,
       ...
       infotheo.computational_security.benaloh_indcpa_scheme.benaloh_script,
       infotheo.computational_security.benaloh_indcpa_scheme.residuosity_totalE}
  \rocqok
  \uses{def:hop_script,def:script_result,def:claim,def:loss,def:benaloh_residuosity_assumption,thm:benaloh_residuosity_epsilon_le,lem:unit_fdist_translateE}
  ...
  \[ \mathrm{tot}[\mathsf{residuosity\_y}, \mathsf{residuosity\_0}]
       = \varepsilon_{\mathrm{res}} + \varepsilon_{\mathrm{res}}
       = 2\,\varepsilon_{\mathrm{res}} \]
  One place separates it from the Paillier script. Its middle identity takes no
  ...
  without reading the key's order condition. The bound
  $2\,\varepsilon_{\mathrm{res}}$ is read off the script through
  \texttt{hop\_script\_total} and \texttt{residuosity\_totalE}. Both terms are
  conditional on $\mathcal{B}$, and
  Theorem~\ref{thm:benaloh_residuosity_epsilon_le} is again this script read
  through soundness at its two class memberships.
\end{definition}

\begin{definition}[Alice's trace argument as a script opening at the protocol]
  \label{def:alice_trace_script}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_label,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_trace_script}
  \rocqok
  \uses{def:hop_script,def:script_result,def:claim,def:loss,def:alice_trace,lem:trace_of_hop_tuple,lem:alice_view_advantage,lem:guess_all_zero}
  Alice's guessing argument as one script, opening at the executed protocol.
  ...
  protocol itself rather than the tuple $\hoptuple{0}$. The script's first line
  steps to the tuple.
  \[ \mathrm{same\ to}\ \gamma_0(D_{g \circ \tau})\ \mathrm{by}\
       \texttt{accept\_trace\_tupleE} \]
  That step is an identity, free in the loss: by
  ...
  $\gamma_2$ at that lift, each spending the advantage of the reduction its
  label names, at Bob's key and then at Charlie's, and a $\mathrm{plus}$ step
  adds the term of the all-zero game against the zero game. The script's type
  is therefore a bound on $\lvert \gamma_\tau(D_g) - 0 \rvert$, which
  \texttt{advantage0} reads as $\gamma_\tau(D_g)$. \texttt{alice\_claim} fixes what
  each of the three labels claims: the games a hop label joins and the
  advantage it loses, and the all-zero game against the zero game.
  \[ \mathrm{Script}_C\big(\gamma_\tau(D_g),\ 0,\
       [\mathsf{cpa\_bob}, \mathsf{cpa\_charlie},
        \mathsf{uniform\_fiber}]\big),
     \qquad
     \mathrm{tot}(s) = \tfrac{1}{m} + \epshopsum{D_{g \circ \tau}} \]
  The script spends the two hop labels before the zero-game label, so the bound
  reorders, which is what \texttt{alice\_totalE} proves.
  ...
  its gap exactly the term it spends, read as an inequality through
  \texttt{le\_of\_eq}. The $\mathrm{plus}$ step is justified by
  \texttt{all\_zero\_game\_V2\_le\_invm}, which is
  Lemma~\ref{lem:guess_all_zero} carried to the game form and is the one place
  the fiber enters. The trace guessing bound of
  Theorem~\ref{thm:alice_trace_bounds} is this script read through soundness,
  \texttt{guess\_V2\_acceptE}, \texttt{advantage0} and
  \texttt{alice\_totalE}.
\end{definition}

\begin{definition}[Alice's trace against the simulation as one script]
  \label{def:alice_trace_sim_script}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.accept_trace_ideal_tupleE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_trace_sim_script}
  \rocqok
  \uses{def:alice_trace_script,def:alice_trace,lem:trace_of_hop_tuple,lem:simulator_factorization}
  The two hops of Definition~\ref{def:alice_trace_script} with one
  $\mathrm{same}$ step at each end. The script opens at $\gamma_\tau(D)$, runs
  the two ciphertext replacements at the lifted test $D \circ \tau$, and stops
  at $\Pr_{\Ideal^\tau}[D]$, the probability that the same test accepts the
  ...
  \[ \mathrm{Script}_C\big(\gamma_\tau(D),\ \Pr_{\Ideal^\tau}[D],\
       [\mathsf{cpa\_bob},\ \mathsf{cpa\_charlie}]\big),
     \qquad \mathrm{tot}(s) = \epshopsum{D \circ \tau} \]
  The script stops at the simulated trace itself, so the bound read off it
  through soundness is the trace-level simulation bound of
  Theorem~\ref{thm:alice_trace_bounds}.
...
\begin{definition}[Alice's trace argument at the class epsilon]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_claim_admissible,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_script,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.f_guess_V2_le}
  \rocqok
  \uses{def:alice_trace_script,def:indcpa_assumption,def:claim,def:loss}
  The script of Definition~\ref{def:alice_trace_script} at the $k$-th instance
  of a sequence, over the dictionary $C^{\mathcal{A}}$ that charges each hop
  the advantage $\mathcal{A}$ assumes in place of the advantage its own
  reduction shows. The three games and the three labels are those of
  \texttt{alice\_claim}.
  ...
  The two class memberships are hypotheses of the script, spent in the
  justification of the hop each one licenses, which is the shape
  Definition~\ref{def:paillier_script} has. The zero-game label loses the same
  unconditional residue as in Definition~\ref{def:alice_trace_script}, so the
  ...
  \[ \mathrm{tot}[\mathsf{cpa\_bob}, \mathsf{cpa\_charlie},
       \mathsf{uniform\_fiber}]
       = \tfrac{1}{m} + 2\,\varepsilon_{\mathrm{cpa}} \]
  The script is read twice along the sequence of instances: at each parameter
  through soundness and \texttt{alice\_admissible\_totalE}, which is
  \texttt{f\_guess\_V2\_le}, and as a sequence of results under
  Theorem~\ref{thm:alice_guess_negligible}.
  Theorem~\ref{thm:alice_guess_admissible} is the first reading at the
  sequence which repeats one instance.
\end{definition}

\begin{definition}[Alice's trace against the simulation at the class epsilon]
  \label{def:alice_sim_script}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_trace_sim_advantage,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_sim_script}
  \rocqok
  \uses{def:alice_trace_sim_script,def:alice_script}
  The script of Definition~\ref{def:alice_trace_sim_script} at the $k$-th
  instance, over the dictionary $C^{\mathcal{A}}$ of
  Definition~\ref{def:alice_script} at the $k$-th test, which charges each of
  the two hops the advantage $\mathcal{A}$ assumes in place of the advantage
  its own reduction shows. The two class memberships are hypotheses of the
  script, spent in the justification of the hop each one licenses.
  \[ \mathrm{Script}_{C^{\mathcal{A}}}\big(\gamma_\tau(D),\
       \Pr_{\Ideal^\tau}[D],\
       [\mathsf{cpa\_bob},\ \mathsf{cpa\_charlie}]\big),
     \qquad \mathrm{tot}(s) = 2\,\varepsilon_{\mathrm{cpa}} \]
  The two $\mathrm{same}$ steps at the ends carry the test between the
  executed trace and the hopping tuple at no loss, and the script stops at the
  simulated trace itself, so the distance between its two games is the
  distance a trace test sees between the executed trace and the simulation,
  and the script bounds that distance by two class-conditional terms and
  nothing else. \texttt{f\_sim\_advantageE} identifies the $k$-th member of the
  distance sequence with that gap.
  \[ \mathrm{adv}^\tau(D)
       = \big\lvert \gamma_\tau(D) - \Pr_{\Ideal^\tau}[D] \big\rvert \]
  So \texttt{f\_sim\_advantageE} with \texttt{hop\_script\_total} gives
  $\mathrm{adv}^\tau(D) \le 2\,\varepsilon_{\mathrm{cpa}}$ at each $k$.
\end{definition}

Each script states a bound at one security parameter, where an asymptotic
notion has nothing to measure. The three statements that close this section
read a script along a sequence of parameters instead, which is the form a
computational claim finally takes. A script indexed by the parameter returns a
sequence of results, and negligibility is a property of that sequence that no
...
\begin{definition}[A dictionary all of whose labels lose a negligible sequence]
  ...
  The condition quantifies over the whole label type, so whatever a script
  over $C$ spends is covered by it. It is attached to the dictionary once, and
  a theorem about any script written at that dictionary then names no label:
  the dictionary carries the per-label statements the theorem would otherwise
  have to list. Recovering the statements from a script is by the name of its
  dictionary, so a dictionary
...
\begin{theorem}[A sequence script has a negligible advantage]
  \label{thm:advantage_negligible}
  \rocq{infotheo.computational_security.epshop_sequence.advantage_negligible}
  \rocqok
  \uses{def:negligible_claims,def:script_result,def:negligible_fun,lem:loss_eval_negligible}
  Let $P$ be a sequence script, a result $P_k$ at every security parameter
  over a dictionary all of whose labels are negligible, and let its loss be the
  same at every parameter.
  ...
  The script supplies the two facts the argument runs on: its advantage is
...
\begin{definition}[The DSDP sequence a scheme sequence builds]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.mk_dsdp_instance,
  ...
  the parties' keys are generated from. From $\mathcal{Q}$ and that data the
  instance at $k$, the sequence of instances, and an asymptotic value for that
  sequence are read off.
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  \rocqok
  \uses{def:negligible_fun,def:dsdp_instance,def:dsdp_instance_sequence,def:dsdp_asymptotic,thm:alice_guess_admissible,def:alice_script,def:negligible_claims,thm:advantage_negligible}
  ...
  It is Theorem~\ref{thm:advantage_negligible} read over the scripts of
  Definition~\ref{def:alice_script}, one at each $k$. Each script spends the
  same three labels, so the loss is the same at every $k$, and the loss of each
  label along the sequence is one of the two fields of
...
\begin{theorem}[The trace simulation distance sequence is negligible]
  ...
  \rocqok
  \uses{def:alice_sim_script,def:dsdp_asymptotic,def:negligible_claims,thm:advantage_negligible}
  ...
  over the scripts of Definition~\ref{def:alice_sim_script}, one at each $k$.
  That script spends the two hop labels alone, so the whole distance is
