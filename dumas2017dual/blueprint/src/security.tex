...
\begin{definition}[One instance of a DSDP family]
  \label{def:dsdp_instance}
  \rocq{infotheo.dumas2017dual.dsdp.fdist_hopping.dsdp_instance_family.dsdp_instance}
  \rocqok
  \uses{def:alice_trace}
  An instance is the scheme, its coin index type with a nonemptiness proof, the coin
  ...
  every concrete trace bound applies at each $k$ unchanged. The real field
...
\begin{theorem}[The trace guessing family is negligible]
  ...
  $\mathcal{A}_k$ and a predictor $g_k$ at each $k$, if both reduction adversaries
  ...
  predictor $\delta$, whose guessing probability is $1$ at every $k$.
...
\begin{corollary}[The hypotheses eventually reject the decrypting family]
  ...
  the classifier at each such $k$ answers false on the reduction adversary $\delta$
...
\begin{corollary}[The bound along a Paillier family]
  ...
  once $k \mapsto 1/(p_k q_k)$ is negligible and the assumed per-$k$
  advantages are.
...
\begin{corollary}[The bound along a Benaloh family]
  ...
  once $k \mapsto 1/r_k$ is negligible and the assumed per-$k$ advantages are.
