...
game vocabulary of \texttt{computational\_security/}, collected as a field of
the results record \texttt{dsdp\_security}, and read off at the three settings
of \texttt{dsdp\_main.v}. Each chapter below takes one of them and chains it
...
\begin{definition}[The distinguisher of a predictor]
  ...
  \rocq{infotheo.computational_security.indcpa_game.distinguisher_of_predictor,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.guess_V2_acceptE}
  \rocqok
...
\begin{lemma}[The all-zero endpoint is the simulator]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_spectator_law,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_idealE}
  \rocqok
...
\end{theorem}
\begin{proof}
  \uses{lem:simulator_factorization,def:alice_hops}
  \rocqok
  The ideal-world law is the hop-$2$ law, so the gap is
  $\lvert \gamma_0(D) - \gamma_2(D) \rvert$, which is the gap result of the
  fragment of Definition~\ref{def:alice_hops}. The triangle inequality that
  composes the two hop equalities is the one the fragment already carries.
\end{proof}
...
\begin{theorem}[The three bounds at the trace]
  ...
  \rocqok
  \uses{def:dsdp_setting,lem:trace_of_hop_tuple,def:alice_trace_chain,def:alice_trace_sim_chain,thm:alice_guess_real,thm:alice_unpredictability,thm:alice_simulation_secure}
  ...
  The guessing bound is the soundness field of
  Definition~\ref{def:alice_trace_chain} and the simulation bound the gap
  result of Definition~\ref{def:alice_trace_sim_chain}.
...
each hop costs the advantage of one reduction. This chapter adds the two
...
\begin{lemma}[A finite sum of negligible sequences is negligible]
  ...
  chain totals one cost per label of its loss, so a bound read off such a
  total is negligible as soon as each label's cost sequence is.
\end{lemma}
...
\begin{theorem}[The class-conditional trace bound]
  ...
  \rocqok
  \uses{def:dsdp_setting,thm:alice_trace_bounds,def:indcpa_assumption,def:alice_trace_chain_assumed}
  ...
  Definition~\ref{def:alice_trace_chain_assumed}, which spends the two
...
\begin{theorem}[Paillier's real-or-zero advantage under composite residuosity]
  ...
  what they cost: one call to $\mathcal{B}$ carries the real arm from
...
\begin{corollary}[The bound at the Paillier scheme]
  ...
  \[ \Pr[\,g(\mathrm{AliceTrace}) = V_2\,]
       \le \tfrac{1}{pq} + 4\,\varepsilon_{\mathrm{dcr}} \]
  ...
  decisional composite residuosity and nothing further, at the cost
...
costs. The three programs of the section are the Paillier hybrid, the Benaloh
...
\begin{itemize}
  ...
  \item \texttt{hop l e to g' by H} invokes the assumption named $l$, loses
    ...
    source game, the target and the cost through its claim, and $e$, $g'$ and
    $H$ are each checked against it.
  \item \texttt{same to g' by H} rewrites the game to $g'$ at no loss,
    guaranteed by $H : \mathrm{current} = g'$.
  \item \texttt{s ;; plus l c by H} adds an external term to the loss: the
    last game is at most $c$, a term labelled $l$, guaranteed by
    $H : \mathrm{current} \le c$, so the first game is at most the total, the
    hops' losses plus $c$.
  \item \texttt{s ;; bound c by H} returns the bound: the total of the loss is
    $c$, guaranteed by $H : \mathrm{total} = c$, so the first game is at most
    $c$. A program reaching it without a \texttt{plus} returns the bound on
    the gap between its first and its last game.
\end{itemize}
The label slot names the assumption invoked and where, the cost slot is that
...
\begin{definition}[A label and the claim it stands for]
  ...
  \rocq{infotheo.computational_security.epshop.claim,
       infotheo.computational_security.epshop.claim_cost,
       infotheo.computational_security.epshop.hop_src,
       infotheo.computational_security.epshop.hop_tgt,
       infotheo.computational_security.epshop.plus_game,
       infotheo.computational_security.epshop.hop_obligation,
       infotheo.computational_security.epshop.plus_obligation}
  \rocqok
  A label is an element of a type the client chooses, and a claim function
  sends it to what it asserts. A hop claim $\mathrm{Hop}(s, t, e)$ names a
  source game, a target game and a cost and asserts that the two games lie
  within that cost of each other; a terminal claim $\mathrm{Plus}(g, e)$ names
  a game and a cost and asserts that the game lies below the cost.
  \[ \mathrm{obl}_{\mathrm{h}}(\mathrm{Hop}(s,t,e))
       = \big(\lvert s - t \rvert \le e\big), \qquad
     \mathrm{obl}_{\mathrm{p}}(\mathrm{Plus}(g,e)) = \big(g \le e\big) \]
  \[ \mathrm{obl}_{\mathrm{h}}(\mathrm{Plus}(g,e)) = \bot, \qquad
     \mathrm{obl}_{\mathrm{p}}(\mathrm{Hop}(s,t,e)) = \bot \]
  The cost of either claim is $e$, and the source, the target and the game are
  read off by projections returning $0$ on the other constructor. The claim is
  the whole content of a label: it is what a step written under that label is
  checked against, so a step cannot record a call to an assumption it did not
  make, and a label meant for the terminal statement cannot be spent on a hop.
\end{definition}
...
\begin{definition}[A loss and the labels it is a list of]
  ...
  $\mathrm{ev}$ sums the cost each label's claim names, a monoid morphism to
  the additive reals.
  \[ \mathrm{ev}(s) = \sum_{l \in s} \mathrm{cost}(l), \qquad
     \mathrm{ev}[\,] = 0, \qquad \mathrm{ev}[l] = \mathrm{cost}(l) \]
  ...
  cost, and the two agree. On a list of literal labels the fold reduces to a
  left-associated sum of the costs those labels claim, with no summation and
  no list left in it.
  \[ \mathrm{tot}(s) = \mathrm{ev}(s), \qquad
     \mathrm{tot}[l_1, l_2, l_3]
       = \big(\mathrm{cost}(l_1) + \mathrm{cost}(l_2)\big) + \mathrm{cost}(l_3) \]
...
\begin{definition}[A chain and the four statements that build one]
  ...
  \rocqok
  \uses{def:loss}
  A chain is a first endpoint $a$, a current endpoint $b$, a loss $s$, and a
  ...
  \[ \mathrm{start}(g) = \big(g,\; g,\; [\,]\big), \qquad
     \mathrm{hop}(l, e, g', H) = \big(\mathrm{src}(l),\; g',\; [l]\big)
       \quad\text{for } H : \mathrm{obl}_{\mathrm{h}}(l) \]
  ...
  $e = \mathrm{cost}(l)$ and $g' = \mathrm{tgt}(l)$, each discharged by
  reflexivity, so the cost and the target written at a step are the cost and
  ...
  equality of advantages rather than an assumed bound is justified by. In
...
\begin{definition}[What a program returns]
  ...
  \rocq{infotheo.computational_security.epshop.chain_result,
       ...
       infotheo.computational_security.epshop.chain_plus,
       infotheo.computational_security.epshop.chain_bound}
  \rocqok
  \uses{def:chain}
  A result is a quantity $a$, a loss $s$, a bound $c$, a proof that $c$ bounds
  $a$, and a proof that $c$ is the total of $s$.
  \[ a \le c, \qquad \mathrm{tot}(s) = c \]
  A chain returns one on its own, on the gap between its endpoints, at the
  total of its loss; $\mathrm{plus}$ returns one on the first endpoint, having
  added the external term $c$ under the label $l$ for
  $H : \mathrm{obl}_{\mathrm{p}}(l)$ together with the two equations
  $c = \mathrm{cost}(l)$ and $\mathrm{game}(l) = b_m$, so the term written and
  the game the chain stopped at are both what the label claims; and
  $\mathrm{bound}$ republishes a result at an explicit $c$ for
  $H : \mathrm{tot}(s) = c$.
  \[ m = \big(\lvert a_m - b_m \rvert,\; s_m,\; \mathrm{tot}(s_m)\big),
     \qquad
     \mathrm{plus}(m, l, c, H) = \big(a_m,\;
       s_m \mathbin{+\!\!+} [l],\;
       \mathrm{tot}(s_m \mathbin{+\!\!+} [l])\big) \]
  \[ \mathrm{bound}(b, c, H) = \big(a_b,\; s_b,\; c\big) \]
  $\mathrm{plus}$ is where a statement changes sort, from a comparison of two
  acceptance probabilities to a bound on one. A secrecy theorem has the second
  shape, so a hybrid reaches its published form there, and the terminal label
  $l$ is the one summand of such a bound that comes from the endpoint rather
  than from a step. $\mathrm{bound}$ is where the published number is written
  ...
  Both are written after the double semicolon because they return a result
...
\begin{lemma}[The category laws, as equalities of records]
  ...
  costs nothing classical and the three laws are equalities rather than
...
\begin{definition}[The two ciphertext replacements as one fragment]
  \label{def:alice_hops}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_hops,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_hops_assumed}
  \rocqok
  \uses{def:chain,def:claim,def:loss,lem:alice_view_advantage,def:indcpa_assumption}
  The two ciphertext replacements as one chain over $\gamma_0(D)$,
  $\gamma_1(D)$ and $\gamma_2(D)$, carrying no terminal statement. Its loss is
  the two labels $\mathsf{cpa\_bob}$ and $\mathsf{cpa\_charlie}$, and the gap
  result it returns on its own bounds the distance between the real hopping
  tuple and the all-zero one, which is the simulation bound.
  \[ \mathrm{tot}(s) = \epshopsum{D} \]
  Each hop is justified by Lemma~\ref{lem:alice_view_advantage}, which makes
  its gap exactly the advantage it logs, read as an inequality through
  \texttt{le\_of\_eq}. \texttt{alice\_hops\_assumed} is the same fragment over
  the dictionary $C^{\mathcal{A}}$ that charges each hop the advantage
  $\mathcal{A}$ of Definition~\ref{def:indcpa_assumption} assumes, justified
  by the class membership of that hop's reduction, so its two memberships are
  parameters spent inside the fragment.
  \[ \mathrm{tot}(s) = 2\,\varepsilon_{\mathrm{cpa}} \]
  Both fragments enter a longer program through a let-bound name, an
  application not being a statement of the entry.
\end{definition}

\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  \label{def:alice_chain}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_label,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_claim,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.all_zero_game_V2_le_invm,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_chain,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_totalE}
  \rocqok
  \uses{def:chain,def:chain_result,def:claim,def:loss,def:alice_hops,lem:alice_view_advantage,lem:guess_all_zero,thm:alice_guess_real}
  Alice's ladder as a chain over $\gamma_0(D_g), \gamma_1(D_g), \gamma_2(D_g)$,
  the acceptance probabilities of a predictor's distinguisher at the three
  hops, joined by the fragment of Definition~\ref{def:alice_hops}, which enters
  the program through a let-bound name. Each hop logs the advantage
  of the reduction its label names, at Bob's key and then at Charlie's, and the
  terminal statement bounds the all-zero endpoint under a third label, so the
  program is a chain bound on $\gamma_0(D_g)$. \texttt{alice\_claim} fixes what
  each of the three labels claims: the games a hop label joins and the
  advantage it costs, and the all-zero game the terminal label bounds.
  \[ \mathrm{tot}(s) = \tfrac{1}{m} + \epshopsum{D_g} \]
  ...
  This loss has two kinds of term and the labels separate them. The two hop terms
  are the advantages of the reductions $\hopred{0}{D_g}$ and $\hopred{1}{D_g}$,
  each of which becomes conditional on a computational assumption only where a
  later statement requires it to be small. The terminal term
  $\mathsf{uniform\_plain}$ is information-theoretic and unconditional, the
  residue the leaked output leaves along the solution fiber. The two hops are
  those of Definition~\ref{def:alice_hops}, justified by
  Lemma~\ref{lem:alice_view_advantage}, which makes each gap
  exactly the term it logs, read as an inequality through
  \texttt{le\_of\_eq}; the terminal statement is justified by
  \texttt{all\_zero\_game\_V2\_le\_invm}, which is
  Lemma~\ref{lem:guess_all_zero} carried to the game form and is the one place
  the fiber enters. Theorem~\ref{thm:alice_guess_real} states the bound this
  program returns and is its soundness field.
\end{definition}

\begin{definition}[Alice's trace argument as a chain opening at the protocol]
  \label{def:alice_trace_chain}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.accept_trace_tupleE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_chain}
  \rocqok
  \uses{def:alice_chain,def:alice_hops,def:alice_trace,lem:trace_of_hop_tuple}
  The chain of Definition~\ref{def:alice_chain} preceded by one further game.
  ...
  $\gamma_\tau(D)$ the probability that $D$ accepts a value sampled from it, so
  the object the argument starts from is the executed protocol and not the
  tuple $\hoptuple{0}$.
  ...
  $\tau$, so a test on the trace accepts as often as its lift on the tuple.
  The two hops are the fragment of Definition~\ref{def:alice_hops} at the
  lifted test $D_{g \circ \tau}$, entering through a let-bound name, and the
  terminal statement is that of Definition~\ref{def:alice_chain}, so
  the loss and the bound are the same three terms.
  \[ \mathrm{tot}(s) = \tfrac{1}{m} + \epshopsum{D_{g \circ \tau}} \]
  The trace bound of Theorem~\ref{thm:alice_trace_bounds} is the soundness
  field of this result.
\end{definition}

\begin{definition}[Alice's trace against the simulation as one chain]
  \label{def:alice_trace_sim_chain}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.accept_trace_ideal_tupleE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_chain}
  \rocqok
  \uses{def:alice_hops,def:alice_trace_chain,def:alice_trace,lem:trace_of_hop_tuple,lem:simulator_factorization}
  The fragment of Definition~\ref{def:alice_hops} with one step at each end
  that adds nothing to the loss. The chain opens at $\gamma_\tau(D)$, runs the
...
\begin{definition}[Alice's trace argument at the assumed advantage]
  \label{def:alice_trace_chain_assumed}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_claim_assumed,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_assumed_totalE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_chain_assumed}
  \rocqok
  \uses{def:alice_trace_chain,def:alice_hops,def:indcpa_assumption,def:claim,def:loss}
  ...
  The two class memberships are parameters of the program and are spent inside
  the fragment \texttt{alice\_hops\_assumed} of
  Definition~\ref{def:alice_hops}, one in the justification of each hop, which
  is the shape Definition~\ref{def:paillier_chain} has. The terminal label costs the same
  unconditional residue as in Definition~\ref{def:alice_trace_chain}, so the
  total adds one unconditional term to two assumption-conditional ones, and
  \texttt{alice\_assumed\_totalE} is the reordering that states it.
  \[ \mathrm{tot}(s) = \tfrac{1}{m} + 2\,\varepsilon_{\mathrm{cpa}} \]
  Theorem~\ref{thm:alice_guess_admissible} is the soundness field of this
  result at its two class memberships.
\end{definition}

\begin{definition}[Alice's trace against the simulation at the assumed advantage]
  \label{def:alice_trace_sim_chain_assumed}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_advantage,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_chain_assumed,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_advantage_firstE}
  \rocqok
  \uses{def:alice_trace_sim_chain,def:alice_trace_chain_assumed}
  The chain of Definition~\ref{def:alice_trace_sim_chain} over the dictionary
  $C^{\mathcal{A}}$ of Definition~\ref{def:alice_trace_chain_assumed}, which
  ...
  parameters of the program and are spent inside the fragment
  \texttt{alice\_hops\_assumed}, one in the justification of each hop.
  \[ \mathrm{loss}(s) = [\mathsf{cpa\_bob},\ \mathsf{cpa\_charlie}],
  ...
  distance by two assumption-conditional terms and nothing else.
  \[ \mathrm{adv}^\tau(D)
       = \big\lvert \gamma_\tau(D) - \Pr_{\Ideal^\tau}[D] \big\rvert
       = a_s, \qquad
...
\begin{definition}[A dictionary all of whose labels cost a negligible sequence]
  \label{def:negligible_claims}
  \rocq{infotheo.computational_security.epshop_family.negligibleClaims}
  \rocqok
  \uses{def:claim,def:negligible_fun}
  A sequence of dictionaries $C_k$ indexed by the security parameter, together
  with the negligibility of the cost of every label of the label type, read
  along that parameter.
  \[ \forall l,\ \mathrm{negl}\big(k \mapsto \mathrm{cost}(C_k(l))\big) \]
...
\begin{lemma}[The total of a label list along the security parameter]
  ...
  \rocq{infotheo.computational_security.epshop_family.loss_eval_negligible}
  \rocqok
...
\begin{theorem}[A sequence of programs opens at a negligible quantity]
  \label{thm:first_negligible}
  \rocq{infotheo.computational_security.epshop_family.first_negligible}
  \rocqok
  ...
  Then any $f$ identified with the game $P$ opens at is negligible.
  \[ \big(\forall k,\ s_{P_k} = s_{P_0}\big) \implies
     \big(\forall k,\ f(k) = a_{P_k}\big) \implies \mathrm{negl}(f) \]
  The program supplies the two facts the argument runs on: its first game is
...
\begin{definition}[A sequence of DSDP instances]
  ...
  The two facts are the two terms of the class-conditional bound, and the
  ...
  measured in the plaintext space alone, and it holds against an adversary of any
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  \rocqok
  \uses{def:negligible_fun,def:dsdp_instance,def:dsdp_instance_sequence,thm:alice_guess_admissible,def:alice_trace_chain_assumed,def:negligible_claims,thm:first_negligible}
  ...
  It is Theorem~\ref{thm:first_negligible} read over the program of
  Definition~\ref{def:alice_trace_chain_assumed} at the $k$-th instance. That
  program spends the same three labels at every $k$, and the cost of each of
  them along the sequence is one of the two negligibility fields of
  $\mathcal{Q}$: both hop labels cost $\mathrm{negl}(k \mapsto
  \varepsilon_{\mathrm{cpa},k})$ and the terminal label costs
  ...
  same two fields of $\mathcal{Q}$ eventually reject $\delta$'s reduction
  adversary.
\end{theorem}
...
\begin{theorem}[The trace simulation distance sequence is negligible]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance_sequence.alice_trace_sim_advantage_negligible}
  \rocqok
  \uses{def:alice_trace_sim_chain_assumed,def:negligible_claims,thm:first_negligible}
  Along a sequence $\mathcal{Q}$ of DSDP instances and a family $D_k$ of tests
  ...
  parameter is free to grow. It is Theorem~\ref{thm:first_negligible} read
  over the program of Definition~\ref{def:alice_trace_sim_chain_assumed} at
...
\begin{definition}[What a setting proves]
  ...
  value measures one execution against one named adversary and says nothing
  about repeated queries or about a wider class.
\end{definition}
...
\begin{definition}[The Paillier setting]
  ...
  $\mathcal{B}_k$, so every $\varepsilon_{\mathrm{cpa}}$ read off at this
  setting is $2\varepsilon_{\mathrm{dcr},k}$.
\end{definition}
...
\begin{corollary}[The twenty-six at the Paillier setting]
  ...
  class-conditional trace bounds are restated in residuosity epsilons.
  \[ \mathrm{negl}\big(k \mapsto
       \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]\big), \qquad
     \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]
       \le \tfrac{1}{p_k q_k} + 4\,\varepsilon_{\mathrm{dcr},k} \]
  ...
  $\mathcal{B}_k$, and its advantage with $2\varepsilon_{\mathrm{dcr},k}$, both
...
\begin{definition}[The Benaloh setting]
  ...
  so every $\varepsilon_{\mathrm{cpa}}$ read off at this setting is
  $2\varepsilon_{\mathrm{res},k}$.
\end{definition}
...
\begin{corollary}[The twenty-six at the Benaloh setting]
  ...
  restated in residuosity epsilons.
  \[ \mathrm{negl}\big(k \mapsto
       \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]\big), \qquad
     \Pr[\,g_k(\mathrm{AliceTrace}_k) = V_2\,]
       \le \tfrac{1}{r_k} + 4\,\varepsilon_{\mathrm{res},k} \]
  ...
  assumption-conditional field is read at with the one derived from
  $\mathcal{B}_k$, and its advantage with $2\varepsilon_{\mathrm{res},k}$, both
