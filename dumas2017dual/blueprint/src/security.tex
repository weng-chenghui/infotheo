...
\begin{theorem}[The class-conditional trace bound]
  ...
  \rocqok
  \uses{def:dsdp_setting,thm:alice_trace_bounds,def:indcpa_assumption,def:alice_trace_chain_admissible}
...
\begin{definition}[The two ciphertext replacements as one fragment]
  ...
  \texttt{le\_of\_eq}. The fragment carries the simulation bound of
  Theorem~\ref{thm:alice_simulation_secure} and nothing else; the five
  programs below write their two hops where they take them.
\end{definition}
...
\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  ...
  hops, written where they are taken and spanning the same two gaps as the
  fragment of Definition~\ref{def:alice_hops}. Each hop logs the advantage
  ...
  $\mathsf{uniform\_fiber}$ is information-theoretic and unconditional, the
...
\begin{definition}[Alice's trace argument as a chain opening at the protocol]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.accept_trace_tupleE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.hop_tuple_distinguisher,
  ...
  The two hops span the same two gaps as the fragment of
  Definition~\ref{def:alice_hops} at the lifted test $D_{g \circ \tau}$, which
  is \texttt{hop\_tuple\_distinguisher} applied once under a local name, and
  the step to the zero game and the terminal statement are those of
...
\begin{definition}[Alice's trace against the simulation as one chain]
  ...
  The two hops of Definition~\ref{def:alice_hops} with one step at each end
...
\begin{definition}[Alice's trace argument at the class epsilon]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_claim_admissible,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_admissible_totalE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_chain_admissible}
  \rocqok
  ...
  The two class memberships are variables of the program's section and are
  spent in the justification of the hop each one licenses, which is the shape
  Definition~\ref{def:paillier_chain} has. The zero-game label costs the same
...
\begin{definition}[Alice's trace against the simulation at the class epsilon]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_advantage,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_sim_chain_admissible,
  ...
  variables of the program's section and are spent in the justification of the
  hop each one licenses.
  \[ \mathrm{loss}(s) = [\mathsf{cpa\_bob},\ \mathsf{cpa\_charlie}],
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  \rocqok
  \uses{def:negligible_fun,def:dsdp_instance,def:dsdp_instance_sequence,thm:alice_guess_admissible,def:alice_trace_chain_admissible,def:negligible_claims,thm:advantage_negligible}
...
\begin{theorem}[The trace simulation distance sequence is negligible]
  ...
  \rocqok
  \uses{def:alice_trace_sim_chain_admissible,def:negligible_claims,thm:advantage_negligible}
