...
a list of labelled terms rather than a single real, so a finished derivation
carries the assumptions its bound rests on alongside their numeric total. epsHop
...
costs. The three programs at the end of the section are the Paillier hybrid, the
Benaloh hybrid, and Alice's two-hop ladder, each written in it.
...
\begin{itemize}
  ...
  \item \texttt{hop l e to g' by H} invokes the assumption named $l$, loses
    ...
    $H : \lvert \mathrm{current} - g' \rvert \le e$.
...
exact. Each proof slot is mandatory: a statement is ill-formed without it, and a
proof about the wrong pair of games neither convinces nor names a step, its type
being what fixes the game the step departs from. So a chain that exists has no
pending obligation, and a bound is read off it by projection.

\begin{definition}[A loss and the terms it is a list of]
  \label{def:loss}
  \rocq{infotheo.computational_security.epshop.loss_term,
       infotheo.computational_security.epshop.loss_term_cost,
       infotheo.computational_security.epshop.loss,
...
\begin{definition}[A chain and the four statements that build one]
  ...
  \[ \mathrm{start}(g) = \big(g,\; g,\; [\,]\big), \qquad
     \mathrm{hop}(l, e, g', H) = \big(x,\; g',\; [(l, e)]\big)
       \quad\text{for } H : \lvert x - g'\rvert \le e \]
  ...
  rests on. $\mathrm{same}$ is the step an information-theoretic identity
  between two games takes, and it leaves the loss as it stands;
  \texttt{le\_of\_eq} turns such an identity into the inequality a
  $\mathrm{hop}$ asks for, which is what a hop whose gap is an equality of
  advantages rather than an assumed bound is justified by. In $\mathrm{hop}$
  and $\mathrm{same}$ the source endpoint $x$ is not written: it is read off
  the type of the justification. Composition $\mathrm{then}$ is partial, sound
...
\begin{definition}[A bound read off a chain]
  ...
  \rocq{infotheo.computational_security.epshop.chain_bound,
       infotheo.computational_security.epshop.chain_plus}
  \rocqok
  \uses{def:chain}
  A chain bound is a first endpoint $a$, a loss $s$, and a proof that the total
  of the loss bounds $a$ itself.
  \[ a \le \mathrm{ev}(s) \]
  $\mathrm{plus}(m, l, c, H_c)$ builds one from a chain $m$ by adding the
  external term $c$, labelled $l$, to its loss, for $H_c : b_m \le c$.
  \[ \mathrm{plus}(m, l, c, H_c) = \big(a_m,\;
       s_m \mathbin{+\!\!+} [(l, c)]\big) \]
  ...
  a step. It is the \texttt{plus} statement of a program, written after the
  double semicolon because it returns a chain bound where every other statement
  returns a chain.
\end{definition}
...
\begin{definition}[The Paillier hybrid as a chain]
  ...
  \rocq{infotheo.computational_security.paillier_indcpa_scheme.dcr_g,
       infotheo.computational_security.paillier_indcpa_scheme.dcr_0,
  ...
  arm. Its loss is two terms, labelled $\mathsf{dcr\_g}$ and $\mathsf{dcr\_0}$
  after the reduction each hop invokes the assumption through.
  \[ \mathrm{ev}(s) = 2\,\varepsilon_{\mathrm{dcr}} \]
  ...
  Theorem~\ref{thm:paillier_dcr_epsilon_le} is $\mathrm{ev}(s)$ and its two
  hypotheses are those two memberships, so that theorem is the soundness field
  of this chain rather than a triangle inequality of its own.
\end{definition}

\begin{definition}[The Benaloh hybrid as a chain]
  \label{def:benaloh_chain}
  \rocq{infotheo.computational_security.benaloh_indcpa_scheme.residuosity_y,
       infotheo.computational_security.benaloh_indcpa_scheme.residuosity_0,
  ...
  labels $\mathsf{residuosity\_y}$ and $\mathsf{residuosity\_0}$ and the same
  two-term loss.
  \[ \mathrm{ev}(s) = 2\,\varepsilon_{\mathrm{res}} \]
  ...
  the soundness field of this chain at its two class memberships.
\end{definition}

\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  \label{def:alice_chain}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.cpa_bob,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.cpa_charlie,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.uniform_plain,
  ...
  program is a chain bound on $\gamma_0(D_g)$.
  ...
  the fiber enters. Theorem~\ref{thm:alice_guess_real} states $\mathrm{ev}(s)$
  and is the soundness field of this chain bound.
\end{definition}
