...
\begin{lemma}[Decoding Alice's trace at her own key]
  ...
  The encoding therefore costs Alice nothing and withholds nothing from her: a
...
\begin{theorem}[Guessing from the raw interpreter trace]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_raw_trace_guess_V2_le}
  \rocqok
  \uses{def:dsdp_setting,thm:alice_trace_bounds,lem:raw_trace_decode}
  ...
  Proved in \texttt{dsdp\_alice\_trace\_link.v}, the bound is field
  \texttt{raw\_trace\_guess\_V2\_le} of \texttt{dsdp\_security}, read off at
  each instance of \texttt{dsdp\_main.v}.
\end{theorem}

\begin{theorem}[Simulation at the raw trace, averaged over the coin]
  \label{thm:raw_trace_simulation}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_raw_trace_sim_advantage_avg_le}
  \rocqok
  \uses{def:dsdp_setting,thm:alice_trace_bounds,lem:raw_trace_decode}
  ...
  Proved in \texttt{dsdp\_alice\_trace\_link.v}, the bound is field
  \texttt{raw\_trace\_sim\_advantage\_avg\_le} of \texttt{dsdp\_security}, read
...
\begin{definition}[The asymptotic content of a sequence]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance_sequence.dsdp_asymptotic}
  \rocqok
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  $\mathcal{N}$: both hop labels cost $\mathrm{negl}(k \mapsto
...
\begin{definition}[The setting a DSDP security statement is made over]
  ...
  Read with the unconditional field of an asymptotic value for $\mathcal{Q}$,
  the link field
...
\begin{definition}[What a setting proves]
  ...
  \rocqok
  \uses{def:dsdp_setting,def:indcpa_assumption,def:dsdp_asymptotic,def:negligible_fun}
  A value of the results record at a setting $\mathcal{X}$ is the twenty-one
  statements of the two axes read at $\mathcal{X}$, each with one proof, and
  they are the results the paper presents and no others. Six are counting
  statements about the relay pair and the two relay views. Three are at
  Alice's hopping tuple: the guessing bound, the simulation bound, and the
  residue a predictor is left with at the all-zero endpoint. Two are the
  entropy equalities at the leaked output and at that endpoint. Three are at
  Alice's trace: the guessing bound and the coin-averaged simulation bound at
  the raw interpreter trace, and the zero conditional entropy the encoded
  trace leaves. Two are the class-conditional trace bound at the two
  spellings of its unconditional term. Four are the obstructions that force
  that class restriction, the last of them along the whole sequence. One is
  negligibility along $\mathcal{X}$'s sequence, read at an asymptotic value
  supplied with that statement rather than carried by $\mathcal{X}$: the
  nineteen before the two asymptotic ones are made at one security parameter
  and do not read it.
  A value asserts nothing wider than those twenty-one. Six of them are
  ...
  trace each determine Bob's input, and the four decrypting-predictor
  statements show that the class restriction cannot be widened. Next to the
  field reading $\HH(V_2 \mid \hoptuple{2}) = \log m_k$ this looks
  contradictory until the conditioner of each is named, the all-zero endpoint
  of the hop ladder on one side and the executed trace on the other.
  \[ \HH(V_2 \mid \mathrm{AliceTrace}) = 0, \qquad
     \HH(V_2 \mid \hoptuple{2}) = \log m_k \]
  The two trace-axis bounds are read at the raw trace the run of the protocol
  produces rather than at the encoded trace it is carried through to reach a
  finite carrier. Decoding at Alice's own key inverts that encoding, so the
  two levels are one observation and the same two advantages pay for both.
...
\begin{theorem}[Every setting proves them]
  ...
  \rocqok
  \uses{def:dsdp_security,def:dsdp_setting,thm:dsdp_centropy_uniform,thm:US_e1_centropy_V2_eq0,thm:bob_privacy_V1,thm:bob_privacy_V3,thm:charlie_privacy_V2,thm:alice_guess_real,thm:alice_unpredictability,cor:alice_guess_view,thm:alice_simulation_secure,thm:centropy_endpoints,thm:alice_trace_bounds,thm:raw_trace_guess,thm:raw_trace_simulation,lem:guess_all_zero,thm:centropy_trace_tuple,thm:centropy_trace_eq0,thm:alice_guess_admissible,thm:decrypt_lower_bound,cor:decrypt_not_admissible,cor:admissible_premise_free,cor:decrypt_sequence_not_admissible,thm:alice_guess_negligible}
  Every setting proves the twenty-one, after du2002's pair
  \texttt{scalar\_product\_is\_leakage\_free} and
  \texttt{scalar\_product\_is\_leakage\_freeP}.
  \[ \forall \mathcal{X},\ \text{the twenty-one statements hold at }
     \mathcal{X} \]
...
\begin{definition}[The Paillier setting]
  ...
  ring, the three private keys, the two encryption coins, and one decisional
  composite residuosity record $\mathcal{B}_k$ at each $k$. The two
  negligibility facts stay outside the setting and enter with the one
  asymptotic statement read at it. The counting side is the uniform law on five
  ...
  assumed on the computational side is the sequence $\mathcal{B}_k$; the
  IND-CPA assumption at $k$ is the value
...
\begin{corollary}[The twenty-one at the Paillier setting]
  ...
  \rocqok
  \uses{def:paillier_setting,thm:dsdp_securityP,cor:alice_guess_paillier,thm:alice_guess_negligible,def:dsdp_asymptotic,def:dcr_assumption,def:paillier_indcpa_assumption}
  All twenty-one statements hold at the Paillier setting, each written out at
...
\begin{definition}[The Benaloh setting]
  ...
  modulus $n_k$ and the exponent $r_k$; the two negligibility facts stay
  outside the setting, as at Paillier, and
...
\begin{corollary}[The twenty-one at the Benaloh setting]
  ...
  \rocqok
  \uses{def:benaloh_setting,thm:dsdp_securityP,thm:alice_guess_negligible,def:dsdp_asymptotic,def:benaloh_residuosity_assumption,def:benaloh_indcpa_assumption}
  All twenty-one statements hold at the Benaloh setting, and the asymptotic one
...
\begin{corollary}[The twenty-one at the idealized setting]
  ...
  All twenty-one statements hold at the idealized setting, which takes no
