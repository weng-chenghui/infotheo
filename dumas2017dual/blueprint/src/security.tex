...
\begin{definition}[The IND-CPA assumption of Paillier, derived]
  ...
  $\varepsilon_{\mathrm{cpa}}$ off this value, so each is measured in
...
\section{The chain a multi-hop bound is assembled in}
...
section states the layer they are written in. A chain is a morphism between
acceptance probabilities graded by an accumulated loss, and that loss is a list
of labelled terms rather than a single real, so a finished derivation carries
the assumptions its bound rests on alongside their numeric total. The three
programs at the end of the section are the Paillier hybrid, the Benaloh hybrid,
and Alice's two-hop ladder, each written in this layer.

\begin{definition}[A loss and the terms it is a list of]
  \label{def:loss}
  \rocq{infotheo.computational_security.hop_chain.loss_term,
       infotheo.computational_security.hop_chain.loss_term_cost,
       infotheo.computational_security.hop_chain.loss,
       infotheo.computational_security.hop_chain.loss_eval,
       infotheo.computational_security.hop_chain.loss_eval_nil,
       infotheo.computational_security.hop_chain.loss_eval1,
       infotheo.computational_security.hop_chain.loss_eval_cat}
  \rocqok
  A loss term is a label $l$, drawn from a type the client chooses, paired with
  a real $e$. A loss is a list of loss terms, the free monoid on them, and its
  total $\mathrm{ev}$ is the sum of the reals, a monoid morphism to the
  additive reals.
  \[ \mathrm{ev}(s) = \sum_{(l,\,e)\,\in\,s} e, \qquad
     \mathrm{ev}[\,] = 0, \qquad \mathrm{ev}\big[(l, e)\big] = e, \qquad
...
\begin{definition}[A chain and the four statements that build one]
  \label{def:chain}
  \rocq{infotheo.computational_security.hop_chain.chain,
       infotheo.computational_security.hop_chain.chain_start,
       infotheo.computational_security.hop_chain.chain_hop,
       infotheo.computational_security.hop_chain.chain_eq,
       infotheo.computational_security.hop_chain.chain_bind}
  \rocqok
  \uses{def:loss}
  A chain is a first endpoint $a$, a current endpoint $b$, a loss $s$, a
  premise $P$, and a proof that $P$ bounds the distance between the endpoints
  by the total of the loss. The endpoints are acceptance probabilities.
  \[ P \Longrightarrow \lvert a - b \rvert \le \mathrm{ev}(s) \]
  Four statements build one, each re-establishing that invariant, and are
  written below as the four data $(a, b, s, P)$ they carry.
  \[ \mathrm{start}(g) = \big(g,\; g,\; [\,],\; \mathrm{True}\big), \qquad
     \mathrm{hop}(l, e, x, g')
       = \big(x,\; g',\; [(l, e)],\; \lvert x - g'\rvert \le e\big) \]
  \[ \mathrm{eq}(x, g') = \big(x,\; g',\; [\,],\; x = g'\big), \qquad
     \mathrm{bind}(m, f) = \big(a_m,\; b_{f(b_m)},\;
       s_m \mathbin{+\!\!+} s_{f(b_m)},\; P_m \wedge P_{f(b_m)}\big) \]
  ...
  rests on. $\mathrm{eq}$ is the step an information-theoretic identity between
  two games takes, and it leaves the loss as it stands. $\mathrm{bind}$ is
  sound only under the side condition $a_{f(b_m)} = b_m$, that the continuation
  starts where the previous fragment stopped, and it is where the one triangle
  inequality of the development is discharged, once, so that a chain of any
  length needs none. Without that side condition the concatenated loss would
  bound nothing about the two outer endpoints. Composition has a notation of
  its own, in a scope of its own, whose tokens differ from those of the
  distribution bind of \texttt{indcpa\_game.v}, so both may be open at once.
\end{definition}

\begin{definition}[A bound read off a chain]
  \label{def:chain_bound}
  \rocq{infotheo.computational_security.hop_chain.chain_bound,
       infotheo.computational_security.hop_chain.chain_eval}
  \rocqok
  \uses{def:chain}
  A chain bound is a first endpoint $a$, a loss $s$, a premise $Q$, and a proof
  that $Q$ bounds $a$ itself by the total of the loss.
  \[ Q \Longrightarrow a \le \mathrm{ev}(s) \]
  $\mathrm{eval}(m, l, c)$ builds one from a chain $m$ by charging its current
  endpoint at $c$ under the label $l$.
  \[ \mathrm{eval}(m, l, c) = \big(a_m,\;
       s_m \mathbin{+\!\!+} [(l, c)],\; P_m \wedge b_m \le c\big) \]
  This is where a statement changes sort, from a comparison of two acceptance
  probabilities to a bound on one. A secrecy theorem has the second shape, so
  a hybrid reaches its published form here, and the terminal term $(l, c)$ is
  the one summand of such a bound that comes from the endpoint rather than from
  a step.
\end{definition}

\begin{lemma}[The laws, up to an equivalence the total is invariant under]
  \label{lem:chain_equiv}
  \rocq{infotheo.computational_security.hop_chain.chain_equiv,
       infotheo.computational_security.hop_chain.chain_equiv_loss_eval,
       infotheo.computational_security.hop_chain.loss_eval_perm,
       infotheo.computational_security.hop_chain.left_unit_equiv,
       infotheo.computational_security.hop_chain.right_unit_equiv,
       infotheo.computational_security.hop_chain.loss_catA,
       infotheo.computational_security.hop_chain.chain_observable_eq}
  \rocqok
  \uses{def:chain}
  Two chains are equivalent when their endpoints agree, their losses are
  permutations of one another, and their premises are equivalent. The total is
  an invariant of that relation, a sum over a list being invariant under
  permutation of the list.
  \[ c_1 \sim c_2 \;\Longrightarrow\;
       \mathrm{ev}(s_{c_1}) = \mathrm{ev}(s_{c_2}) \]
  The two unit laws hold up to $\sim$, and concatenation of losses is
  associative.
  \[ \mathrm{bind}(\mathrm{start}(g), f) \sim f(g), \qquad
     \mathrm{bind}(m, \mathrm{start}) \sim m, \qquad
     s_1 \mathbin{+\!\!+} (s_2 \mathbin{+\!\!+} s_3)
       = (s_1 \mathbin{+\!\!+} s_2) \mathbin{+\!\!+} s_3 \]
  Equality of records is the wrong relation to state the unit laws at. The left
  unit replaces a premise $P$ by $\mathrm{True} \wedge P$, which is equivalent
  to $P$ and not equal to it, and two chains that agree on their four
  observable fields are equal only through proof irrelevance, which is
  \texttt{chain\_observable\_eq} and costs one classical axiom. Stating the
  laws at $\sim$ leaves that axiom unused, and it loses nothing a bound needs:
  what a chain is read for is the total, and the total is what $\sim$ fixes.
  So the regrouping and unit rearrangements the notation performs leave every
  bound of the three programs below unchanged.
\end{lemma}

\begin{definition}[The Paillier hybrid as a chain]
  \label{def:paillier_chain}
  \rocq{infotheo.computational_security.paillier_indcpa_scheme.paillier_multiplying_hop,
       infotheo.computational_security.paillier_indcpa_scheme.paillier_plain_hop,
       infotheo.computational_security.paillier_indcpa_scheme.paillier_chain,
       infotheo.computational_security.paillier_indcpa_scheme.paillier_chain_lossE,
       infotheo.computational_security.paillier_indcpa_scheme.paillier_chain_premise}
  \rocqok
  \uses{def:chain,def:loss,def:dcr_assumption,thm:paillier_dcr_epsilon_le,lem:unit_fdist_translateE}
  ...
  arm. Its loss is two terms, labelled $\mathsf{multiplying}$ and
  $\mathsf{plain}$ after the reduction each hop runs, and its premise is the
  two class memberships and nothing further.
  \[ \mathrm{ev}(s) = 2\,\varepsilon_{\mathrm{dcr}}, \qquad
     P = \mathrm{adm}\big(\mathcal{B}_{\mathrm{mul}}\big) \wedge
         \mathrm{adm}\big(\mathcal{B}_{\mathrm{pl}}\big) \]
  Both terms are conditional on $\mathcal{B}$, so this loss has one currency.
  The number in Theorem~\ref{thm:paillier_dcr_epsilon_le} is $\mathrm{ev}(s)$
  and its two hypotheses are $P$, so that theorem is the soundness field of
  this chain read at those hypotheses rather than a triangle inequality of its
  own.
\end{definition}

\begin{definition}[The Benaloh hybrid as a chain]
  \label{def:benaloh_chain}
  \rocq{infotheo.computational_security.benaloh_indcpa_scheme.benaloh_multiplying_hop,
       infotheo.computational_security.benaloh_indcpa_scheme.benaloh_plain_hop,
       infotheo.computational_security.benaloh_indcpa_scheme.benaloh_chain,
       infotheo.computational_security.benaloh_indcpa_scheme.benaloh_chain_lossE,
       infotheo.computational_security.benaloh_indcpa_scheme.benaloh_chain_premise}
  \rocqok
  \uses{def:chain,def:loss,def:benaloh_residuosity_assumption,thm:benaloh_residuosity_epsilon_le,lem:unit_fdist_translateE}
  The same three steps at $T = \mathbb{Z}_n$ and the exponent $r$, with the
  same two labels and the same two-term loss.
  \[ \mathrm{ev}(s) = 2\,\varepsilon_{\mathrm{res}}, \qquad
     P = \mathrm{adm}\big(\mathcal{B}_{\mathrm{mul}}\big) \wedge
         \mathrm{adm}\big(\mathcal{B}_{\mathrm{pl}}\big) \]
  ...
  $\mathbb{Z}_n$ by its type, so the $\mathrm{eq}$ step there is discharged
  ...
  the soundness field of this chain at $P$.
\end{definition}

\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  \label{def:alice_chain}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_bob_hop,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_charlie_hop,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_plain_size,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_chain,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_chain_premise,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_chain_bound,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_bound_premise,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_chain_lossE}
  \rocqok
  \uses{def:chain,def:chain_bound,def:loss,lem:alice_view_advantage,lem:guess_all_zero,thm:alice_guess_real}
  ...
  hops, joined by the two ciphertext replacements. Each hop logs the advantage
  of the reduction its label names, at Bob's key and then at Charlie's.
  Evaluating the chain at the all-zero endpoint under a third label appends the
  plaintext-space term and yields a chain bound on $\gamma_0(D_g)$.
  \[ \mathrm{ev}(s) = \tfrac{1}{m} + \epshopsum{D_g} \]
  This loss has two currencies and the labels separate them. The two hop terms
  ...
  $\mathsf{plain\_size}$ is information-theoretic and unconditional, the
  residue the leaked output leaves along the solution fiber. The chain's own
  premise holds outright, Lemma~\ref{lem:alice_view_advantage} making each hop
  gap exactly the term it logs, and the bound's premise adds only the endpoint
  bound of Lemma~\ref{lem:guess_all_zero}, which is the one place the fiber
  enters. Theorem~\ref{thm:alice_guess_real} states $\mathrm{ev}(s)$ and is the
  soundness field of this bound at that premise.
\end{definition}
...
\begin{lemma}[The class-conditional bound is negligible as a sequence]
  ...
  \rocq{infotheo.computational_security.negligible.negligible_fun_predictor_bound}
  \rocqok
...
\begin{theorem}[The trace guessing sequence is negligible]
  ...
  1/m_k)$ and $\mathrm{negl}(k \mapsto \varepsilon_{\mathrm{cpa},k})$, are
  ...
  Every currency is hypothesis-conditional, measured at each $k$ as in
