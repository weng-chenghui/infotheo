...
\begin{definition}[Party views]
  ...
  random tape it draws, the ciphertexts it receives, and, for Alice, the
  decrypted output $S$. Alice's view carries her key, her inputs and masks, the
  output $S$, her two combine coins, and the three ciphertext hops. Each
  relay's view carries its key, its own input, the ciphertexts it receives, and
  the two coins it draws.
\end{definition}
...
\begin{theorem}[Residual uncertainty of the relay inputs]
  ...
  free. Those hypotheses are fields of the record
  \texttt{dsdp\_random\_inputs}, which also carries the six coins one run
  draws and their joint independence of the eleven, and which
  \texttt{uniform\_inputs} inhabits at every modulus. Because the
  each-against-the-rest independence reaches Alice's three weights, the
  equality covers honest sampling, where her weights are independent of her
  input and of one another. At every modulus the equality is exact and holds
  against an adversary of any
  running time.
\end{theorem}
\begin{proof}
...
\begin{definition}[The encryption coins of one run]
  \label{def:dsdp_enc_coins}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.dsdp_enc_coins,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.card_dsdp_enc_coins,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.dsdp_enc_coins_fdist}
  \rocqok
  \uses{def:protocol_flow}
  A coin record holds the six coin indices one DSDP execution consumes: the two
  under which Bob and Charlie encrypt their inputs to Alice, the two Alice
  encrypts her combines under, Bob's second-hop coin, and Charlie's
  re-encryption coin. Its law is the uniform distribution on the record.
  \[ c = \big(\rho_2, \rho_3,\ \alpha_1, \alpha_2,\ \beta, \gamma\big),
     \qquad \mathcal{L}(c) = \mathcal{U}\big(\mathrm{Renc}^6\big) \]
  Uniformity of the record says three things at once about the six coins: each
  is uniform, they are independent, and each is fresh where it is drawn. That
  is what a reduction reads when it hands one coordinate to the challenger and
...
\begin{definition}[The corrupted-Alice experiment]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_sampleT,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.EncCoins,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.RB1,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.RC1,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.RB2,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.RC2,
  ...
  \rocqok
  \uses{def:party_views,def:dsdp_enc_coins}
  The experiment is the uniform distribution on the product of five
  coordinates: the two honest inputs $V_2, V_3$, Alice's two mask plaintexts
  $R_2, R_3$, and one coin record of Definition~\ref{def:dsdp_enc_coins}, whose
  six components are read off it as $\rho_2, \rho_3, \alpha_1, \alpha_2, \beta,
  \gamma$.
...
\begin{definition}[The hop ladder]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.bob_real_cipher,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.charlie_reenc_cipher,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.alice_hop_tuple,
  ...
  Alice's tuple carries her two masks, her two combine coins, the output $S$,
  the two ciphertexts she receives from the relays, and Charlie's
  re-encryption of the aggregate under Alice's own key. Three tuples differ
  only in the plaintexts of the two relay ciphertexts.
  \[ \hoptuple{0} = \big(R_2, R_3, \alpha_1, \alpha_2, S,\;
       \Enc(\pk_B, V_2, \rho_2),\; \Enc(\pk_C, V_3, \rho_3),\; C_\gamma\big) \]
  \[ \hoptuple{1} = \big(R_2, R_3, \alpha_1, \alpha_2, S,\;
       \Enc(\pk_B, 0, \rho_2),\; \Enc(\pk_C, V_3, \rho_3),\; C_\gamma\big) \]
  \[ \hoptuple{2} = \big(R_2, R_3, \alpha_1, \alpha_2, S,\;
       \Enc(\pk_B, 0, \rho_2),\; \Enc(\pk_C, 0, \rho_3),\; C_\gamma\big) \]
  \[ C_\gamma = \Enc(\pk_A,\; S - u_1 v_1 + R_2 + R_3,\; \gamma) \]
  The eighth slot is the message Charlie returns to Alice, encrypted under her
  own key, and it is the same random variable in all three tuples: its
  plaintext is a function of the output Alice is allowed to learn and of her
  two masks, so no hop replaces it and it carries no relay input of its own.
...
\begin{definition}[Real-or-zero advantage of a single-query adversary]
  ...
  state and one challenge ciphertext. Encryption randomness is uniform, drawn
  as a coin index whose map to randomness carries a left inverse. Every
  statement over the scheme record therefore ranges over injective index maps,
  which is what lets a randomness recorded in a party's trace be read back as
  the coin naming it.
  The challenger at hidden bit $b$ is the encryption law of $p_A$ at $b = 1$ and the
...
\begin{lemma}[Guessing at the all-zero endpoint]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.AliceSpectatorPre,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_hop_secrecy.spectator_pre_indep,
  ...
  most $1/m$. The all-zero tuple is a function of the leaked output and of a
  spectator independent of the honest inputs, hence conditionally independent
  of $V_2$ given the output, and conditioned on the output alone $V_2$ is
  uniform.
...
\begin{definition}[The distinguisher of a predictor]
  ...
  \rocq{infotheo.computational_security.indcpa_game.predictor,
       infotheo.computational_security.indcpa_game.distinguisher_of_predictor,
...
\begin{definition}[The simulator and the ideal-world law]
  ...
  \rocqok
  \uses{def:alice_hop_ladder}
  From a value $s$ of the output, the simulator draws two uniform masks and one
  uniform coin record, and assembles the all-zero tuple: $s$ unchanged, an
  encryption of zero under each relay key, and Charlie's re-encryption under
  Alice's key of $s$ net of her own term and plus her masks. The ideal-world
  law samples the honest input pair, computes its output, and returns that pair
  beside a simulated tuple.
  \[ \Sim(s) = \big(R_2, R_3, \alpha_1, \alpha_2, s,\;
       \Enc(\pk_B, 0, \rho_2),\; \Enc(\pk_C, 0, \rho_3),\;
       \Enc(\pk_A, s - u_1 v_1 + R_2 + R_3, \gamma)\big) \]
  \[ \Ideal = \big[(v_2,v_3) \sim \mathcal{U};\ t \sim \Sim(S(v_2,v_3));\
       (v_2, v_3, t)\big] \]
  The masks and the six coins are drawn uniformly and independently of the
  honest inputs, so $s$ is the only thing the simulator reads and the output
  slot and the re-encryption slot are the only places it appears.
\end{definition}
...
\begin{definition}[Drawing a coin inside a program]
  \label{def:interpreter_sample}
  \rocq{infotheo.smc.smc_interpreter.Sample,
       infotheo.smc.smc_interpreter.step_sample,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.dsdp_seeds}
  \rocqok
  \uses{def:protocol_flow,def:dsdp_enc_coins}
  A party program draws its encryption randomness at a $\mathrm{Sample}$ node.
  Each party carries a seed stream beside its trace, and a step at a
  $\mathrm{Sample}$ substitutes the head of that party's stream into the
  continuation, advances the stream, and records the drawn value at the head of
  that party's trace.
  \[ \mathrm{step}\big(\mathrm{Sample}\ f,\ \mathit{tr},\ r :: \mathit{sd}\big)
       = \big(f(r),\ r :: \mathit{tr},\ \mathit{sd}\big) \]
  The three DSDP streams are the six coins of one record, two per party in the
  order that party encrypts. A party's trace holds every coin that party drew,
  beside the image of those coins through its program, so Alice's trace holds
  her two combine coins and no coin of another party. The observation the
  bounds below are stated at therefore contains the randomness Alice drew
  herself, so an adversary reading it is granted her coins along with her
  messages.
\end{definition}
...
\begin{definition}[Alice's executed trace]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.trace_dataT,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_traceT,
  ...
  \rocqok
  \uses{def:protocol_flow,def:alice_hop_ladder,def:interpreter_sample}
  ...
  eighteen entries, each a plaintext, a ciphertext, a coin, or a key mark.
  Alice's trace is that reader at the DSDP protocol and at Alice.
...
\begin{lemma}[Decoding Alice's trace at her own key]
  ...
  itself, a ciphertext as itself, a coin as the randomness it names, and the
  private-key mark as that key. At Alice's own key pair the round trip is
  exact, her trace carrying no public-key mark to constrain.
...
\begin{lemma}[Alice's trace holds her own key only]
  \label{lem:raw_trace_priv_keys}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.trace_priv_keys,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_raw_trace_priv_keysE}
  \rocqok
  \uses{lem:raw_trace_decode,lem:trace_of_hop_tuple}
  Reading the private keys off a raw interpreter trace keeps the key marks and
  drops every other entry. At Alice's raw trace the result is the one-element
  list of her own key.
  \[ \mathrm{priv\_keys}\big(\mathrm{AliceRawTrace}(s)\big)
       = [\,\mathrm{dk}_A\,] \]
  That a corrupted Alice holds no relay's private key is the premise the two
  hops of Chapter~\ref{ch:hopping} are stated under, and here it is an equation
  about the executed run rather than a condition imposed on the adversary. The
  boundary is sharp on the other side: a party that does hold Bob's key reads
  his input off the same trace with probability one,
  by Theorem~\ref{thm:decrypt_lower_bound}.
\end{lemma}
...
\begin{theorem}[Simulation at the raw trace]
  \label{thm:raw_trace_simulation}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_raw_trace_sim_advantage_le}
  \rocqok
  \uses{thm:alice_trace_bounds,lem:raw_trace_decode}
  A Boolean test tells the real raw trace from the simulated one no more often
  than the two hop advantages of its decoded lift.
  \[ \big| \Pr[\,D(\mathrm{Real})\,]
       - \Pr[\,D(\mathrm{Ideal})\,] \big|
     \le \epshopsum{D} \]
  The programs draw the re-encryption coin, so the statement is one about the
  protocol rather than about one of its executions, and both terms of the
  bound are conditional on the IND-CPA assumption, one at Bob's key and one at
  Charlie's.
  Proved in \texttt{dsdp\_alice\_main.v} as
  \texttt{alice\_raw\_trace\_sim\_advantage\_le}.
\end{theorem}

\begin{theorem}[Trace and tuple leave the same uncertainty]
  \label{thm:centropy_trace_tuple}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_of_hop_tuple,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.hop_tuple_of_alice_trace,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.alice_trace_of_hop_tupleK,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_trace_link.centropy_V2_hop_tupleE}
  \rocqok
  \uses{def:alice_trace,lem:trace_of_hop_tuple}
  Alice's executed trace and her hop-$0$ tuple carry the same eight values, the
  trace writing her two combine coins at slots $2$ and $3$, and
  $\mathrm{hop\_tuple\_of\_alice\_trace}$ reads those eight slots back.
  Encoding the tuple into a trace is left-invertible, so conditioning on either
  object leaves the same uncertainty about Bob's input, for all weights.
  \[ \HH(V_2 \mid \mathrm{AliceTrace}) = \HH(V_2 \mid \hoptuple{0}) \]
  The equality is a relabelling of one object as the other, and no coordinate
  of the trace is claimed to be uninformative about Bob's input.
...
\begin{lemma}[An inverse of an exponentially growing sequence is negligible]
  \label{lem:negligible_fun_inv_exp2}
  \rocq{infotheo.computational_security.negligible.negligible_fun_inv_exp2,
       infotheo.computational_security.negligible.negligible_fun_inv_ge_exp2,
       infotheo.computational_security.negligible.negligible_fun_inv_expnn,
       infotheo.computational_security.negligible.negligible_fun_inv_ge_expnn}
  \rocqok
  \uses{def:negligible_fun}
  A sequence of naturals that dominates $2^k$, and one that dominates
  $(k+2)^{k+2}$, have negligible inverses.
  \[ \big(\forall k,\ 2^k \le f(k)\big) \;\Longrightarrow\;
     \mathrm{negl}\big(k \mapsto 1/f(k)\big) \]
  This is where the security parameter becomes a bit length. A scheme sequence
  whose modulus at $k$ has $k$ bits discharges the unconditional term of every
  bound read along it here, so that term is a consequence of the key length
  rather than a hypothesis carried beside the scheme.
\end{lemma}
...
\begin{corollary}[The bound at a Paillier instance]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.paillier_trace_guess_V2_admissible_pq_le,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.paillier_epsilon_dcrE}
  \rocqok
  ...
  identity \texttt{paillier\_epsilon\_dcrE}. What is assumed here is
...
\begin{corollary}[The bound at a Benaloh instance]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.benaloh_trace_guess_V2_admissible_pq_le,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.benaloh_epsilon_residuosityE}
  \rocqok
  ...
  \texttt{benaloh\_epsilon\_residuosityE}.
\end{corollary}
...
\begin{definition}[Alice's trace argument as a chain opening at the protocol]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.alice_label,
       ...
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_main.accept_hop_tupleE,
...
\begin{definition}[The decrypting predictor]
  ...
  The predictor $\delta$ reads Bob's ciphertext off slot $5$ of Alice's trace
  and decrypts it with Bob's private key. Correctness of decryption makes Bob's
  input a function of the trace.
  \[ \delta(t) = \Dec(dk_B,\, t_5),\qquad V_2 = \delta \circ \mathrm{AliceTrace} \]
...
\begin{definition}[The key material of a scheme sequence]
  \label{def:keygen_sequence}
  \rocq{infotheo.computational_security.indcpa_scheme_sequence.keygen_sequence}
  \rocqok
  \uses{def:indcpa_epsilon}
  Key material for a sequence of schemes $\mathcal{S}_k$ is a nonempty finite
  seed space at each $k$ together with the private key a seed generates there.
  \[ \mathcal{K} = \Big(k \mapsto \mathrm{Seed}_k,\
       k \mapsto \big(\mathrm{Seed}_k \to \mathrm{priv}(\mathcal{S}_k)\big)\Big) \]
  A private key is a bare type, over which no distribution is well-typed, so
  the seed space is where a key generation law would live and a party's key is
  named by a seed rather than drawn. Nonemptiness is recorded in the successor
  form the uniform law on a finite type takes.
\end{definition}

\begin{definition}[A sequence of IND-CPA schemes]
  \label{def:indcpa_scheme_sequence}
  \rocq{infotheo.computational_security.indcpa_scheme_sequence.indcpa_scheme_sequence,
       infotheo.computational_security.indcpa_scheme_sequence.f_size_scheme,
       infotheo.computational_security.indcpa_scheme_sequence.f_adv_scheme}
  \rocqok
  \uses{def:keygen_sequence,def:indcpa_assumption,def:negligible_fun}
  A scheme sequence is an IND-CPA scheme at each security parameter, the
  assumption made at it, key material along it, and the two negligibility facts
  every bound read along it carries, as one record.
  \[ \mathcal{Q} = \Big(k \mapsto \mathcal{S}_k,\ k \mapsto \mathcal{A}_k,\
       \mathcal{K},\
       \mathrm{negl}\big(k \mapsto \tfrac{1}{m_k}\big),\
       \mathrm{negl}\big(k \mapsto \varepsilon_{\mathrm{cpa},k}\big)\Big) \]
  The assumption at $k$ is made about the scheme value the record carries, so
  the two name one coin space. At this layer $k$ is read as a key length: each
  concrete sequence below carries the bit length of its modulus at $k$ and
  derives $\mathrm{negl}(k \mapsto 1/m_k)$ from it, which puts that fact where
  the scheme is built rather than where a bound is stated.
\end{definition}
...
\begin{definition}[One DSDP instance]
  ...
  unit of the plaintext ring, and the three private keys: exactly the
  parameters every corrupted-Alice trace bound is stated at, so every such
  bound applies at one instance unchanged. The scheme enters as one component,
  so an instance and the assumption made about it name the same scheme.
  \[ \mathcal{I} = \big(\mathcal{S},\
       v_1, u_1, u_2, u_3,\ dk_A, dk_B, dk_C\big) \]
  The encryption coins are drawn by the run, at the $\mathrm{Sample}$ nodes of
  Definition~\ref{def:interpreter_sample}, so they are coordinates of the
  experiment rather than fields of the instance.
...
\begin{definition}[The DSDP sequence a scheme sequence builds]
  \label{def:mk_dsdp_instance_sequence}
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.dsdp_alice_data,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.mk_dsdp_instance,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.mk_dsdp_instance_sequence,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_instance.mk_dsdp_asymptotic}
  \rocqok
  \uses{def:indcpa_scheme_sequence,def:dsdp_instance_sequence,def:dsdp_asymptotic}
  The corrupted-Alice data along a scheme sequence $\mathcal{Q}$ is Alice's
  input, her three weights with $u_3$ a unit at every $k$, and the three seeds
  the parties' keys are generated from. From $\mathcal{Q}$ and one such record
  the instance at $k$, the sequence of instances, and an asymptotic value for
  that sequence are read off.
  \[ \mathcal{I}_k = \big(\mathcal{S}_k,\
       v_{1,k}, u_{1,k}, u_{2,k}, u_{3,k},\
       \kappa_k(s_A), \kappa_k(s_B), \kappa_k(s_C)\big) \]
  The asymptotic value adds no hypothesis: its two fields are the two
  negligibility fields of $\mathcal{Q}$. What a file stating a bound in the
  security parameter would otherwise assume beside its scheme variable is
  therefore discharged at the scheme sequence.
\end{definition}
...
\begin{theorem}[The trace simulation distance sequence is negligible]
  ...
  $\mathcal{N}$, and a sequence $D_k$ of tests
  ...
  second time, at the sequence of tests rather than at the test a predictor
  induces, because a statement made at the predictor's own test would be
  weaker than a statement quantified over every test.
\end{theorem}

\begin{definition}[The idealized scheme sequence]
  \label{def:idealized_scheme_sequence}
  \rocq{infotheo.computational_security.idealized_indcpa_scheme.idealized_scheme,
       infotheo.computational_security.idealized_indcpa_scheme.card_plain_idealized,
       infotheo.computational_security.idealized_indcpa_scheme.idealized_keygen,
       infotheo.computational_security.idealized_indcpa_scheme.idealized_size_negligible,
       infotheo.computational_security.idealized_indcpa_scheme.idealized_scheme_sequence}
  \rocqok
  \uses{def:indcpa_scheme_sequence,def:cipher_constant_assumption,lem:negligible_fun_inv_exp2}
  The idealized scheme at $k$ is the idealized AHE scheme over the plaintext
  ring $\mathbb{Z}_{(k+2)^{k+2}}$, its key material a one-element seed space
  and the zero private key, and its assumption the cipher-constant one.
  \[ \#\lvert\mathrm{plain}(\mathcal{S}_k)\rvert = (k+2)^{k+2}, \qquad
     \varepsilon_{\mathrm{cpa},k} = 0 \]
  It inhabits Definition~\ref{def:indcpa_scheme_sequence} under no assumption
  at all: the assumed advantage is zero at every $k$ and the plaintext
  cardinality outgrows every polynomial, so both negligibility fields are
  proved where the sequence is built. A bound read along it keeps only its
  unconditional term.
\end{definition}
...
\begin{definition}[The idealized witnesses]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.counting.dsdp_random_inputs.uniform_inputs,
       infotheo.dumas2017dual.dsdp.counting.dsdp_random_inputs.uniform_coins_indep,
  ...
  \rocqok
  \uses{def:cipher_constant_assumption,def:negligible_fun,def:idealized_scheme_sequence}
  ...
  constants $0$, $0$, $1$ and the three keys constant as well. The six coins
  are held at a one-point space, so their joint independence of the eleven
  inputs is proved where the witness is built and assumes nothing. A weight
  drawn uniformly takes the value $0$ at some sample, and Charlie's weight has
  to be invertible at every sample, which is why the weights stay outside the
  sample
...
\begin{definition}[The Paillier scheme sequence]
  \label{def:paillier_scheme_sequence}
  \rocq{infotheo.computational_security.paillier_indcpa_scheme.paillier_sequence,
       infotheo.computational_security.paillier_indcpa_scheme.f_size_paillier_negligible,
       infotheo.computational_security.paillier_indcpa_scheme.f_adv_paillier_negligible,
       infotheo.computational_security.paillier_indcpa_scheme.paillier_scheme_sequence}
  \rocqok
  \uses{def:indcpa_scheme_sequence,def:dcr_assumption,def:paillier_indcpa_assumption,lem:negligible_fun_inv_exp2}
  A Paillier sequence is two factors $p_k, q_k$ above one, a decisional
  composite residuosity record at their product, the bit length of that
  product, negligibility of the assumed residuosity advantage, and key
  material.
  \[ 2^k \le p_k q_k, \qquad
     \mathrm{negl}\big(k \mapsto \varepsilon_{\mathrm{dcr},k}\big) \]
  The bit-length field is what makes $k$ a key length here: the plaintext space
  at $k$ is $\mathbb{Z}_{p_k q_k}$, so $\mathrm{negl}(k \mapsto 1/m_k)$ follows
  from it by Lemma~\ref{lem:negligible_fun_inv_exp2}. Reading the record as a
  scheme sequence therefore assumes residuosity and that bit length and nothing
  else, both negligibility fields of
  Definition~\ref{def:indcpa_scheme_sequence} being derived, the
  assumption-conditional one at twice $\varepsilon_{\mathrm{dcr},k}$.
\end{definition}
...
\begin{definition}[The Paillier instance sequence]
  ...
  \rocqok
  \uses{def:dsdp_instance_sequence,def:dcr_assumption,def:paillier_indcpa_assumption,def:paillier_scheme_sequence,def:mk_dsdp_instance_sequence}
  ...
  $u_3$ a unit of the plaintext ring and the three private keys their seeds
  generate, and whose assumption at $k$ is the IND-CPA record
...
\begin{corollary}[The Paillier readings]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.paillier_trace_guess_V2_negligible,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.paillier_assumption_dcrE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.paillier_epsilon_dcrE,
  ...
  Along the Paillier sequence a predictor sequence admissible at every $k$
  ...
  \texttt{paillier\_assumption\_dcrE} and
  \texttt{paillier\_epsilon\_dcrE} identify the record every
  ...
  asymptotic value, one of the two negligible functions
  Theorem~\ref{thm:alice_guess_negligible} consumes.
\end{corollary}

\begin{definition}[The Benaloh scheme sequence]
  \label{def:benaloh_scheme_sequence}
  \rocq{infotheo.computational_security.benaloh_indcpa_scheme.benaloh_sequence,
       infotheo.computational_security.benaloh_indcpa_scheme.f_size_benaloh_negligible,
       infotheo.computational_security.benaloh_indcpa_scheme.f_adv_benaloh_negligible,
       infotheo.computational_security.benaloh_indcpa_scheme.benaloh_scheme_sequence}
  \rocqok
  \uses{def:indcpa_scheme_sequence,def:benaloh_residuosity_assumption,def:benaloh_indcpa_assumption,lem:negligible_fun_inv_exp2}
  A Benaloh sequence is a ciphertext modulus $n_k$ and a plaintext block size
  $r_k$, both above one, an $r_k$-th residuosity record at $n_k$, a bit length
  for each of the two, negligibility of the assumed residuosity advantage, and
  key material.
  \[ 2^k \le r_k, \qquad 2^k \le n_k, \qquad
     \mathrm{negl}\big(k \mapsto \varepsilon_{\mathrm{res},k}\big) \]
  The two bit lengths measure different things and the record keeps them apart.
  The block size is the plaintext space, so $2^k \le r_k$ is what
  $\mathrm{negl}(k \mapsto 1/m_k)$ is derived from; the modulus is what a
  Benaloh key is, so $2^k \le n_k$ is the key length. Reading the record as a
  scheme sequence derives both negligibility fields of
  Definition~\ref{def:indcpa_scheme_sequence}, the assumption-conditional one
  at twice $\varepsilon_{\mathrm{res},k}$.
\end{definition}
...
\begin{definition}[The Benaloh instance sequence]
  ...
  \rocqok
  \uses{def:dsdp_instance_sequence,def:benaloh_residuosity_assumption,def:benaloh_indcpa_assumption,def:benaloh_scheme_sequence,def:mk_dsdp_instance_sequence}
  The sequence whose $k$-th instance is the Benaloh scheme at ciphertext
  modulus $n_k$ and block size $r_k$, carrying the same weights and keys as
  the Paillier sequence, and whose assumption at $k$ is the IND-CPA
...
\begin{corollary}[The Benaloh readings]
  ...
  \rocq{infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.benaloh_trace_guess_V2_negligible,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.benaloh_assumption_residuosityE,
       infotheo.dumas2017dual.dsdp.hopping.dsdp_alice_instances.benaloh_epsilon_residuosityE,
  ...
  Along the Benaloh sequence a predictor sequence admissible at every $k$
  matches Bob's input with negligible probability, and at one fixed instance
  the class-conditional trace bound is written in residuosity epsilons, at
  the two spellings of its unconditional term.
  \[ \mathrm{negl}\big(k \mapsto
  ...
  \texttt{benaloh\_assumption\_residuosityE} and
  \texttt{benaloh\_epsilon\_residuosityE} identify the record every
  assumption-conditional bound along the sequence is read at with the one
  derived from $\mathcal{B}_k$, and its advantage with
...
\begin{theorem}[Relay privacy of $V_1$]
  ...
  relay sees, so a corrupted relay learns nothing about it. Each view holds the
  two coins that relay draws beside its key, its input and the ciphertexts it
  receives, and the bound stays at $\log m$ with them in it.
...
\begin{lemma}[The relay view is independent of $V_1$]
  ...
  \rocqok
  The coin-free part of each relay's view, its key with its input and the
  ciphertexts it receives, is a deterministic image of data assumed independent
  of $V_1$, and the two coins that relay draws are independent of that part
  taken together with $V_1$, so the view carrying its coins is independent of
  $V_1$.
...
\begin{proof}
  ...
  \texttt{inde\_RV\_comp}, which leaves the coin-free part independent of
  $V_1$. The field \texttt{coins\_indep} of the inputs record makes the six
  coins of a run jointly independent of the eleven inputs, and an
  \texttt{inde\_RV\_comp} image of it makes the relay's own two coins
  independent of the pair of $V_1$ with that part. Contraction,
  \texttt{inde\_RV\_contraction}, joins the two into independence of $V_1$
  from the part with its coins, and a last \texttt{inde\_RV\_comp} reorders
  that tuple into the view.
\end{proof}
...
\begin{theorem}[Bob's view hides $V_3$]
  ...
  fresh one-time pad $R_3$, so Bob's view is independent of $V_3$. Bob's two
  coins are in that view, and the record makes the six coins of a run jointly
  independent of the eleven inputs, so they leave the bound where it is.
...
\begin{theorem}[Charlie's view hides $V_2$]
  ...
  $V_2$. Charlie's two coins are in that view under the same coin law, and they
  leave the bound where it is.
...
\chapter{The relay bounds at the executed trace}

The three bounds above are stated at a tuple standing for what a relay observes.
Running the three DSDP party programs at a symbolic cipher and reading the trace
the interpreter writes for a relay makes that tuple the object the executed
protocol hands it.

\begin{definition}[The symbolic DSDP interface]
  \label{def:symbolic_interface}
  \rocq{infotheo.dumas2017dual.dsdp.core.dsdp_interface.Symbolic_DSDP_Interface}
  \rocqok
  \uses{def:protocol_flow}
  % library: dsdp_interface.v is cited in prose and is outside the coqdoc
  %   MODULES scope, so its symbolic plumbing takes no exclude entry.
  The symbolic instance of the DSDP interface takes plaintexts in the plaintext
  ring, a ciphertext as a plaintext under a party label, coins in a bare finite
  type, and a private key as a party key mark. Encryption ignores its coin and
  returns the labelled plaintext, and decryption returns that plaintext at the
  party the label names and at a decryption key.
  \[ \mathrm{party\_E}(i, x, r) = (i, x), \qquad
     \mathrm{party\_D}\big((i, \mathrm{Dec}, k), (i, x)\big)
       = \mathrm{Some}\ x \]
  The three piSMC programs that run at it are the programs of
  Definition~\ref{def:protocol_flow}, instantiated at this interface rather
  than at a scheme. Every carrier is finite, so a trace of a run at it is a
  sequence over a finite alphabet and needs no encoding into one.
\end{definition}

\begin{definition}[The symbolic run and its three traces]
  \label{def:symbolic_run}
  \rocq{infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.dsdp_run_traces_symbolicE}
  \rocqok
  \uses{def:symbolic_interface,def:interpreter_sample}
  The interpreter runs the three programs for eighteen rounds on seed streams
  read off the six coin coordinates of one sample, two per party. It writes
  thirteen entries for Alice, six for Bob and five for Charlie, newest first.
  Bob's six are his second coin $r_{b2}$, the Charlie-key combine and the
  Bob-key combine he receives, his first coin $r_{b1}$, his input $V_2$ and his
  key mark $k_b$. Charlie's five are his second coin, the aggregate ciphertext
  Bob forwards, his first coin, his input $V_3$ and his key mark.
  \[ \mathrm{BobTrace} = \big[\, r_{b2},\ (\mathrm{Charlie},\ V_3 U_3 + R_3),\
     (\mathrm{Bob},\ V_2 U_2 + R_2),\ r_{b1},\ V_2,\ k_b \,\big] \]
  \[ \mathrm{CharlieTrace} = \big[\, r_{c2},\
     (\mathrm{Charlie},\ V_3 U_3 + R_3 + V_2 U_2 + R_2),\ r_{c1},\ V_3,\ k_c
     \,\big] \]
  Alice's thirteen entries are stated by the same equality and no bound is read
  at them. Her axis is the hopping one, at the real scheme, where
  Lemma~\ref{lem:trace_of_hop_tuple} states the matching identity.
\end{definition}

\begin{lemma}[A relay's trace is its view recoded]
  \label{lem:relay_trace_of_view}
  \rocq{infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.symbolic_trace_of_run,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.bob_trace_of_viewE,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.charlie_trace_of_viewE,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.bob_trace_of_viewK,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.charlie_trace_of_viewK}
  \rocqok
  \uses{def:symbolic_run,def:party_views}
  The trace the interpreter produces for a relay is the image of that relay's
  view under a map $\rho$, and a reader $\rho'$ inverts $\rho$ on the left, so
  the trace and the view are the same observation written twice.
  \[ \mathrm{RelayTrace} = \rho \circ \mathrm{RelayView}, \qquad
     \rho' \circ \rho = \mathrm{id} \]
  The reader is total, returning a fixed tuple on the entry shapes no trace of
  the run takes, and $\rho$ is where the coins of
  Definition~\ref{def:party_views} earn their place: the trace records them, so
  a view without them could not be the same observation.
\end{lemma}

\begin{theorem}[The relay bounds at the trace]
  \label{thm:relay_trace_privacy}
  \rocq{infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.bob_trace_privacy_V1,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.bob_trace_privacy_V3,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.charlie_trace_privacy_V1,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.charlie_trace_privacy_V2,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.centropy_V1_bob_traceE,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.centropy_V3_bob_traceE,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.centropy_V1_charlie_traceE,
       infotheo.dumas2017dual.dsdp.counting.dsdp_relay_trace.centropy_V2_charlie_traceE}
  \rocqok
  \uses{lem:relay_trace_of_view,thm:bob_privacy_V1,thm:bob_privacy_V3,thm:charlie_privacy_V2}
  Conditioning on a relay's trace is conditioning on its view, by the identity
  of Lemma~\ref{lem:relay_trace_of_view}, so each bound of the three chapters
  above is read at the trace the executed protocol hands that relay.
  \[ \HH(V_1 \mid \mathrm{BobTrace}) = \log m > 0, \qquad
     \HH(V_3 \mid \mathrm{BobTrace}) = \log m > 0 \]
  \[ \HH(V_1 \mid \mathrm{CharlieTrace}) = \log m > 0, \qquad
     \HH(V_2 \mid \mathrm{CharlieTrace}) = \log m > 0 \]
  These four are at the symbolic cipher model, where a ciphertext exposes its
  plaintext and no ciphertext depends on a coin, so the coin slots the trace
  records are independent components of it rather than the randomness any
  ciphertext in it was built with. Alice's trace theorems of
  Chapter~\ref{ch:trace_link} are at the real scheme instead, which is why hers
  carry an assumption-conditional term and these four carry none.
\end{theorem}
