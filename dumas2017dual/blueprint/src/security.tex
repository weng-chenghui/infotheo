...
\begin{definition}[The Paillier hybrid as a chain]
  ...
  $\mathcal{B}$, so this loss has one currency. The number in
...
\begin{definition}[Alice's ladder as a chain, and the bound read off it]
  ...
  \rocqok
  \uses{def:chain,def:chain_result,def:claim,def:loss,lem:alice_view_advantage,lem:guess_all_zero,thm:alice_guess_real}
  ...
  justified by Lemma~\ref{lem:alice_view_advantage}, which makes each gap
...
\begin{definition}[Alice's trace argument as a chain opening at the protocol]
  ...
  \rocqok
  \uses{def:alice_chain,def:alice_trace,lem:trace_of_hop_tuple}
  ...
  The two hops and the terminal statement are those of
  Definition~\ref{def:alice_chain} at the lifted test $D_{g \circ \tau}$, so
...
\begin{definition}[Alice's trace argument at the assumed advantage]
  ...
  it, one in the justification of each hop, which is the shape
  Definition~\ref{def:paillier_chain} has. The terminal label costs the same
