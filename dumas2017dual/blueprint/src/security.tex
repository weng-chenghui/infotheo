...
compose to the full bound $1/m + 2\,\epscpaof{E}$. The part closes with the
...
\begin{theorem}[Ciphertext-view advantage]
  ...
  Every valid adversary distinguishes DSDP's real corrupted-Alice game from its
  all-zero endpoint with advantage at most $2\,\epscpaof{E}$, twice the scheme's
  IND-CPA bound, because the corrupted-Alice view has exactly two encryption hops.
  \[ \AdvE(\real, \zero, A) \le 2\,\epscpaof{E} \]
\end{theorem}
...
\begin{theorem}[Guessing the challenge secret, real game]
  ...
  injectively, Alice guesses Bob's secret $V_2$ with probability at most
  $1/m + 2\,\epscpaof{E}$: the fiber bound $1/m$ at the all-zero endpoint plus the
  $2\,\epscpaof{E}$ cost of moving to the real game. The two legs join by the
  triangle inequality.
  \[ \Pr[\mathrm{guess} = V_2] \le 1/m + 2\,\epscpaof{E} \]
\end{theorem}
...
\begin{theorem}[Unpredictability entropy]
  ...
  Equivalently in entropy form, the predictor's unpredictability entropy on the
  output-exposing real game is at least $\log m - \log(1 + 2m\,\epscpaof{E})$,
  approaching the full $\log m$ as $\epscpaof{E} \to 0$.
  \[ \Hunp \ge \log m - \log\!\big(1 + 2m\,\epscpaof{E}\big) \]
\end{theorem}
...
inputs, up to the same $2\,\epscpaof{E}$. The scope is average-case: the honest
...
\begin{theorem}[Bounded simulation security]
  ...
  The output-exposing real game and the simulator composed with the ideal
  functionality are distinguished with advantage at most $2\,\epscpaof{E}$ by any
  valid admissible adversary. The simulator fabricates the four-cipher view from
  encryptions of zero and fresh samples; the honest inputs and $S$ have no path
  into it, a type-level allowed-information witness.
  \[ \AdvE(\real_S,\ \Sim \circ \Ideal,\ A) \le 2\,\epscpaof{E} \]
\end{theorem}
\begin{proof}
  \uses{lem:simulator_factorization}
  \rocqok
  Triangle through the all-zero endpoint: the real-to-zero distance is
  $2\,\epscpaof{E}$ (Part~\ref{part:autoderiv}), and the zero endpoint is perfectly
...
\begin{theorem}[View-law statistical distance]
  \label{thm:alice_view_statdist}
  \rocq{infotheo.dumas2017dual.dsdp.dsdp_main.dsdp_alice_view_statdist_le}
  \rocqok
  \uses{thm:alice_simulation_secure}
  The statistical distance between the real corrupted-Alice view law (the received
  ciphers together with the leaked output $S$) and the simulated one is at most
  $2\,\epscpaof{E}$, same average-case scope.
  \[ \statdist(\text{real view law},\ \text{simulated view law}) \le
        2\,\epscpaof{E} \]
\end{theorem}
