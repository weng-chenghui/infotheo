...
ciphertexts she receives leak computationally, bounded by the IND-CPA advantages
of the two reductions a distinguisher induces on the scheme, one per encryption
hop. The scalar-product output $S$ she decrypts leaks information-theoretically,
bounded by $1/m$, with $m$ the size of the message
...
that form and bounded information-theoretically. The full bound, for a
distinguisher $A$, is $1/m + \epshopsum{A}$.

The single-hop IND-CPA advantage is a function $\epscpaof{E,R}$ of an encryption
scheme $E$ and a reduction package $R$: the \ssprove{} advantage of $R$ between
the real-encryption and the zero-encryption oracle of $E$. Every computational
bound below is such an advantage at an explicitly constructed reduction, so no
scheme-independent constant bounding it appears anywhere; a scheme that returns
its plaintext admits a reduction reaching $1$. For a concrete scheme, that these
advantages are small is a cryptographic assumption about the scheme, recorded in
prose here rather than as a formal hypothesis. All bounds below are for DSDP's
fixed scheme $E$: a distinguisher $A$ against a DSDP game induces one hop
reduction $A \circ \shimof{i}$ per encryption hop, and $\epshopsum{A}$
abbreviates the sum of the advantages of the two of them.
...
ciphertext channel by $\epshopsum{A}$, every choice the argument depends on
gathered in one record from which a generic theorem reads off the advantage. It
is the worked instance of the derivation framework developed in the thesis part
on the derived game. Part~II is a different tool. The output $S$ is the plaintext
scalar
...
then composes that $1/m$ with the Part~I two-reduction sum across the triangle
inequality.

Game hopping reduces a protocol's ciphertext privacy to a chain of IND-CPA swaps
costing one reduction advantage per hop, $k$ of them for $k$ hops. The method
takes the chain of games as given, and someone writes that chain once per
protocol: the real game, every intermediate game, and the oracle each hop calls.
That hand-writing is the
...
syntax. That bound is a sum, over the encryption hops the game contains, of the
IND-CPA advantage of the reduction routing that hop through the oracle. The walk
that produces the view runs to the decryption Alice performs, so the same denoted
game also exposes the
...
\node[box, text width=3.2cm] (bound)  at (1.9,-5.6) {Computational bound $\varepsilon_0 + \varepsilon_1$};
...
\node[box, text width=4.6cm] (compose) at (4.4,-7.2) {Full corrupted-Alice secrecy $1/m + \varepsilon_0 + \varepsilon_1$};
...
through a hybrid ladder of $k$ IND-CPA hops, generically one reduction advantage
per hop and $\varepsilon_0 + \varepsilon_1$ for DSDP's two hops, where
$\varepsilon_i = \epscpaof{E,\, A \circ \shimof{i}}$ is the IND-CPA advantage of
the reduction a distinguisher $A$ induces on hop $i$. The output channel (right,
Part~II) is bounded by the fiber argument $1/m$. The two compose to
$1/m + \varepsilon_0 + \varepsilon_1$.}
...
adversary's advantage by the sum, over the hops, of its site reductions'
advantages. The DSDP instance fills the record, its hop count computes to two,
and the bound reads off as $\epshopsum{A}$.
...
telescopes the ladder of endpoints into a sum, over the encryption hops, of the
IND-CPA advantages of the reductions that route one hop through the oracle.
...
\begin{definition}[One-hop oracle routing]
  ...
  real or zero oracle reproduces one rung of the ladder. Write $\shimof{i}(g)$
...
\begin{lemma}[Adjacent rungs cost one query]
  ...
  at most the IND-CPA advantage, against the scheme $E$ the game denotes into, of
  the reduction that routes hop $i$ through the oracle.
  \[ \AdvE(\sem{g^{(i)}}, \sem{g^{(i+1)}}, A)
       \le \epscpaof{E,\, A \circ \shimof{i}(g)} \]
\end{lemma}
\begin{proof}
  \uses{lem:hop_equiv,def:indcpa_assumption,def:hybrid_endpoints}
  \rocqok
  By a triangle chain through the two shim compositions, then folding the shim
  into the adversary so the middle hop is that reduction's advantage.
\end{proof}
...
\begin{lemma}[Telescoping bound]
  ...
  \rocqok
  A contiguous block of rungs costs at most one hop-reduction advantage per rung.
  \[ \text{a block of } n{+}1 \text{ rungs starting at } s
       \ \le\ \sum_{l < n+1} \epscpaof{E,\, A \circ \shimof{s+l}(g)} \]
\end{lemma}
\begin{proof}
...
\begin{theorem}[Generic hybrid bound]
  ...
  the real and all-zero denoted games is at most the sum, over the encryption hop
  sites, of the IND-CPA advantages of the site reductions.
  \[ \AdvE(\sem{g^{(0)}},\sem{g^{(\nhops(g))}},A)
       \ \le\ \sum_{l < |\sites(g)|} \epscpaof{E,\, A \circ \shimof{l}(g)} \]
\end{theorem}
\begin{proof}
...
advantage by the sum, over the hops, of the IND-CPA advantages of its site
reductions against the record's scheme.
...
\begin{theorem}[Generic corrupted-view secrecy]
  ...
  between the real game and the all-zero game is at most the sum, over the
  corrupted view's encryption hops, of the IND-CPA advantages of the site
  reductions against the scheme $E_P$ the record $P$ carries.
  \[ \AdvE(\real(P), \zero(P), A) \ \le\
        \sum_{l < \nhops(\view(P))}
          \epscpaof{E_P,\, A \circ \shimof{l}(\gtrace(\view(P)))} \]
\end{theorem}
\begin{proof}
...
off as the two-summand $\epshopsum{A}$ for DSDP's scheme, the whole
researcher-supplied input being the record's fields.
...
\begin{theorem}[DSDP corrupted-Alice secrecy]
  ...
  at most the sum of the IND-CPA advantages of its two hop reductions.
...
\begin{corollary}[Loose-argument form]
  ...
  \rocqok
  The same two-summand bound stated directly over the denoted games, packaging
  the loose arguments into the record.
  \[ \AdvE(\sem{g^{(0)}}_E, \sem{g^{(2)}}_E, A) \ \le\ \epshopsum{A} \]
\end{corollary}
\begin{proof}
...
% probability identity, and the result composes with the Part I two-reduction sum.
