...
\node[box] (comb)   at (7.4,3.5) {Derived combine terms};
\node[box, text width=3.0cm] (cfg) at (4.4,4.6) {Declared security-model configuration};
...
\node[box, text width=4.2cm] (bound) at (4.4,-5.4) {Advantage $\le k\,\epscpa$};
...
The symbolic run yields the derived combine terms. Together with a declared
security-model configuration they form an observation trace, which the front end
lowers to the reified game. The back end denotes that game into SSProve, then
bounds the advantage through a hybrid ladder of $k$ IND-CPA hops.}
