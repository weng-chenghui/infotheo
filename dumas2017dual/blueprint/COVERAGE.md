...
At this commit the checker exits 0: `code=475 blueprint=229 excl=246`, no
...
The scheme-sequence records carry the nodes `lem:negligible_fun_inv_exp2`,
`def:keygen_sequence`, `def:indcpa_scheme_sequence`,
`def:mk_dsdp_instance_sequence`, `def:idealized_scheme_sequence`,
`def:paillier_scheme_sequence` and `def:benaloh_scheme_sequence`, each citing
the two negligibility lemmas its record derives rather than assumes.
